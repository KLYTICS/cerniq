...
function build(
  ...
  const idp = {
    ...
    ensurePrincipalForOrg: vi.fn(
      async () => opts.ensureResult ?? { principalId: 'p_x', created: false },
    ),
  };
...
describe('Auth0Service.handleActionLogin', () => {
  it('writes APPROVED audit when MFA satisfied', async () => {
    ...
    const args = ((audit.append as unknown as jest.Mock).mock.calls[0]?.[0] ?? {}) as {
      decision?: string;
    };
...
describe('Auth0Service.exchangeToken', () => {
  it('rejects when adapter returns null (bad token)', async () => {
    ...
    await expect(svc.exchangeToken({ access_token: 'bad' })).rejects.toBeInstanceOf(
  ...
  it('rejects when org_id is missing', async () => {
    ...
    await expect(svc.exchangeToken({ access_token: 't' })).rejects.toBeInstanceOf(
      UnauthorizedException,
    );
  });
  ...
  it('returns API key with VERIFIED band when MFA satisfied', async () => {
    ...
    expect(r.api_key_id).toMatch(/^cerniq_live_/);
    expect(r.principal_id).toBe('p_acme');
    expect(r.roles).toEqual(['cerniq:admin']);
    const args = ((audit.append as unknown as jest.Mock).mock.calls[0]?.[0] ?? {}) as {
