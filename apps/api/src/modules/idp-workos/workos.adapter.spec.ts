...
function fakeWorkos(
  ...
  return {
    ...
    getOrganization: jest.fn(
      async () => opts.org ?? { id: 'org', name: 'Acme', domains: [{ domain: 'acme.com' }] },
    ),
  };
}
...
describe('WorkOsAdapter.verifyAccessToken', () => {
    ...
    const a = build(
      fakeWorkos({
        authResponse: {
          ...
          organizationId: 'org_1',
          roles: ['cerniq:admin', 'role:not-cerniq-prefixed'],
          mfaEnrolled: true,
          sessionId: 's_1',
          expiresAt: Math.floor(Date.now() / 1000) + 3600,
        },
  ...
  it('uses Redis cache on second call', async () => {
    const cachedUser = {
      idpUserId: 'cached',
      idpOrganizationId: 'org_c',
      idpDomain: 'cached.com',
      email: 'c@b.co',
      emailVerified: true,
      name: 'C',
      roles: [],
