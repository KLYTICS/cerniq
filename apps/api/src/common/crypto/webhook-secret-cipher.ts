...
//   - AAD = "okoro.webhook-secret.v1" — domain-separates this DEK from any
...
//   - Errors are typed via OkoroError (InternalError) — never raw throws.
...
const TAG_BYTES = 16;
const KEY_BYTES = 32;
const AAD = Buffer.from('okoro.webhook-secret.v1', 'utf8');
...
export class WebhookSecretCipher {
  ...
  constructor(config: AppConfigService) {
    ...
    if (provided) {
      ...
      if (buf.length !== KEY_BYTES) {
        ...
        throw new InternalError(
          `OKORO_WEBHOOK_SECRET_DEK_B64 decoded to ${buf.length} bytes; expected ${KEY_BYTES}.`,
        );
      }
      this.dek = buf;
      return;
    ...
    if (config.nodeEnv === 'production') {
      ...
      throw new InternalError(
        'OKORO_WEBHOOK_SECRET_DEK_B64 is required in production. Generate with: ' +
          'node -e "console.log(require(\'crypto\').randomBytes(32).toString(\'base64\'))"',
      );
    }

    // Dev/test: ephemeral key. Log so a developer can pin it across restarts
    // by exporting OKORO_WEBHOOK_SECRET_DEK_B64 — otherwise existing
    // ciphertexts in their local DB will fail to decrypt after a restart.
    this.dek = randomBytes(KEY_BYTES);
    this.logger.warn(
      'OKORO_WEBHOOK_SECRET_DEK_B64 not set — generated ephemeral DEK. ' +
        `Pin across restarts: OKORO_WEBHOOK_SECRET_DEK_B64=${this.dek.toString('base64')}`,
    );
  }
