...
// OkoroError; the HttpExceptionFilter maps them to the public envelope.
//
...
import type { ErrorCode } from '@okoro/types';
import { HttpException, HttpStatus } from '@nestjs/common';


import { ERROR_CATALOG, type ErrorCatalogEntry } from './error-catalog.js';

interface OkoroErrorOptions {
  details?: unknown;
  cause?: unknown;
}

export abstract class OkoroError extends HttpException {
  ...
  constructor(status: HttpStatus, message: string, opts: OkoroErrorOptions = {}) {
   ...
   * don't extend OkoroError (e.g. CircuitOpenError).
   *
 ...
 * type contract in @okoro/types stays unchanged; the discriminator for
 * client code is the error message + the `instanceof AlreadyRotatedError`
 * check on the server side.
 */
export class AlreadyRotatedError extends OkoroError {
  static override readonly catalogKey = 'AlreadyRotatedError';
  readonly code = 'CONFLICT' as const;
  constructor(message = 'This API key has already been rotated; rotate the active key instead.', opts?: OkoroErrorOptions) {
    super(HttpStatus.CONFLICT, message, opts ?? {});
  }
}

export class IdempotencyConflictError extends OkoroError {
  static override readonly catalogKey = 'IdempotencyConflictError';
  readonly code = 'IDEMPOTENCY_CONFLICT' as const;
  constructor(opts?: OkoroErrorOptions) {
    super(
      HttpStatus.CONFLICT,
...
export class RateLimitedError extends OkoroError {
  static override readonly catalogKey = 'RateLimitedError';
  readonly code = 'RATE_LIMITED' as const;
  constructor(retryAfterSeconds?: number, opts?: OkoroErrorOptions) {
...
export class InternalError extends OkoroError {
  static override readonly catalogKey = 'InternalError';
  readonly code = 'INTERNAL' as const;
  constructor(message = 'Internal server error.', opts?: OkoroErrorOptions) {
    super(HttpStatus.INTERNAL_SERVER_ERROR, message, opts ?? {});
  }
}

export class ServiceUnavailableError extends OkoroError {
  static override readonly catalogKey = 'ServiceUnavailableError';
  readonly code = 'SERVICE_UNAVAILABLE' as const;
  constructor(message = 'Service temporarily unavailable.', opts?: OkoroErrorOptions) {
 ...
 * response (see `DENIAL_REASON_PRECEDENCE` in @okoro/types).
 */
export class TrialExhaustedError extends OkoroError {
  static override readonly catalogKey = 'TrialExhaustedError';
  readonly code = 'BILLING' as const;
  constructor(message = 'Free trial verify cap reached. Upgrade to continue.', opts?: OkoroErrorOptions) {
