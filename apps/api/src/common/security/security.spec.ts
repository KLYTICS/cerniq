...
describe('buildCorsDelegate', () => {
  ...
  it('returns wildcard CORS for /v1/verify (public hot path)', () => {
    const delegate = buildCorsDelegate({ managementOrigins: 'https://app.cerniq.io' });
  ...
  it('reflects allow-listed origin for management endpoints with credentials', () => {
    const delegate = buildCorsDelegate({
      managementOrigins: 'https://app.cerniq.io,https://docs.cerniq.io',
    });
    delegate(
      { url: '/v1/agents/agt_abc', headers: { origin: 'https://app.cerniq.io' } } as never,
      cb,
    );
    const opts = cb.mock.lastCall?.[1];
    expect(opts.origin).toBe('https://app.cerniq.io');
  ...
  it('isWildcard correctly identifies "*"', () => {
    ...
    expect(isWildcard(' * ')).toBe(true);
    expect(isWildcard('https://app.cerniq.io')).toBe(false);
  });
});
