...
export class AppConfigService {
  ...
  /**
   * OKORO audit-chain + JWKS signing keypair.
   *
   * Canonical envs are `OKORO_SIGNING_PRIVATE_KEY` / `OKORO_SIGNING_PUBLIC_KEY`.
  ...
  get auditEd25519PrivateB64(): string | undefined {
    if (this.config.OKORO_SIGNING_PRIVATE_KEY) return this.config.OKORO_SIGNING_PRIVATE_KEY;
    if (this.config.AUDIT_ED25519_PRIVATE_KEY_B64) {
      this.logger.warn(
        'AUDIT_ED25519_PRIVATE_KEY_B64 is deprecated; rename to OKORO_SIGNING_PRIVATE_KEY before v0.2.',
      );
  ...
  get auditEd25519PublicB64(): string | undefined {
    if (this.config.OKORO_SIGNING_PUBLIC_KEY) return this.config.OKORO_SIGNING_PUBLIC_KEY;
    if (this.config.AUDIT_ED25519_PUBLIC_KEY_B64) {
      this.logger.warn(
        'AUDIT_ED25519_PUBLIC_KEY_B64 is deprecated; rename to OKORO_SIGNING_PUBLIC_KEY before v0.2.',
      );
  ...
  get okoroSigningPublicKey(): string | undefined {
    return this.config.OKORO_SIGNING_PUBLIC_KEY;
  }
  get okoroSigningKeyRotatedAt(): string | undefined {
    return this.config.OKORO_SIGNING_KEY_ROTATED_AT;
  }
  ...
  get webhookSecretDekB64(): string | undefined {
    return this.config.OKORO_WEBHOOK_SECRET_DEK_B64;
  }
  ...
  /** Lookup a Stripe price id by OKORO plan tier. Returns undefined for FREE. */
