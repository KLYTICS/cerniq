...
//
// Phase 1 reads `AEGIS_DASHBOARD_API_KEY` from env (the dashboard's own
...
export async function getSession(): Promise<DashboardSession | null> {
  ...
  const principalId = process.env.AEGIS_DASHBOARD_PRINCIPAL_ID;
  const email = process.env.AEGIS_DASHBOARD_EMAIL ?? 'developer@local';
...
export async function getSessionApiKey(): Promise<string | null> {
  ...
  const fromEnv = process.env.AEGIS_DASHBOARD_API_KEY;
  return fromEnv && fromEnv.length > 0 ? fromEnv : null;
}

export function authConfigured(): boolean {
  return Boolean(process.env.AEGIS_DASHBOARD_API_KEY);
}
