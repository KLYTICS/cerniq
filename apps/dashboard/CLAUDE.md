# AEGIS Dashboard - Claude contract
...
- Dashboard assumptions must come from `@aegis/types`, API discovery endpoints,
...
- Pricing is the canonical example: `resolvePricing()` should prefer
  `/.well-known/pricing.json` via `AEGIS_API_BASE_URL`, expose provenance, and
...
- Dashboard build for broad UI/router changes: `pnpm --filter @aegis/dashboard build`
