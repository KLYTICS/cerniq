...
export interface SubscribeResult {
  ok: true;
  id: string;
  /** Shown ONCE on creation. Caller must record it; OKORO only stores the bcrypt hash. */
  secret: string;
}
...
export async function subscribeWebhook(formData: FormData): Promise<SubscribeOutcome> {
  ...
  try {
    ...
    if (err instanceof OkoroAuthMissingError) {
      return { ok: false, error: 'Dashboard not authorized — set OKORO_DASHBOARD_API_KEY.' };
    }
