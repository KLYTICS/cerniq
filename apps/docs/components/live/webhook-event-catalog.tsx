import { WEBHOOK_EVENT } from '@okoro/types';
      ...
      <table className="w-full text-sm">
        <thead className="bg-[var(--okoro-steel)] text-xs uppercase tracking-wider text-[var(--okoro-fog)]">
          <tr>
            ...
            <th className="px-4 py-3 text-left">When OKORO emits it</th>
        ...
        <tbody>
          {EVENTS.map((event) => {
            ...
            return (
              <tr key={event} className="border-t border-[var(--okoro-mist)]">
                <td className="px-4 py-3 font-mono text-[var(--okoro-cyan)]">{event}</td>
                <td className="px-4 py-3 text-[var(--okoro-fog)]">
                  {meta?.when ?? 'See WEBHOOKS.md'}
                </td>
                <td className="px-4 py-3 font-mono text-xs text-[var(--okoro-fog)]">
                  {meta?.payload ?? '—'}
                </td>
              </tr>
            );
