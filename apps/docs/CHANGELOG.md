...
After pushing the platform commits (de3f7e3, 299c73f, b02348e), ran `pnpm --filter @cerniq/docs build` end-to-end for the first time. The build surfaced 6 issues that the source-only audit had missed. All fixed in this round:
...
**Build result after fixes**: `pnpm --filter @cerniq/docs build` exits 0; 35 routes generated; all 25 docs pages prerendered; `/api/docs`, `/api/search`, `/llms.txt`, `/sitemap.xml`, `/robots.txt`, `/opengraph-image`, `/twitter-image` all wired.
...
- **Fixed**: GitHub org references across all docs MDX. Was `github.com/cerniqlabs/cerniq` (55 refs); actual is `github.com/klytics/cerniq` per repo remote and `packages/sdk-ts/package.json` `repository.url`.
...
- **Fixed**: `.github/workflows/docs.yml` typecheck job ran `tsc --noEmit` before the Fumadocs MDX source was generated, so the `@/.source` import would fail. Added a `pnpm --filter @cerniq/docs exec fumadocs-mdx` step before typecheck, plus explicit OpenAPI and SDK generate steps so the CI run mirrors what `predev` does locally.
...
- **Added**: TypeDoc auto-generated `@cerniq/sdk` reference. `scripts/generate-sdk-docs.mjs` runs on every `pnpm dev`/`pnpm build`; output gitignored under `content/docs/sdk/(generated)/typescript/`.
- **Added**: Curated SDK landing pages for Python (`cerniq`), CLI (`cerniq`), relying-party verifier (`@cerniq/verifier-rp`), and MCP packages (`@cerniq/mcp-server`, `@cerniq/mcp-bridge`). All linked from the new `/docs/sdk` section in nav.
...
- **Fixed**: `<SdkVersionBadges/>` displayed `@cerniq/sdk-ts` (directory name) and `@cerniq/cli` (Go directory) instead of `@cerniq/sdk` and `cerniq (cli)` — the actual published names.
...
- **Added**: CERNIQ brand theme via Tailwind v4 + CSS variables sourced from `brand/02_design-tokens.json`.
