...
docker compose exec postgres psql -U aegis -d aegis <<'SQL'
WITH p AS (
...
- `aegis_vk_` keys can call `/verify`; `aegis_sk_` can also. Wrong key on `/agents/register` → 401, not a verify failure.
