---
title: AEGIS as backbone — multi-project adoption playbook
status: draft
last-reviewed: 2026-05-02
owner: operator (Erwin)
audience: AEGIS contributors + sister-project operators (FORGE, CerniQ, Apex, Bimba)
---

# AEGIS as backbone — multi-project adoption playbook

> **Thesis.** AEGIS is not just one of the operator's projects — it is
...
| AEGIS   | Neutral identity / policy / audit for AI agents     | (substrate — has no first-party agents)      |
...
parallel attack surfaces. **AEGIS is the consolidation play.**

Once AEGIS goes live, the four sister projects integrate as relying
parties (RPs). They issue scoped policies, bind agents to those
policies, and call `/v1/verify` before every consequential action.
AEGIS becomes the cross-cutting backbone the way Stripe became the
cross-cutting payment substrate for early SaaS.

## 2. The integration contract (per-project)

### 2.1 What AEGIS provides
...
- **Audit chain**: every decision (verify or denial) is appended to
  the AEGIS audit chain, signed, and exportable as NDJSON.
...
- **JWKS** at `/.well-known/jwks.json` for offline verification when
  AEGIS is unreachable (signed policy still holds).

### 2.2 What the RP provides

- API key (`X-AEGIS-Verify-Key`) — verify-scoped, distinct from any
...
- Per-decision verify call wrapping each consequential action — the
  RP's existing authorization layer is *not* replaced; AEGIS sits
...
if (!verdict.valid) {
  // RP UI shows verdict.denialReason — never auto-retries on AEGIS denials
  return denyToUser(verdict.denialReason);
}

// AEGIS auditEventId goes into the RP's own audit row for cross-link
await rpAudit.append({ aegisAuditEventId: verdict.auditEventId, ... });
```

## 3. Per-project integration plan

Each project adopts AEGIS in three phases. Phase 0 (decide) and
...
- **Phase 0**: file `docs/decisions/00XX-aegis-as-identity-substrate.md`
  in FORGE's repo. Identify the 6 cross-bible "decision points" (RBAC
  v11 transitions) where AEGIS gating belongs.
- **Phase 1 (shadow)**: dual-call AEGIS verify alongside existing RBAC,
  log delta to a side table. No UX change. Validates that AEGIS denials
  match the operator's policy intent before enforcing.
- **Phase 2 (enforce)**: AEGIS verify is the gate. RBAC remains
  upstream (role-based access to the *function*); AEGIS gates *agent
  authorization* within the function.
- **Risk**: FORGE has 1 088 tests (Bible matrix). Shadow phase must
  not regress any. Plan a `--filter aegis-shadow` test ring.
...
- **Phase 0**: AEGIS becomes the *trust gate* for the agent layer's
  per-tenant HTTP surface (`project_cerniq_agent_api.md`). The
  cost-breaker pattern stays CerniQ-side; AEGIS handles identity +
  policy + audit upstream.
- **Phase 1**: AEGIS verify on every agent-layer ingress, denial log
  to `agent_aegis_denials` side table. Eval harness gets a new
  AEGIS-denial-rate metric.
- **Phase 2**: enforce. Agent calls without an AEGIS-valid policy are
  rejected at the per-tenant edge.
- **Risk**: CerniQ has CAMEL cert + model cards + RLS (Supreme Bible
  Vol I+II). AEGIS denial reasons must surface in the model card audit
...
- **Phase 0**: file an Apex ADR. The 4 personas (Erwin / Risk /
  Compliance / Operator) each have AEGIS implications: Compliance
...
- **Phase 1**: shadow on reconcile workflow only. Apex feedback memory
  forbids "LLM calls on auditor artifacts" (#5) — AEGIS denials
  preserve this because AEGIS itself does no LLM inference.
- **Phase 2**: enforce on cinematic-mode auto-trade actions before
  any production trade. Paper-trade can run shadow indefinitely.
- **Risk**: Apex's swarm is rules-based, not LLM. AEGIS adds
...
- **Phase 0**: stabilize first; AEGIS planning happens after the
...
Each project is a distinct AEGIS `Principal`. A FORGE agent's policy
...
— AEGIS dashboard lists all four as siblings, no cross-Principal
data flows.

### 4.2 Single audit-chain root, per-Principal slices

Currently AEGIS uses one global audit chain (per ARCHITECTURE.md §15
...
The 9 AEGIS denial reasons (CLAUDE.md invariant 6) are *neutral* —
they do not encode FORGE / CerniQ / Apex / Bimba domain semantics.
Each project translates AEGIS denials into its own user-facing
language. Translation tables live per-project in
`<project>/docs/aegis-denial-mapping.md` (template provided in §5).

### 4.4 Webhook fan-out

Each project subscribes to its own subset of AEGIS events
...
`docs/aegis-denial-mapping.md`:

| AEGIS denialReason       | RP user message (English)                          | RP user message (Spanish, where applicable)         | RP UX surface           |
|--------------------------|----------------------------------------------------|------------------------------------------------------|--------------------------|
...
> (`status.aegislabs.io` reads `incidents.{open,history}.json` published
...
- AEGIS is **not** the agent runtime. CerniQ's agent layer, FORGE's AI
  Rail, Apex's swarm — they keep running their own runtimes. AEGIS is
  the cryptographic checkpoint.
- AEGIS is **not** the policy authoring UI. Each project authors
  policies through its own admin surface and registers them with AEGIS.
- AEGIS is **not** a multi-tenant SaaS for external customers — yet.
  The four sister projects are AEGIS's first customers. External
...
| Verify contract          | `docs/spec/AEGIS_API_SPEC.yaml` § Verify                     |
