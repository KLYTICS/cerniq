# CERNIQ — Architectural Specification (Distilled)

> Source of truth: the CERNIQ internal master suite (5 docs, KLYTICS).
...
agents. Every agent-initiated action passes through CERNIQ. The verify endpoint is the entire architecture's
...
| L2 — Policy   | `policy`   | Scoped, time-bounded permissions; instant revoke; CERNIQ-signed JWTs                |
| L3 — BATE     | `bate`     | Trust score 0–1000, signal ingestion, scorer kernel, rule-based v1 / ML v2          |
| L4 — Audit    | `audit`    | Append-only CERNIQ-signed events; SOC2/FINRA/COSSEC export                          |
...
### Policy capability token (issued by CERNIQ, signed by CERNIQ Ed25519)

Returned at policy creation. Lets relying parties verify offline that CERNIQ issued the policy. Same JWT shape; payload includes `scopes[]` and `label`.

### Audit record signature

Ed25519, CERNIQ-held (separate keypair from the policy-signing key). Public key at `GET /v1/.well-known/jwks.json` (and `GET /v1/.well-known/audit-signing-key` as a single-key convenience endpoint). Rationale: `docs/THREAT_MODEL_v2.md` § 4.2.
...
packages/sdk-ts         npm: @cerniq/sdk (TypeScript)
packages/sdk-py         PyPI: cerniq (scaffold only)
