---
title: OKORO as backbone — multi-project adoption playbook
status: draft
last-reviewed: 2026-05-02
owner: operator (Erwin)
audience: OKORO contributors + sister-project operators (FORGE, CerniQ, Apex, Bimba)
---

# OKORO as backbone — multi-project adoption playbook

> **Thesis.** OKORO is not just one of the operator's projects — it is
...
parallel attack surfaces. **OKORO is the consolidation play.**

Once OKORO goes live, the four sister projects integrate as relying
parties (RPs). They issue scoped policies, bind agents to those
policies, and call `/v1/verify` before every consequential action.
OKORO becomes the cross-cutting backbone the way Stripe became the
cross-cutting payment substrate for early SaaS.

## 2. The integration contract (per-project)

### 2.1 What OKORO provides
...
- **Audit chain**: every decision (verify or denial) is appended to
  the OKORO audit chain, signed, and exportable as NDJSON.
...
- **JWKS** at `/.well-known/jwks.json` for offline verification when
  OKORO is unreachable (signed policy still holds).

### 2.2 What the RP provides

- API key (`X-OKORO-Verify-Key`) — verify-scoped, distinct from any
...
const okoro = new Okoro({
  baseUrl: process.env.OKORO_BASE_URL,
...
if (!verdict.valid) {
  // RP UI shows verdict.denialReason — never auto-retries on OKORO denials
  return denyToUser(verdict.denialReason);
}

// OKORO auditEventId goes into the RP's own audit row for cross-link
await rpAudit.append({ okoroAuditEventId: verdict.auditEventId, ... });
```

## 3. Per-project integration plan

Each project adopts OKORO in three phases. Phase 0 (decide) and
...
- **Phase 0**: file `docs/decisions/00XX-okoro-as-identity-substrate.md`
  in FORGE's repo. Identify the 6 cross-bible "decision points" (RBAC
  v11 transitions) where OKORO gating belongs.
- **Phase 1 (shadow)**: dual-call OKORO verify alongside existing RBAC,
...
- **Phase 1**: OKORO verify on every agent-layer ingress, denial log
  to `agent_okoro_denials` side table. Eval harness gets a new
  OKORO-denial-rate metric.
- **Phase 2**: enforce. Agent calls without an OKORO-valid policy are
  rejected at the per-tenant edge.
- **Risk**: CerniQ has CAMEL cert + model cards + RLS (Supreme Bible
  Vol I+II). OKORO denial reasons must surface in the model card audit
...
- **Phase 0**: file an Apex ADR. The 4 personas (Erwin / Risk /
  Compliance / Operator) each have OKORO implications: Compliance
...
- **Phase 1**: shadow on reconcile workflow only. Apex feedback memory
  forbids "LLM calls on auditor artifacts" (#5) — OKORO denials
  preserve this because OKORO itself does no LLM inference.
- **Phase 2**: enforce on cinematic-mode auto-trade actions before
  any production trade. Paper-trade can run shadow indefinitely.
- **Risk**: Apex's swarm is rules-based, not LLM. OKORO adds
...
- **Phase 0**: stabilize first; OKORO planning happens after the
...
Each project is a distinct OKORO `Principal`. A FORGE agent's policy
...
— OKORO dashboard lists all four as siblings, no cross-Principal
data flows.

### 4.2 Single audit-chain root, per-Principal slices

Currently OKORO uses one global audit chain (per ARCHITECTURE.md §15
...
`<project>/docs/okoro-denial-mapping.md` (template provided in §5).

### 4.4 Webhook fan-out

Each project subscribes to its own subset of OKORO events
...
- OKORO is **not** the agent runtime. CerniQ's agent layer, FORGE's AI
  Rail, Apex's swarm — they keep running their own runtimes. OKORO is
  the cryptographic checkpoint.
- OKORO is **not** the policy authoring UI. Each project authors
  policies through its own admin surface and registers them with OKORO.
- OKORO is **not** a multi-tenant SaaS for external customers — yet.
  The four sister projects are OKORO's first customers. External
