...
**CERNIQ is the tool-call checkpoint for AI agents — the layer that every MCP server needs before it executes, and the trust score that every relying party checks before it complies.**

---

## SECTION 1: THE EXACT PROBLEM WE OWN

### What Exists Today vs. What CERNIQ Does

```
TODAY (without CERNIQ):
...
WITH CERNIQ:
  ...
  LLM → agent → @cerniq/mcp-bridge.wrap() → tool call → YOUR API
                       ↑
               Ed25519-signed JWT
...
### Wedge 1 — Protocol-Level Insertion (`@cerniq/mcp-bridge`)
...
const protectedServer = wrap(myMcpServer, { cerniq, actionPrefix: 'mcp.myserver.' });
```
...
- MCP server developers install `@cerniq/mcp-bridge` → they become CERNIQ relying parties
- Their users' agents must carry CERNIQ tokens to call their tools → users become CERNIQ clients
- Each integration creates two new CERNIQ touchpoints with no additional sales effort

**Adoption curve math:** If 1,000 MCP servers install our bridge over 18 months, and each server has 100 agent users, that's 100,000 potential CERNIQ users from the relying-party side alone — agents that need CERNIQ tokens to access tools they already use.
...
**Why this matters commercially:** Relying parties that require `minTrustScore: 700` are creating an implicit distribution requirement. Every developer who wants their agent to access those relying parties must use CERNIQ — not because we require it, but because the relying party does.
...
Companies that implement CERNIQ before Q4 2027 can claim "NIST-aligned agent identity infrastructure" in procurement responses, investor due diligence, and customer security questionnaires. This is not a marketing claim — it's a checkbox in the enterprise sales motion.

**Why first-mover matters here:** Security standards create sticky vendor relationships. The company that passes SOC2 with CERNIQ in their stack does not rip it out when the standards finalize — they cite their existing implementation as compliant. We need to be the reference implementation before the standards lock in.
...
Problem: they're getting fraudulent agent transactions
    ↓
CERNIQ pitch: "Add cerniq.verify(token) before your ACP handler.
              FLAGGED-band agents get rejected.
              VERIFIED agents get approved.
              Every decision is auditable."
    ↓
The merchant now requires CERNIQ tokens from all agents
    ↓
Agents that want to shop at this merchant need CERNIQ
```

Every ACP-integrated merchant is a warm CERNIQ relying-party lead. Stripe launched ACP at Stripe Sessions — every developer at that conference is now in our ICP.
...
CERNIQ uses Ed25519 everywhere:
...
This is tamper-evident by construction. Changing any past event invalidates every subsequent signature. Independent verification requires only the public key (served at `/.well-known/audit-signing-key`) — no CERNIQ API call needed.
...
2. CERNIQ verified the signature, the policy, and the spend limit
...
5. Any party can independently verify the chain without calling CERNIQ
...
Minute 3:  npm install @cerniq/sdk
...
How long would a well-funded competitor take to replicate the Phase 1 CERNIQ stack?
...
Day 1:  Developer hits "agent got blocked" problem
        ...
        → finds CERNIQ docs or community post
...
Day 21: 850/1,000 verifies used
        → CERNIQ trigger: "You're at 85% of free tier"
...
```
Week 1:  Team lead sees junior dev using CERNIQ
...
Week 2:  Email from Erwin: "I see [Company] has 4 devs on CERNIQ —
...
Month 2: Security team evaluates 3 options:
          ...
          - CERNIQ (neutral, developer-first, already in use by their team)
...
**With CERNIQ (add 4 lines):**

```javascript
import { CerniqClient } from '@cerniq/sdk';
const cerniq = new CerniqClient({ apiKey: process.env.CERNIQ_VERIFY_KEY });

app.post('/payment', async (req) => {
  const { acpToken, cerniqToken, amount, currency } = req.body;

  // CERNIQ layer — 4 lines
  const identity = await cerniq.verify(cerniqToken, {
...
_This document is the wedge argument. Everything in it is backed by code in the repo. When the code changes, update this document._

_Last updated: 2026-05-05 | Session: cowork-master-state-analysis_
