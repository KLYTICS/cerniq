# CERNIQ Public Type Contract Audit (2026 Q2)

**Scope of audit**

- `/Users/money/Desktop/CERNIQ/packages/types/src/schemas.ts` (Zod source of truth)
- `/Users/money/Desktop/CERNIQ/packages/types/src/constants.ts`
- `/Users/money/Desktop/CERNIQ/packages/types/src/errors.ts`
- `/Users/money/Desktop/CERNIQ/packages/sdk-ts/src/types.ts` (hand-written SDK types)
- `/Users/money/Desktop/CERNIQ/packages/sdk-ts/src/errors.ts` (CerniqError tree)
- `/Users/money/Desktop/CERNIQ/apps/api/src/modules/{identity,policy,verify}/*.dto.ts`
- `/Users/money/Desktop/CERNIQ/docs/spec/CERNIQ_API_SPEC.yaml`
...
- TS-inferred `SpendLimit` is `{ currency; maxPerTransaction?; maxPerDay?; maxPerMonth? }` — i.e. the empty-bounds case is _fully representable_ in the type. The `verify-rp` package can construct an empty `SpendLimit` and not see any compile error.
...
- `expiresAt: IsoDateTimeSchema` validates ISO format only — _not_ future-ness.
...
- Spec drift: `CERNIQ_API_SPEC.yaml:342` enumerates `[openai, anthropic, google, custom]` (no `huggingface`). Schema and spec disagree. SDK (`sdk-ts/src/types.ts:5`) uses **uppercase** values — three different surfaces, three different shapes.
...
```ts
const AgentRuntimeKnown = z.enum([
  'openai',
  'anthropic',
  'google',
  'huggingface',
...
### 2.9 `DenialReason` — enum, but not bound to `CerniqError`
...
- The `CerniqError` tree (`sdk-ts/src/errors.ts`) tags errors by HTTP `code` (`AUTH_REQUIRED`, `FORBIDDEN`, …) — completely orthogonal to `DenialReason`. There is no `CerniqDeniedVerify` class, and no compile-time link between "this verify came back `valid: false`" and "the denial reason is one of the 9 documented values."
...
3. Introduce a typed `CerniqDeniedVerifyError` that the SDK can throw if the caller used `client.verifyOrThrow()`.
...
| Python `SignContext.merchant_domain` | snake_case                                                         | (n/a)                                                              | (n/a)                                                  | (n/a)                                                    | **HIGH** (cross-SDK case drift; documented at `packages/sdk-py/cerniq/crypto.py:56`)                  |
| `SignContext.ttlSeconds`             | not in any schema                                                  | present (SDK-only)                                                 | not in spec                                            | n/a                                                      | MED                                                                                                   |
| `CerniqError` (SDK)                  | n/a                                                                | `interface CerniqError extends Error { status }`                   | `Error` schema                                         | n/a                                                      | MED — SDK file `types.ts` declares an interface that conflicts with the abstract class in `errors.ts` |

The two `CerniqError` declarations (`sdk-ts/src/types.ts:105` interface vs `sdk-ts/src/errors.ts:7` abstract class) is itself a real-world bug: TypeScript will pick whichever import path is used. The interface is not assignable to the abstract class. **Severity: HIGH.**
...
- **Single source of truth via `@cerniq/types`.** The Zod-first approach is the right call; runtime + compile-time validation in one place is a strong foundation.
...
**Recommendation:** introduce branded primitives across `@cerniq/types`. This is the single change with the largest ratio of bugs prevented to lines of code added.
...
export const FutureIsoDateTimeSchema = IsoDateTimeSchema.refine(
  ...
  .refine(
    (v) => new Date(v).getTime() < Date.now() + POLICY_TTL_MAX_DAYS * 86_400_000,
    'exceeds POLICY_TTL_MAX_DAYS',
  )
  .brand('FutureIsoDateTime');

// SpendLimit as discriminated union (the one-bound variants)
const SpendLimitTransaction = z.object({
  kind: z.literal('per_transaction'),
  currency: CurrencySchema,
  max: z.number().positive(),
});
const SpendLimitDay = z.object({
  kind: z.literal('per_day'),
  currency: CurrencySchema,
  max: z.number().positive(),
});
const SpendLimitMonth = z.object({
  kind: z.literal('per_month'),
  currency: CurrencySchema,
  max: z.number().positive(),
});
export const SpendLimitSchema = z
  .array(z.discriminatedUnion('kind', [SpendLimitTransaction, SpendLimitDay, SpendLimitMonth]))
  .min(1);
...
4. **Phase 4 (1 day):** delete `sdk-ts/src/types.ts` hand-written copies and re-export `@cerniq/types` directly. (The hand-written file is currently the source of all the casing-drift bugs.)
...
| 2   | Delete `sdk-ts/src/types.ts` hand-written copies; re-export `@cerniq/types`                             | CRIT     | S      | Eliminates casing-drift bugs in §2.11 in one PR.                                               |
| 3   | Resolve `CerniqError` interface-vs-class collision in SDK                                               | HIGH     | S      | Pick the abstract class; remove the interface.                                                 |
