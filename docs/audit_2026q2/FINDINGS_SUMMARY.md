# OKORO — 2026 Q2 Audit Findings (master synthesis)

**Date**: 2026-05-01
**Reviewer pool**: 6 parallel sub-agents commissioned by sid=3e2203ee under
claim `OKORO-2026-audit-and-landscape`.
...
| 8 | **CRIT — deploy** | `infra/railway/okoro-api.json` `healthcheckPath: /health` → Railway reports unhealthy on every deploy | ✅ FIXED (this audit; aligned to `/v1/health/ready`) | deploy_readiness §B4 |
...
| 18 | **MED — strategy** | NIST IR alignment is doc-bound (5 days of artefacts: trust framework, did:web resolver, public chain-head feed) — without these OKORO won't appear in the reference-implementation list | ⏳ NEXT SPRINT (`docs/DID_METHOD.md` partial; full Q3 2026 W3C registry submission) | landscape §F-3 |
| 19 | **MED — env** | `OKORO_SIGNING_PUBLIC_KEY` (wellknown) and `AUDIT_ED25519_PUBLIC_KEY_B64` (audit) are separate keys for separate purposes — not a collision but operators must set both. Document or unify. | ⏳ FOLLOWUP DOC | deploy_readiness §B2 |
...
- `pnpm-workspace.yaml` — added `scripts` and `tests` so peer's `@okoro/scripts` and the e2e harness participate in `pnpm install`.
- `infra/railway/okoro-api.json` — `healthcheckPath` aligned to `/v1/health/ready`.
- `packages/types/src/schemas.ts` — `CurrencySchema` extended to FIAT + STABLECOIN sets with `isStablecoin()` helper.
- `packages/types/src/constants.ts` — `OKORO_HEADER_IDEMPOTENCY` already shipped from a previous slot; idempotency interceptor reads it.

### New artefacts
- `packages/mcp-bridge/` (NEW package) — `@okoro/mcp-bridge` skeleton; `wrapMcpHandler()` API + `BridgeDenialError` + trust-band gate. The 2026 distribution wedge.
...
- `docs/DID_METHOD.md` — `did:okoro:<network>:<agent-id>` v0.1 method spec; W3C DID Core v1.1 conformant; targeted Q3 2026 registry submission.
...
- Memory entries (7) at `~/.claude/projects/-Users-money-Desktop-OKORO/memory/` covering user, project, holdco, references, stack, doctrine, working style.
...
- `docs/audit_2026q2/test_coverage.md` (a82667fb) — coverage matrix; 5 highest-risk gaps; e2e-from-`okoro-test.js` mapping.
- `docs/standards/0001-mcp-bridge-positioning.md` (a4814df0) — strategic rationale for shipping `@okoro/mcp-bridge`.
...
3. Operator sets Railway secrets per `infra/railway/api.service.json` env-checklist; both `OKORO_SIGNING_PUBLIC_KEY` and `AUDIT_ED25519_*_KEY_B64` must be set (the env collision was a misdiagnosis — they are deliberately different keys for different signing purposes; the canonical resolution doc is in `docs/audit_2026q2/deploy_readiness.md`).
...
8. Railway `okoro-api` deploy. Verify `/v1/health/ready` returns ok.
9. Run `pnpm db:seed` against the production DB to create the first principal + API keys.
10. End-to-end smoke test using `okoro-test.js` against the live origin.
