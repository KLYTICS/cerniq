# AEGIS — 2026 Q2 Landscape Audit

> **Purpose:** Strategic, non-code review of AEGIS's positioning against the
...
AEGIS's core thesis — **neutral, cryptographic, behavioral attestation
...
1. **ACP is now multi-payment, not Stripe-only.** Treating ACP as
   ...
   non-Stripe SPTs and stablecoin-rail SPTs. AEGIS's `currency` enum
   (`USD | EUR | GBP`) is a public-API liability.
2. **MCP auth has crystallised on OAuth 2.1 + Resource Indicators**
   (RFC 8707) since the March 2025 MCP auth spec was finalised. AEGIS's
   `signedToken` JWT is structurally compatible but the AEGIS SDK does
...
3. **NIST is going to publish guidance against AEGIS, not for it,
...
- **No trust-score primitive.** ACP v1.0 leaves "agent reputation" to
  implementers in §6 ("Out of scope"). This is the gap AEGIS targets.
...
### Where AEGIS plugs in

ACP request → merchant receives `{ spt, agent_id, signed_intent }` →
merchant calls `POST /v1/verify` on AEGIS with the agent's signed
JWT → AEGIS returns `{ valid, trustScore, scopesGranted, denialReason }`.

Merchant decision matrix becomes:

| SPT valid? | AEGIS valid + score ≥ threshold? | Decision |
|------------|----------------------------------|----------|
| ✓          | ✓                                | Approve  |
| ✓          | ✗ (low score / scope miss)       | Step-up auth or decline |
| ✗          | —                                | Decline (payment leg failed) |
| ✓          | (AEGIS down)                     | Implementer choice — usually fail-open w/ logging |

### Misalignments to fix

| # | Issue | AEGIS impact | Backlog ID |
|---|-------|--------------|------------|
...
| ACP-2 | No `spt_hash` field in `VerifyRequest` | Cannot correlate AEGIS audit event to a Stripe charge for SOC2 evidence | M-102 (new) |
| ACP-3 | No published "ACP profile" mapping AEGIS scopes to ACP categories | Merchants writing integration code have to invent the mapping | M-103 (new) |
| ACP-4 | No example/reference repo demonstrating ACP+AEGIS dual-verification | Distribution friction at the worst possible moment | M-104 (new) |

**Verdict: PARTIAL ALIGNMENT.** AEGIS is technically additive to ACP, but
...
### How AEGIS differentiates

| Vector | Auth0 for AI Agents | AEGIS |
|--------|---------------------|-------|
...
verifier. The "Switzerland" framing is defensible *if* AEGIS is also
...
| # | Issue | Backlog ID |
|---|-------|------------|
| AUTH0-1 | AEGIS does not currently expose a CIBA-style async-approval flow for high-value transactions | M-105 (new) |
| AUTH0-2 | No FGA-equivalent for relationship-based access control (only category + domain allowlists) | M-106 (P3+) |
| AUTH0-3 | No public comparison/migration doc ("Auth0 → AEGIS" or "AEGIS + Auth0") | M-107 (docs) |

**Verdict: AEGIS is NOT a direct competitor — different segment,
...
- **No identity-of-the-agent primitive.** The MCP auth spec answers
  ...
  (compliance) — it's the *most direct AEGIS adjacency in the entire
  ecosystem*.

### The `@aegis/mcp-bridge` opportunity
...
MCP client (Claude Desktop)
  ...
  │
  └── X-AEGIS-Agent-Token header (agent's AEGIS-signed JWT)
         │
         ▼
  MCP server (any) wrapped with @aegis/mcp-bridge middleware
         │
         ├── 1. validates OAuth user token (existing MCP flow)
         ├── 2. calls /v1/verify on AEGIS with agent JWT
         ├── 3. enforces denial-precedence rules
         └── 4. emits AEGIS audit event for the tool call
```
...
| # | Issue | Backlog ID |
|---|-------|------------|
| MCP-1 | No `@aegis/mcp-bridge` package | **M-110 (new — Phase 1 wedge)** |
| MCP-2 | AEGIS JWTs do not declare an `aud` (audience) claim conformant with RFC 8707 | M-111 |
| MCP-3 | No DPoP support on AEGIS-issued JWTs | M-112 (see § 6) |
| MCP-4 | No example MCP server using AEGIS for agent identity | M-113 (docs/example) |
...
### How AEGIS maps (honest grading)

| Theme | AEGIS today | Grade | Gap |
|-------|-------------|-------|-----|
...
| Auditability + non-repudiation | Append-only `AuditEvent`, hash-chained, AEGIS-signed | A | Need public chain-head publication for non-repudiation against AEGIS itself |
...
recourse a relying party has. AEGIS does not yet have one. This is a
4–6 page document, not engineering work, but without it AEGIS will not
...
## 5. W3C DID v1.1 — `did:aegis` vs `did:web`
...
|----------|------|------|----------------|
| Publish `did:aegis` method spec | "Real" DID method, signals seriousness | Adds governance + maintenance burden, requires registry submission to W3C CCG | Phase 2+ |
| Use `did:web:aegislabs.io:agents:<id>` | Zero protocol invention; resolution is plain HTTPS GET; works today | "Just" DNS-anchored — no decentralisation theatre | **YES — Phase 1** |
| Use `did:key:z<base58btc-pubkey>` | Pure key-based, no resolution needed | Loses the principal-binding, label, runtime metadata | Use as the *agent's* fallback DID; AEGIS DID becomes the *issuer* DID |

### Recommended implementation

- AEGIS issues `did:web:aegislabs.io:agents:<agentId>` for every agent
  on registration.
- `GET https://aegislabs.io/agents/<agentId>/did.json` returns a W3C DID
  Document with `verificationMethod` (the Ed25519 key), `service` (the
  AEGIS verify endpoint), and a `controller` (the principal's DID).
- The AEGIS *issuer* identity is `did:web:aegislabs.io` — published
...
| DID-2 | No `did:web:aegislabs.io` issuer DID document | M-131 |
...
If AEGIS-issued policy tokens (the `signedToken` JWTs from
...
This collapses adoption friction from "learn AEGIS's HTTP shape" to
"point your existing OAuth resource-server middleware at AEGIS."

### Current state of AEGIS tokens

- Policy tokens are JWTs signed by AEGIS Ed25519 (via `jose`).
...
**Verdict: PARTIAL.** AEGIS chose the right primitive (Ed25519 JWT) but
hasn't dressed it in OAuth 2.1 clothes. DPoP is the single highest-
leverage standards alignment available — it makes AEGIS instantly
...
### Where AEGIS is positioned

- AEGIS itself is **not** an Article 50 obligated party (we are not the
  deployer; we are infrastructure to a deployer).
- BUT — AEGIS audit logs are *the* compliance evidence a deployer
...
| EU-2 | No "agent disclosure header" (e.g. `X-AEGIS-Agent-Disclosure: yes`) that downstream services can surface to end users | M-151 |
| EU-3 | Audit retention configurable by tier but no policy doc covering "EU customer requires 3-year retention" SLA | M-152 |

**Verdict: AEGIS-READY (with docs work).** No engineering changes
...
### How they differ from AEGIS

| Dimension | Cloudflare BotID / Privacy Pass | AEGIS |
|-----------|----------------------------------|-------|
...
Cloudflare-native flow could attach AEGIS verification *inside* a BotID
pipeline (BotID says "yes, this is a real agent and not a script-kiddie
crawler"; AEGIS says "yes, and here's *which* agent and *whose*").
...
| CF-2 | No demo of BotID + AEGIS dual-flow | M-160 (low priority) |
...
| PQ-2 | No JWKS-per-agent endpoint (only AEGIS-issuer JWKS) | M-171 |
...
| Auth0 differentiation | AEGIS-READY (positioning) | Low |
...
| EU AI Act docs | AEGIS-READY (docs only) | Medium |
| Cloudflare convergence | AEGIS-READY (Phase 3) | Low |
...
|----------|------|-------|----------------|-----------|
| 1 | **M-110** | `@aegis/mcp-bridge` package (Node + Python) | 1–2 weeks | Distribution wedge with the largest 2026 agent ecosystem. Detailed rationale in `docs/standards/0001-mcp-bridge-positioning.md`. |
...
| 3 | **M-130** | Per-agent `did:web:aegislabs.io:agents:<id>` DID document endpoint | 1–2 days | Single highest-signal NIST-alignment artefact. |
...
| 7 | **M-120** | Publish "AEGIS Trust Framework v1" document (governance, audit, recourse) | 2–3 days (doc only) | Required artefact for NIST reference-implementation listing. |
| 8 | **M-104** | Reference repo: ACP + AEGIS dual-verification example merchant | 2–3 days | Sales/marketing artefact that doubles as integration test. |
...
## Things AEGIS should explicitly NOT do (negative scope)

For the audit record:

1. **Do not invent a `did:aegis` method.** `did:web` is sufficient and
   has zero governance burden. Re-evaluate at Year 3 when the volume of
   AEGIS DIDs justifies the W3C CCG submission cost.
2. **Do not become an MCP server.** AEGIS *wraps* MCP servers via a
...
3. **Do not implement FIDO2/WebAuthn for agents.** WebAuthn presumes a
   human authenticator. The right primitive for agents is what AEGIS
...
- [ ] Confirm specific stablecoin merchant acceptance (Best Buy,
      Target, Delta — these are the AEGIS deck's example merchants).
