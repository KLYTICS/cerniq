---
title: CERNIQ — Brand & Design Foundation
audience: every Claude session, designer, contractor, and AI tool that produces an CERNIQ surface
...
- Developer-facing: _"Verified agent identity and signed audit in <80ms.
  Drop-in SDK. Zero key custody."_
- Security-buyer-facing: _"Cryptographic gate for agent actions. Public-key
  only. Append-only audit. SOC2-bound."_
- Investor-facing: _"The Switzerland of agent identity — protocol-, vendor-,
  and model-neutral. Verification choke point for every agent transaction."_
...
2. **Non-custodial.** CERNIQ holds public keys only. Private keys live with
   the agent. A breach of CERNIQ does not compromise an agent.
...
### 1.3 What CERNIQ is **not** (anti-positioning)

Surfaces must never imply any of the following — they are competitor
ground or wrong-shape framing:

- _"Single source of agent truth"_ — we are a verifier, not an authority
- _"Manage your agent's keys"_ — we never hold private keys, ever
- _"Replace Auth0/Okta"_ — we plug into them; human IAM is their lane
- _"AI security"_ in the prompt-injection / red-team sense — that's the
  agent runtime's problem, not ours
- _"Blockchain"_ anything — Ed25519 + signed audit ≠ blockchain, do not
  conflate
...
- **Cryptographically grounded.** When we describe a guarantee, we name
  ...
  "tamper-proof logs."
- **Confident, not boastful.** State what CERNIQ does. Skip "industry-
...
- **Builder-respectful.** The reader is a senior engineer or security
  ...
  CERNIQ-specific shape (denial precedence, BATE, principal binding).
...
| Docs quickstart         | Direct, imperative, terse           | "Install the CLI. Authenticate. Run `cerniq doctor`. Verify your first call."                                                         |
| Pitch deck              | Strategic, market-aware             | "ACP solved payments. CERNIQ solves the verification gap Stripe explicitly left to implementers."                                     |
| Error message           | Factual, action-pointing            | "`SCOPE_NOT_GRANTED` — policy `pol_01H...` does not include scope `payments.transfer`. Add it via dashboard or `cerniq policy edit`." |

### 2.3 Word choices

- **Use:** _agent_, _principal_, _relying party_, _verify_, _attest_, _sign_,
  _revoke_, _scope_, _policy_, _audit chain_, _trust score_.
- **Avoid:** _passport_ (we are not), _identity provider_ (we are not),
  _AI safety_ (different problem), _blockchain_, _web3_, _bulletproof_,
  _military-grade_, _unhackable_, _cutting-edge_.
- **House style:** "CERNIQ" is always all-caps in body text. SDK package is
  lowercase `@cerniq/sdk`. Endpoints are mono: `POST /v1/verify`.
...
3. **Vercel** (small dose) — the typographic discipline and command-palette
   ...
   CERNIQ reads as more enterprise than indie-hacker.

### 3.2 Things we deliberately _don't_ take from references
...
The brand is 80% neutral. Color carries semantic weight precisely _because_
it's rare.
...
### 4.2 Brand accent — "Cerniq Blue"

```
cerniq-50  #ECFEFF   tint, subtle highlight backgrounds
cerniq-100 #CFFAFE   hover tint
cerniq-300 #67E8F9   accents in dark mode
cerniq-500 #06B6D4   primary brand — buttons, links, focus rings
cerniq-600 #0891B2   primary hover
cerniq-700 #0E7490   primary pressed
cerniq-900 #164E63   deep brand (illustrations)
```

Cerniq Blue is **only** used for: primary CTA, the CERNIQ wordmark,
...
info:    cerniq-500            — same as brand; use sparingly
```
...
shift one step (e.g. cerniq-500 → cerniq-300 on dark surfaces) for AA
contrast.
...
- Custom SVGs for: CERNIQ mark, denial-precedence ladder, BATE score gauge
...
Until that ships, surfaces use the wordmark "CERNIQ" set in Inter 700,
tracking-tight, with `cerniq-500` color, no logomark.
...
- Audit log new event: 320ms fade-in row with 1px cerniq-500 left border
...
- **Abstract geometry.** Sparse, monochromatic, used only as a hero
  texture. Examples: a faint grid, a faded radial gradient, the CERNIQ
  mark at 4% opacity.
...
- Fill: slate-100 (light) / slate-900 (dark) for boxes; cerniq-50 /
  cerniq-900 for highlighted boxes
...
Code samples are a first-class visual element on CERNIQ surfaces. They
must always:
...
- Highlight the CERNIQ-specific lines with a left border in cerniq-500
...
in cerniq-700, and CERNIQ-specific calls (`cerniq.verify`, etc.) in cerniq-500
**bold-italic**. The bold-italic on CERNIQ calls is the brand's typographic
signature.
...
_intent_ every prompt downstream must respect.

| Component              | Intent                                                                                                                                   |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| Primary button         | Solid `cerniq-500`, white text, 36px tall (default), rounded-md, no gradient, no shadow at rest, shadow-sm on hover                      |
| Secondary button       | 1px slate-200 border, slate-700 text, white background, same dimensions                                                                  |
| Tertiary / link button | cerniq-600 text, no background, underline on hover only                                                                                  |
| Input                  | 1px slate-200 border, rounded-sm, 36px tall, slate-900 text, slate-400 placeholder, focus ring 2px cerniq-500/40                         |
...
- Every interactive element has a visible focus state — the 2px
  cerniq-500/40 ring is the canonical one
...
(Cloudflare-Auth0), §4 color system (cerniq-500 = #06B6D4 single brand
accent), §5 typography (Inter + JetBrains Mono only), §10 code-sample
treatment (mandatory bold-italic on CERNIQ-specific calls), §11
