---
title: OKORO — Developer Dashboard Design Prompts
...
```
Build a dashboard home page for OKORO — a verification + attestation
...
- 240px left sidebar with icon + label nav: Home, Agents, Policies,
  ...
  3px okoro-500 left border + slate-800 bg.
...
3. Two-column grid below:
   - Left col (2/3 width): "Verify volume" line chart, slate-800 bg,
     ...
     (okoro-500), denied verifies (rose-500). Tooltip on hover with
     mono numbers.
...
```
Build the /agents page for the OKORO dashboard. Same project + brand
foundation.
...
- A 720px-tall area chart of BATE score over the last 30 days,
  Recharts, okoro-500 line. Gridlines at 300 and 700.
...
- "Live" toggle — when on, new events stream in at top with a 320ms
  fade-in row + 1px okoro-500 left-border pulse
...
- Submit disabled until valid; required fields marked with a small
  text-xs okoro-500 asterisk after the label.
...
2. Authenticate — shows `okoro login --device-code`; polls for
   completion.
3. Run okoro doctor — shows the command; the right-side terminal
   streams the diagnostic output.
4. Scaffold an integration — radio picker (fintech-payments / ai-platform-
   tool-call / saas-seat-provisioning), then `okoro init --industry ...`
   one-liner.
...
- Status: pending (slate-400 circle), in-progress (okoro-500 with
...
```
Design the OKORO developer dashboard in Figma. Audience: developers
...
Token foundation: the OKORO Brand Foundation v1 in
...
```
PROJECT: OKORO Developer Dashboard v1 (apps/dashboard)
...
2. Add tailwind.config.ts at apps/dashboard/ with the full token set
   from §13 of the foundation doc. The okoro ramp must be exposed as
   `colors.okoro.*` so utilities like `bg-okoro-500` work.
...
5. Initialize shadcn/ui at apps/dashboard with the new-york style.
   Override primary to okoro-500 in the components.json.
...
7. Verify by running `pnpm --filter @okoro/dashboard build`. Lint must
...
2. Data fetching: use Server Components. Add loaders in
   apps/dashboard/app/_data/ that call the OKORO API via the SDK
   (@okoro/sdk). Key invariant: every loader takes principalId from
...
3. The "Verify chain integrity" button runs the verification CLIENT-
   ...
   pass/fail. This proves to the user that OKORO does not need to be
...
- Failure of step 3 (okoro doctor) routes the user to a "Diagnose"
