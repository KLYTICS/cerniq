---
title: CERNIQ — Brand Identity Design Prompts
...
1. **AI image-gen tools** (Midjourney, Ideogram, DALL-E 3, Recraft) —
   useful for _exploration_ of mark concepts, never for shipping
   production assets.
2. **Identity designer (human)** — the only acceptable shipping path
   ...
   CERNIQ cannot afford that on a security product.
3. **In-repo SVG generation** — for the wordmark variants, favicon,
   and the supporting visual system (the CERNIQ mark as ASCII-style
...
5. Avoid every visual cliché CERNIQ lives next to: shield, lock,
...
- The **wordmark "CERNIQ"** is set in Inter 700, tracking-tight,
  `cerniq-500` color. This is the placeholder.
- There is **no logomark yet**. Surfaces use the wordmark only.
- The **favicon** is a 32×32 SVG of the wordmark "A" cropped to a
  rounded square in `cerniq-500`. Placeholder.
...
called CERNIQ. The mark expresses cryptographic neutrality — a balanced
...
```
A flat vector logomark for CERNIQ, a neutral verification layer for
...
_non-logo_ visual elements of the identity: the diagrams, the layer
stack motif, the data-flow illustrations.

### B.1 The 4-layer stack motif (canonical brand visual)

```
Design a vector illustration of the CERNIQ 4-layer stack as a brand
...
- A single vertical cerniq-500 line (2px) runs through all 4 slabs at
  x=120px from left, with a 6px filled cerniq-500 dot at the midpoint
...
Deliverables:
- SVG (light variant)
- SVG (dark variant — invert neutrals, keep cerniq-500)
...
- A simplified favicon-sized version (32×32) showing only the 4
  slabs in slate-700 with the cerniq-500 verify line
```

### B.2 Request lifecycle diagram

```
Design a horizontal swim-lane diagram of an CERNIQ verify request.
...
- 5 horizontal swim lanes, each 64px tall, labeled left:
  Agent, CERNIQ Edge, CERNIQ Origin, Relying Party, Audit Sink
...
- 8 sequential steps shown as arrows + small annotations across the
  ...
  2. → CERNIQ Edge: receive
  3. CERNIQ Edge: validate signature, fetch policy from cache
  4. (cache miss) → CERNIQ Origin: fetch policy + BATE
  5. → CERNIQ Edge: evaluate denial precedence
...
- Row backgrounds escalate in subtle red intensity: top (most-
  ...
  alarming
- A vertical cerniq-500 line (2px) on the left of the ladder labeled
...
```
PROJECT: CERNIQ Brand Identity v1
...
2. Wordmark "CERNIQ" — custom-drawn, not pulled from a typeface;
...
4. Color application — using the Brand Foundation §4 palette only.
   Cerniq-500 is the brand's only accent; mark must work without it
...
- Sub-brand identities (e.g. CERNIQ for Fintech) — not designed yet
...
- All-caps "CERNIQ" is preferred — reads more institutional.
...
2. Add Wordmark.tsx — renders "CERNIQ" in Inter 700 tracking-tight
   cerniq-500. Props: size ('sm'|'md'|'lg'), monochrome (boolean),
...
3. Add Mark.tsx — renders the placeholder logomark, which until the
   ...
   horizontal slabs with the cerniq-500 vertical verify line). When the
...
2. The template:
   ...
   - Top-left: CERNIQ wordmark in cerniq-500 (40px tall)
   ...
   - Below title: page description in Inter 400 24px slate-600, max
     2 lines
   - Bottom row: a small 4-layer-stack mark on the left, "cerniq.dev"
...
each CERNIQ public repo.

Tasks:
1. Add scripts/generate-github-social.ts at the repo root.
2. The card:
   - slate-950 bg
   - Center-left: CERNIQ wordmark in cerniq-300 (lighter ramp for dark
     surface), 56px tall
   ...
   - Center-right: the 4-layer stack mark in slate-700 with cerniq-300
     verify line, 240px tall
   - Bottom-left: "github.com/cerniq" in Inter mono 16px slate-500
