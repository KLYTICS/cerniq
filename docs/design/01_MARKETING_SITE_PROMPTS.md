---
title: OKORO — Marketing Site Design Prompts
...
sees. It must in <8 seconds communicate: *what OKORO is*, *why it isn't a
...
```
Build a marketing hero section for OKORO — a neutral verification, policy,
...
- A subtle radial gradient anchored top-right at okoro-100 → transparent

Copy:
- Eyebrow (text-xs all-caps tracking-wider okoro-700): "Verification layer"
...
- Lede (text-lg slate-600, max-width 540px):
  "OKORO is the neutral cryptographic gate for AI agents.
  Public-key registry, signed audit chain, policy-bound scopes —
  drop-in SDK for any agent runtime."
- Two CTAs:
  1) Primary: "Read the quickstart" — okoro-500 bg, white text, ArrowRight icon
...
- Code (JetBrains Mono 14px line-height 24px):
    ...
    const okoro = new Okoro({ apiKey: process.env.OKORO_KEY });

    // Register an agent (one-time)
    const agent = await okoro.agents.create({
      runtime: "anthropic",
    ...
    const result = await okoro.verify(signed);

    if (!result.valid) {
      throw new Error(result.reason); // e.g. SCOPE_NOT_GRANTED
    }

- Highlight lines 9–12 with a 3px okoro-500 left border
- The tokens "okoro.verify" and "agent.sign" must render in okoro-500
  bold-italic
...
Subhead (text-lg slate-600 max-width 720px): "Each OKORO request runs
...
- Left side (96px wide): a layer number badge (slate-100 bg, slate-700
  ...
  ShieldCheck, Activity, FileCheck respectively) at 24px okoro-500
...
4. Audit & Compliance Rail — "Append-only, hash-chained, OKORO-signed.
...
Block 1 — "Non-custodial by design."
  Body: "OKORO issues an Ed25519 keypair. The private key is generated
  client-side and stored on the agent host. We register only the public
  key. A breach of OKORO does not compromise an agent."
  Footer link: "Read ADR-0002 →" (okoro-600, text-sm)
...
okoro-500, top-left of each card.
...
Tiers (anchor card 3 as the recommended one — slight okoro-500
...
- Tier name (text-lg font-medium), price (text-3xl font-bold slate-900,
  ...
  icon at 16px okoro-500.
- Recommended card has okoro-500 1px border, okoro-50/40 bg tint, and a
  pill at top center: "Recommended" (text-xs all-caps okoro-700 on okoro-100).
...
- Below the columns, a single horizontal divider (1px slate-200), then a
  bottom row: OKORO wordmark left, copyright + status pill right.

Status pill: "All systems operational" with a 6px filled emerald-500 dot,
text-xs slate-500. Links to status.okoro.dev (placeholder).
...
- Security: Threat model, Denial precedence, Audit chain, Compliance,
  Bug bounty, security@okoro.dev
- Company: About, Blog, Customers, Careers, Legal, Contact

Brand block (left, span 1 col):
- Wordmark "OKORO" (Inter 700 tracking-tight okoro-500 text-xl)
...
```
Design a marketing landing page for OKORO, an AI-agent verification &
...
2. Hero section, 720px tall, slate-50 bg with a faint 24px grid pattern
   ...
   row; right = a code sample card with copy button. See OKORO Brand
...
4. "How it works" section — 4 horizontal stacked cards representing the
   OKORO layer stack (Identity, Policy, BATE, Audit). Each card has a
...
5. Live-integration section — a wider code sample (full integration in
   ...
   line triggers in the OKORO pipeline.
...
8. Pricing section — 4 tiers, recommended tier highlighted with okoro-500
   border.
9. Final CTA band — slate-900 bg, white text, single h2 "Drop OKORO into
   your stack in 10 minutes." with two CTAs.
10. Footer — 5-column links, brand block, status pill.

Use the OKORO Brand Foundation tokens (color §4, type §5, spacing §6,
...
Reads as "this is a control we'd actually evaluate." Use OKORO Brand
...
6. Compliance posture — a table mapping OKORO controls to SOC2 CC#,
   ISO 27001 A#, NIST CSF, EU AI Act. Status column: In place /
   In progress / Roadmap. Be honest — pre-launch, most cells are
   "In progress."
7. Reporting + bug bounty — small section with security@okoro.dev,
...
2. The request lifecycle diagram — a horizontal swim-lane diagram
   showing 5 actors (Agent, OKORO Edge, OKORO Origin, Relying Party,
...
```
PROJECT: OKORO Marketing Site v1 (launch)
CONTEXT: OKORO is the neutral verification, policy, and behavioral
...
1. OKORO is non-custodial. We hold public keys only.
...
HARD CONSTRAINTS:
- Okoro-500 (#06B6D4) is the only brand accent. No purple, no orange,
...
These prompts assume the agent has the OKORO repo open and can run pnpm,
...
App Router, Tailwind CSS, shadcn/ui, and the OKORO design tokens.

Read first:
- /Users/money/Desktop/OKORO/CLAUDE.md
- /Users/money/Desktop/OKORO/docs/design/00_BRAND_FOUNDATION.md
- /Users/money/Desktop/OKORO/apps/dashboard/package.json (mirror Next/React versions)

Tasks:
1. Create apps/marketing/ as a pnpm workspace package, name "@okoro/marketing".
...
3. Install tailwindcss + @tailwindcss/typography, set up tailwind.config.ts
   with the full OKORO color system from §13 of the foundation doc
   (slate ramp + okoro ramp + semantic) and font stacks from §5.
4. Install shadcn/ui with the new-york style, but override its default
   `primary` color to okoro-500 and its `radius` to 6px. Initialize only
...
5. Add a /lib/seo.ts helper that returns a complete OpenGraph + Twitter
   ...
   "OKORO — The verification layer for AI agents" as the default suffix.
...
9. Verify by running `pnpm --filter @okoro/marketing dev` and confirming
...
2. The CodeSample component should be reusable; export it from
   ...
   code, highlightLines }.
3. The bold-italic on OKORO-specific calls is implemented via a custom
   `okoroHighlight` Prism plugin or a regex-based wrapper that wraps
   `okoro.\w+` and `agent.sign` in <span class="okoro-call"> with the
...
- The okoro-500 calls render with the .okoro-call class.
...
- docs/OKORO_AS_BACKBONE.md § 2.3 (the canonical 20-line consumption
