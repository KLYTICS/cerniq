# `did:aegis` — DID Method Specification (draft v0.1)

> A W3C-conformant Decentralized Identifier method for AEGIS-registered
...
1. **Standards distribution wedge.** Verifiers built around W3C DIDs
   ...
   traceability, healthcare credential systems) can consume `did:aegis`
...
2. **DID compatibility hedge.** If the W3C DID stack wins the standards
   ...
   OAuth/DPoP wins, we still have our REST API. Doctrine: AEGIS is
   neutral and protocol-agnostic.

**Non-goals**: replacing the AEGIS REST API; obsoleting the AEGIS SDK;
selling DIDs as an end product.

## 2. Method name

```
did:aegis:<network>:<agent-id>
```

- `<network>` — `mainnet` (production), `sandbox` (sandbox env), or a
  region prefix in Phase 2.5 (`eu-mainnet`).
- `<agent-id>` — the AEGIS agent identifier, e.g.
  `agt_01HZ9YZXM4QT3B7P8WKJD6R5V`.

### Example

```
did:aegis:mainnet:agt_01HZ9YZXM4QT3B7P8WKJD6R5V
```
...
Creating an AEGIS agent via `POST /v1/agents/register` implicitly creates
...
Resolution endpoint:
```
GET https://api.aegislabs.io/.well-known/did/{did-encoded}
```

Or via the [universal resolver](https://dev.uniresolver.io/) once we
register the method (target Q3 2026):
```
https://dev.uniresolver.io/1.0/identifiers/did:aegis:mainnet:agt_xyz
```
...
```jsonc
{
  ...
  "id": "did:aegis:mainnet:agt_01HZ9YZXM4QT3B7P8WKJD6R5V",
  "controller": "did:aegis:mainnet:principal:p_abc123",
  "verificationMethod": [{
    "id": "did:aegis:mainnet:agt_01HZ9YZXM4QT3B7P8WKJD6R5V#key-1",
    "type": "Ed25519VerificationKey2020",
    "controller": "did:aegis:mainnet:agt_01HZ9YZXM4QT3B7P8WKJD6R5V",
    "publicKeyMultibase": "z6MkpTHR8VNsBxYAAWHut2Geadd9jSrYE6BnUQUpCPVZjy2"
  }],
  "authentication": [
    "did:aegis:mainnet:agt_01HZ9YZXM4QT3B7P8WKJD6R5V#key-1"
  ],
  "assertionMethod": [
    "did:aegis:mainnet:agt_01HZ9YZXM4QT3B7P8WKJD6R5V#key-1"
  ],
  "service": [{
    "id": "did:aegis:mainnet:agt_xyz#aegis-status",
    "type": "AegisAgentStatus",
    "serviceEndpoint": "https://api.aegislabs.io/v1/agents/agt_xyz/status"
  }, {
    "id": "did:aegis:mainnet:agt_xyz#aegis-verify",
    "type": "AegisVerify",
    "serviceEndpoint": "https://api.aegislabs.io/v1/verify"
  }]
}
```

The DID Document is **not signed** by AEGIS — the integrity comes from
TLS to a verifiable AEGIS endpoint. Verifiers MAY additionally use the
status service endpoint to fetch the current trust band.

### UPDATE

DID Documents are derived from AEGIS database state. Updates happen
...
- Trust score changes are NOT reflected in the DID Document itself
  (avoid resolution churn) — fetch via the `AegisAgentStatus` service.
...
A relying party verifying a presentation that includes a `did:aegis`
identity SHOULD:

1. Resolve the DID Document.
2. Confirm the public key matches the one used to sign the presentation
   (Ed25519 raw verification — same operation as `aegis.verify()` does
   internally).
3. Optionally call the `AegisVerify` service endpoint to confirm
   trust-score, scope, spend limits, and revocation state — this is the
   AEGIS value-add over a bare DID resolution.
...
- DIDs leak the AEGIS network (`mainnet` vs `sandbox`).
- The `controller` field reveals the principal-level DID.
- Verifiers should be aware that calling `AegisVerify` with a DID is
  observable to AEGIS — operators wanting unobservable verification
...
1. **Should `did:aegis` be cryptographic-self-asserting (like `did:key`)
   or web-anchored (like `did:web`)?** Currently web-anchored — implies
   AEGIS uptime is part of the trust model. Counter-argument: making it
   self-asserting decouples AEGIS uptime from DID validity, which
...
3. **`did:aegis:eu-mainnet`** vs `did:aegis:mainnet` with a
...
- AEGIS internal: `docs/spec/05_STANDARDS_ROADMAP.md`, `docs/SECURITY.md`
