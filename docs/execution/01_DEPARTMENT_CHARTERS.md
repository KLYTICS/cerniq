---
title: CERNIQ — Department Charters
...
> Every department at CERNIQ is small or notional today. The charter is
...
- Weekly: round summary in `SESSION_HANDOFF.md` (a "round" is the
  CERNIQ sprint unit; see Round 23 entry for the canonical shape).
...
**Mission.** Make every CERNIQ surface — marketing, dashboard, docs,
...
**Mission.** Ensure every claim CERNIQ makes about cryptographic
...
**Mission.** Map CERNIQ controls to the standards customers and regulators
...
- Mapping CERNIQ controls to: SOC2 Trust Services Criteria, ISO 27001
...
- Disclosure controls — what claims CERNIQ makes externally and how
  they are sourced.
...
**Mission.** Ensure CERNIQ knows its numbers, spends them well, and
...
**Mission.** Make sure CERNIQ can do business everywhere it sells, owns
...
- Open-source license review (the SDK is MIT — confirm; dependencies
  audited).
- Trademark filings (CERNIQ, the logomark when delivered).
...
- Open-source dependencies have a license that is compatible with
  CERNIQ distribution (the SDK is MIT; copyleft is forbidden in
...
**Mission.** Position CERNIQ as a contributor to and shaper of the
emerging open standards for agent identity, verification, and
attestation. Make sure CERNIQ does not become locked out of standards
...
- Open-source contributions never reveal proprietary CERNIQ
...
- `docs/standards/*.md`.
- `docs/CERNIQ_AS_BACKBONE.md` (co-owned with Product).
...
This matrix is the social graph of CERNIQ execution. Print it. Tape it
