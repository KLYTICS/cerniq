# CERNIQ — Master Engineering & GTM State Document

## Phase 1 GA Closure | May 2026

### Classification: INTERNAL — Engineering Lead + All Terminals

> **How to read this document.** It is the single source of truth for what CERNIQ _is right now_ — code-proven, not aspirational. Every claim is backed by a specific file or endpoint. Use PART VII to find your terminal assignment. Use PART VIII to find your next task.
...
- Idempotency: Redis SETNX on `cerniq:stripe:event:{eventId}`, 7-day TTL
...
- Redis counter: `cerniq:usage:{principalId}:{YYYY-MM}`
- DB backfill on Redis miss: `AuditEvent.count WHERE principalId + timestamp >= startOfMonth`
- Plan tier cached: `cerniq:plan:{principalId}`, 5-minute TTL
...
10. If band changed: fire `cerniq.agent.trust_score_changed` webhook
...
**Delivery:** BullMQ worker, 5 retries, exponential backoff (1s→2s→4s→8s→16s), HMAC-SHA256 signature on payload with `X-Cerniq-Signature: t={ts},v1={hmac}`.

**Events fired today:**

- `cerniq.agent.trust_score_changed` (on band crossing)
...
Returns JWKS-format JSON with active signing public key. ETag caching, CORS-open. Relying parties poll this to verify audit chain signatures without CERNIQ account. This is the trust anchor for the entire audit chain — any party can independently verify signatures without calling CERNIQ's API.
...
`PrincipalOnboarding` row tracks 7 milestones: hasFirstAgent, hasFirstPolicy, hasFirstVerify, hasKmsConfigured, hasMcpServerRegistered, + 2 more. Milestones are one-way ratchets (never unset). Drives the dashboard wizard and `cerniq doctor` CLI command.
...
#### Module: `mcp` — CERNIQ as an MCP Server

CERNIQ exposes its own MCP server so agents can interact with CERNIQ programmatically (register, verify, query trust scores) through the MCP tool-call protocol.

---

### 2.2 Packages

#### `packages/sdk-ts` — `@cerniq/sdk` (TypeScript)

**Files:** `agent.ts`, `browser.ts`, `crypto.ts`, `http.ts`, `policy.ts`, `types.ts`, `index.ts`

The developer-facing SDK. Key surface:

- `CerniqAgent.sign(payload)` → Ed25519-signed JWT, client-side only (private key never leaves)
- `CerniqClient.verify(token, options)` → calls `/v1/verify`, returns `VerifyResult`
- Works in Node.js and browser (`browser.ts` uses WebCrypto API)

#### `packages/mcp-bridge` — `@cerniq/mcp-bridge`

**THE DISTRIBUTION WEDGE.** `wrap(mcpServer, { cerniq, actionPrefix })` inserts CERNIQ verification before every tool call. Any MCP server + 3 lines = CERNIQ-protected.
...
#### `packages/cli` — `cerniq` CLI

`cerniq doctor`, `cerniq agent register`, `cerniq verify`, `cerniq kms rotate`. Developer tooling for local development and production operations.

#### `packages/audit-verifier` — Standalone Chain Verifier

Any party can run this to independently verify an CERNIQ audit chain without calling our API. This is a core trust property — verifiability is not contingent on CERNIQ's availability.

#### `packages/verifier-rp` — Relying Party Verifier

Lightweight package relying parties embed to verify CERNIQ tokens without an CERNIQ account (for the edge case where latency requirements preclude a round-trip to the CERNIQ API).
...
CERNIQ's distribution wedge is `@cerniq/mcp-bridge`. Here is why it is structurally irreplaceable:

```typescript
// BEFORE CERNIQ: tool call reaches your server with zero identity context
...
// AFTER CERNIQ: 3 lines, verified identity on every tool call
import { wrap } from '@cerniq/mcp-bridge';
const protected = wrap(server, { cerniq, actionPrefix: 'mcp.myserver.' });
...
3. By sitting at the MCP layer, CERNIQ intercepts 100% of agent-initiated actions, not just some.
4. The alternative (wrapping at the HTTP/API layer) is higher friction and misses the scope-level context.

**Network effect:** Every MCP server developer who installs `@cerniq/mcp-bridge` is now:
a) An CERNIQ customer (they need a verify-only API key)
b) A distribution channel (their users' agents need CERNIQ tokens to call their server)
...
**This score is not portable.** A competitor could build a scoring system, but an agent's behavioral history on CERNIQ cannot be migrated. This creates a switching cost that compounds over time.

**The signal flywheel:**

```
More agents using CERNIQ
       ↓
...
More accurate trust scores
       ↓
Relying parties trust CERNIQ scores more
       ↓
More relying parties require CERNIQ
       ↓
More agents use CERNIQ (requirement, not optional)
```
...
**What this means for CERNIQ:**
CERNIQ is already NIST-aligned on every technical dimension that will matter:
...
Companies that implement CERNIQ before NIST finalizes standards can claim "NIST-aligned agent identity infrastructure." This is worth multiples in enterprise sales. Companies that wait until mandate will be scrambling to find a compliant solution — CERNIQ will already have 18 months of production track record.

**Legal precedent tailwind:** Amazon sued Perplexity (Nov 2025) over agent identity violations. When companies are suing each other over this, enterprise legal teams are already drafting compliance requirements. CERNIQ's audit chain is exhibit-quality evidence in such disputes.

### 3.4 The Switzerland Positioning Moat

CERNIQ's architecture is deliberately vendor-neutral:
...
CERNIQ has no such objection. A company using Claude today and migrating to GPT-4o next year has the same CERNIQ integration. This is why infrastructure companies with neutral positioning (Cloudflare, Stripe, Twilio) command premium multiples — they're not betting on which LLM wins. They're the plumbing beneath all of them.
...
- Is it actually authorized by the human? (CERNIQ policy)
- Has its behavior been trustworthy? (BATE)
- Can the merchant independently verify the claim? (CERNIQ audit chain)

CERNIQ is the identity complement to ACP. The pitch to a Stripe-integrated merchant:

> "You're already using Stripe ACP to accept agent payments. CERNIQ is the trust layer that tells you whether to accept the payment request. Add `cerniq.verify(token)` before your ACP handler."
...
| 3-line MCP integration       | `@cerniq/mcp-bridge` `wrap()`          | `packages/mcp-bridge`                  | 🔶 Interface done, transport glue pending |
...
| GTM Requirement                  | Implementation                 | File/Endpoint                         | Status            |
| -------------------------------- | ------------------------------ | ------------------------------------- | ----------------- |
| MCP bridge (`wrap()` 3 lines)    | `@cerniq/mcp-bridge`           | `packages/mcp-bridge/`                | 🔶 Interface done |
| TypeScript SDK                   | `@cerniq/sdk`                  | `packages/sdk-ts/`                    | ✅                |
| Python SDK                       | `cerniq` Python package        | `packages/sdk-py/`                    | 🔶 Scaffold       |
...
| Fintech integration guide        | Stripe+CERNIQ pattern          | `docs/INTEGRATION_GUIDE_FINTECH.md`   | ✅                |
| CERNIQ as MCP server             | Tools: register, verify, trust | `modules/mcp/`                        | ✅                |
...
2. DID method publication (`did:cerniq:`)
...
claude-peers claim cerniq <module-id> --note "implementing X" --ttl 7200
...
**Files to create:** `packages/sdk-py/cerniq/` — `agent.py`, `client.py`, `crypto.py`, `types.py`
...
- `CerniqAgent.sign(payload)` → signs with private key, returns JWT
- `CerniqClient.verify(token)` → calls `/v1/verify`, returns VerifyResult
...
// Extract CERNIQ_HEADER_TOKEN from transport headers
// Call cerniq.verify(token, { action: actionPrefix + method })
...
1. `cerniq_plan_quota_pct` gauge metric — `(monthVerifyCount / monthlyQuota) × 100`
2. Alert: `cerniq_plan_quota_pct{plan="FREE"} > 90` → page (user needs upgrade prompt)
...
```bash
pnpm add @aws-sdk/client-kms @google-cloud/kms -F @cerniq/api
```
...
```bash
pnpm add @nestjs/schedule -F @cerniq/api
```
...
3. `DenialReason` in `verify.dto.ts` matches the list in `CERNIQ_API_SPEC.yaml`
...
| `PLAN_LIMIT_EXCEEDED` not in OpenAPI spec YAML   | Update `docs/spec/CERNIQ_API_SPEC.yaml`                            |
...
1. **Private keys never enter CERNIQ.** Agent private keys are generated client-side. The SDK is the only surface that touches a private key.
...
### What "CERNIQ Won" Looks Like

A developer building a LangChain agent in 2028 does not ask "should I use CERNIQ?" They ask "what's my CERNIQ policy?" the same way they ask "what's my Stripe plan?" or "what's my Auth0 tenant?" CERNIQ is the default agent identity infrastructure — not a product you evaluate but a layer you configure.
...
_Generated: 2026-05-05 | Session: cowork-master-state-analysis | Based on 190 TS files, 50 spec files, 18 modules, 10 packages, 14 DB models_
