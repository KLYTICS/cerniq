---
title: CERNIQ — Failure modes (FMEA)
status: draft
...
# CERNIQ — Failure modes (FMEA)

> **Purpose.** Component-by-component failure mode and effects
> analysis (FMEA) for every load-bearing piece of CERNIQ, with
...
| -------------------------------------- | --------------------------- | ---------------------------------------------------------------------------------- |
| Private keys never enter CERNIQ        | CLAUDE.md inv. 1            | Compromised app-server memory dump                                                 |
...
| C-02 | Random number generator weakness (e.g. predictable agent keypair)                  | 5   | 1   | 5   | 25  | Private keys generated client-side per CLAUDE.md inv. 1; CERNIQ never generates agent keys; CERNIQ-side jti via `crypto.randomUUID()` (Node native CSPRNG) | Code review only                                                                                    | Agent re-issuance after key rotation campaign                                                                                                                | SECURITY_RUNBOOK §"Suspected key compromise" |
...
| ID   | Failure mode                                                          | S   | L   | D   | RPN | Mitigation                                                                                                                                                              | Detection                                                                                 | Recovery                                                                                                                                                  | Runbook                           |
| ---- | --------------------------------------------------------------------- | --- | --- | --- | --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- |
| K-01 | KMS provider regional outage (AWS/GCP/Vault unreachable)              | 4   | 3   | 1   | 12  | Cross-region KMS replication for sign keys; fallback adapter in `kms.module.ts` per env config                                                                          | `kms_sign_error_rate` page > 0.1% over 1 min                                              | Switch `CERNIQ_KMS_PROVIDER` env on the impacted region; rolling restart                                                                                  | DR_RUNBOOK §"KMS provider outage" |
...
| K-05 | KMS rejects sign because IAM policy revoked (operator misconfig)      | 4   | 2   | 1   | 8   | Startup self-check signs a sentinel value; fails fast on broken IAM                                                                                                     | Startup health-check failure → pod refuses to enter ready                                 | Restore IAM policy; `cerniq-cli kms verify` from operator                                                                                                 | RUNBOOK §"KMS IAM"                |
| K-06 | KMS provider audit log gap (cannot prove sign happened)               | 3   | 2   | 4   | 24  | Mirror of every sign request to local outbox row with `kmsRequestId`; audit chain can be cross-verified against KMS-side log                                            | Reconciliation cron compares outbox `kmsRequestId` against KMS provider audit log nightly | Manual reconciliation via cerniq-cli; document gap in compliance evidence                                                                                 | RUNBOOK §"KMS reconciliation"     |
...
| D-04 | Audit table partition not created in time                        | 4   | 2   | 2   | 16  | Partition cron runs 24h ahead at 02:00 UTC on the 1st; alert if next month's partition is missing 12h before boundary                                    | Monitoring `cerniq_audit_partition_exists{month=N+1}` → 0                   | Manual partition creation (`infra/postgres/partition-cron.sql` ad-hoc); investigate cron failure                            | RUNBOOK §"Partition cron"                |
| D-05 | Audit append-only trigger bypassed (UPDATE/DELETE on AuditEvent) | 5   | 1   | 2   | 10  | Trigger in migration `20260502000100_audit_append_only`; only `cerniq_schema_owner` role can disable per ADR-0006                                        | Trigger failure logged; alert on any successful UPDATE/DELETE on AuditEvent | **Do not roll forward**: snapshot + investigate; redaction job uses dedicated grant per ADR-0006 § "Operator authorization" | SECURITY_RUNBOOK §"Audit tamper attempt" |
...
| ID   | Failure mode                                              | S   | L   | D   | RPN | Mitigation                                                                                                                                                                                                      | Detection                                              | Recovery                                                                                                                                                           | Runbook                                  |
| ---- | --------------------------------------------------------- | --- | --- | --- | --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------- |
| R-01 | Redis cluster outage (primary + standby down)             | 4   | 1   | 1   | 4   | Cache reads fall back to Postgres (logged via `cerniq_cache_set_failed_total`); spend evaluation 503; DPoP verify denied                                                                                        | Redis health probe; pod readiness false                | Wait for Railway managed restart; if > 15 min, manual failover from snapshot                                                                                       | DR_RUNBOOK §"Redis cluster down"         |
...
| Q-05 | Cron drift (policy expiry sweep skipped)                    | 2   | 2   | 3   | 12  | BullMQ scheduled job persisted in DB 3; on Redis restart, scheduled jobs resume from AOF                                                     | Skipped-execution metric             | Manual sweep via `cerniq-cli policies expire`; investigate cron health | RUNBOOK §"Cron drift"             |
...
| ID   | Failure mode                                                                                     | S   | L   | D   | RPN | Mitigation                                                                                                                                          | Detection                                                                         | Recovery                                                                                              | Runbook                                |
| ---- | ------------------------------------------------------------------------------------------------ | --- | --- | --- | --- | --------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | -------------------------------------- |
| A-01 | Token replay across consecutive verify calls                                                     | 4   | 5   | 1   | 20  | Layer 1 jti per call (THREAT_MODEL_v2 §7.3); Layer 3 jti set in Redis DB 2; verify-result cache key includes jti per A-016; DPoP nonce per ADR-0010 | `cerniq_replay_blocked_total` rate; BATE weight `AGENT_DPOP_REPLAY_ATTEMPT: -200` | Per-agent revocation if persistent; trust-score-driven cold-start ban                                 | SECURITY_RUNBOOK §"Replay"             |
...
| A-03 | Brute-force key guessing on private key (theoretical)                                            | 5   | 1   | 5   | 25  | Ed25519 128-bit security; private keys never in CERNIQ so not exposed by CERNIQ breach; agent-side compromise responsibility per CLAUDE.md inv. 1   | n/a                                                                               | n/a (out of CERNIQ attack surface)                                                                    | n/a                                    |
...
which means pausing the entire CERNIQ write path, accepting service
...
| ID   | Failure mode                                                                | S   | L   | D   | RPN | Mitigation                                                                                                                                             | Detection                                            | Recovery                                                                  | Runbook                         |
| ---- | --------------------------------------------------------------------------- | --- | --- | --- | --- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------- | ------------------------------------------------------------------------- | ------------------------------- |
| O-01 | Config drift (env var diverges between staging + prod)                      | 2   | 4   | 4   | 32  | Single Zod-validated config in `apps/api/src/config/`; CI enforces config schema parity; quarterly drift audit                                         | Config-diff report from `cerniq-cli config diff`     | Reconcile via PR; document divergence intent                              | RUNBOOK §"Config drift"         |
...
1. K-01 detection within 30 s; cross-region KMS adapter switches to
   us-west via `CERNIQ_KMS_PROVIDER` rolling config.
...
- Validate D-01 mitigation; confirm partition for next month exists
  (manual `cerniq-cli admin partition ensure-next` if cron retry
  exhausts).
...
| ------------ | --------------------------------------------- | ------------ | ---------------------------- |
| C-01..C-08   | `cerniq_crypto_*`                             | page         | on-call + security           |
| K-01..K-06   | `cerniq_kms_*`                                | page         | on-call                      |
| D-01..D-10   | `cerniq_postgres_*`                           | page         | on-call + DBA                |
| R-01..R-07   | `cerniq_redis_*`                              | page or warn | on-call (page on R-04, R-06) |
| Q-01..Q-05   | `cerniq_bullmq_*`                             | warn         | on-call                      |
| E-01..E-06   | `cerniq_external_*`                           | warn         | on-call                      |
| A-01..A-07   | `cerniq_security_*`                           | page         | security                     |
| AC-01..AC-07 | `cerniq_audit_chain_*`                        | page         | security + on-call           |
| O-01..O-07   | (process — no alert; covered by review/audit) | n/a          | operator                     |
| W-01..W-04   | `cerniq_worker_*` (Phase 3)                   | page or warn | on-call                      |
