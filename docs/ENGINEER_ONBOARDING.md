# CERNIQ — Engineer Onboarding Guide

## Your First Day, Week, and Month

> **Welcome to CERNIQ.** You're joining a team building the identity and verification layer for AI agents — the trust infrastructure that makes autonomous AI safe in production. This doc gets you from zero to shipping in 30 days.
...
```
[ ] GitHub access (cerniqlabs/cerniq repo)
...
This is not optional. CERNIQ has strong architecture invariants — knowing them before touching code saves everyone time.
...
```bash
# Clone
git clone git@github.com:cerniqlabs/cerniq.git
cd cerniq
...
apps/api/src/common/
  ...
  errors/           ← CerniqError subclasses (typed errors, not strings)
  guards/           ← ApiKeyGuard, AdminGuard — ALL endpoints go through these

packages/
  types/            ← Zod schemas — the API contract. All constants live here.
  sdk-ts/           ← @cerniq/sdk — the public TypeScript SDK
  verifier-rp/      ← @cerniq/verifier-rp — offline JWT verifier for relying parties
```
...
CERNIQ uses a claim protocol for parallel sessions. Before touching code:
...
5. Crypto changes require a security review tag: @cerniq/security
...
- [ ] Error types use CerniqError subclasses
...
- **Errors are typed** — use `CerniqError` subclasses from `apps/api/src/common/errors`.
...
- `CerniqClient` — principal management (agents, policies, audit)
- `CerniqVerifier` (from `verifier-rp`) — offline JWT verification at relying party
- `CerniqCallbackHandler` — LangChain integration
...
cerniq agents register --name "my-test-agent" --ttl 3600
cerniq policy apply --agent my-test-agent --scope payment:read --limit 500
cerniq verify --agent my-test-agent --scope payment:read --amount 100
cerniq audit tail --agent my-test-agent
...
cerniq agents get --id my-test-agent --show-trust-breakdown
...
import { DEFAULT_SPEND_LIMIT } from '@cerniq/types';
...
# Scripts
pnpm tsx scripts/generate-cerniq-keys.ts         # generate Ed25519 key pairs
pnpm tsx scripts/audit-verify-chain.ts --help   # audit chain verification

# CLI (install globally first)
pnpm build --filter=@cerniq/cli
...
_Onboarding guide version: 1.0 | CERNIQ Phase 1_  
_Next review: after first 3 engineers onboarded_
