# OKORO — Master Engineering & GTM State Document
## Phase 1 GA Closure | May 2026
### Classification: INTERNAL — Engineering Lead + All Terminals

> **How to read this document.** It is the single source of truth for what OKORO *is right now* — code-proven, not aspirational. Every claim is backed by a specific file or endpoint. Use PART VII to find your terminal assignment. Use PART VIII to find your next task.
...
- Idempotency: Redis SETNX on `okoro:stripe:event:{eventId}`, 7-day TTL
...
**Quota enforcement:**
- Redis counter: `okoro:usage:{principalId}:{YYYY-MM}`
- DB backfill on Redis miss: `AuditEvent.count WHERE principalId + timestamp >= startOfMonth`
- Plan tier cached: `okoro:plan:{principalId}`, 5-minute TTL
...
10. If band changed: fire `okoro.agent.trust_score_changed` webhook
...
**Delivery:** BullMQ worker, 5 retries, exponential backoff (1s→2s→4s→8s→16s), HMAC-SHA256 signature on payload with `X-Okoro-Signature: t={ts},v1={hmac}`.

**Events fired today:**
- `okoro.agent.trust_score_changed` (on band crossing)
...
Returns JWKS-format JSON with active signing public key. ETag caching, CORS-open. Relying parties poll this to verify audit chain signatures without OKORO account. This is the trust anchor for the entire audit chain — any party can independently verify signatures without calling OKORO's API.
...
`PrincipalOnboarding` row tracks 7 milestones: hasFirstAgent, hasFirstPolicy, hasFirstVerify, hasKmsConfigured, hasMcpServerRegistered, + 2 more. Milestones are one-way ratchets (never unset). Drives the dashboard wizard and `okoro doctor` CLI command.
...
#### Module: `mcp` — OKORO as an MCP Server
OKORO exposes its own MCP server so agents can interact with OKORO programmatically (register, verify, query trust scores) through the MCP tool-call protocol.

---

### 2.2 Packages

#### `packages/sdk-ts` — `@okoro/sdk` (TypeScript)
**Files:** `agent.ts`, `browser.ts`, `crypto.ts`, `http.ts`, `policy.ts`, `types.ts`, `index.ts`

The developer-facing SDK. Key surface:
- `OkoroAgent.sign(payload)` → Ed25519-signed JWT, client-side only (private key never leaves)
- `OkoroClient.verify(token, options)` → calls `/v1/verify`, returns `VerifyResult`
- Works in Node.js and browser (`browser.ts` uses WebCrypto API)

#### `packages/mcp-bridge` — `@okoro/mcp-bridge`
**THE DISTRIBUTION WEDGE.** `wrap(mcpServer, { okoro, actionPrefix })` inserts OKORO verification before every tool call. Any MCP server + 3 lines = OKORO-protected.
...
#### `packages/cli` — `okoro` CLI
`okoro doctor`, `okoro agent register`, `okoro verify`, `okoro kms rotate`. Developer tooling for local development and production operations.

#### `packages/audit-verifier` — Standalone Chain Verifier
Any party can run this to independently verify an OKORO audit chain without calling our API. This is a core trust property — verifiability is not contingent on OKORO's availability.

#### `packages/verifier-rp` — Relying Party Verifier
Lightweight package relying parties embed to verify OKORO tokens without an OKORO account (for the edge case where latency requirements preclude a round-trip to the OKORO API).
...
OKORO's distribution wedge is `@okoro/mcp-bridge`. Here is why it is structurally irreplaceable:

```typescript
// BEFORE OKORO: tool call reaches your server with zero identity context
...
// AFTER OKORO: 3 lines, verified identity on every tool call
import { wrap } from '@okoro/mcp-bridge';
const protected = wrap(server, { okoro, actionPrefix: 'mcp.myserver.' });
...
3. By sitting at the MCP layer, OKORO intercepts 100% of agent-initiated actions, not just some.
4. The alternative (wrapping at the HTTP/API layer) is higher friction and misses the scope-level context.

**Network effect:** Every MCP server developer who installs `@okoro/mcp-bridge` is now:
a) An OKORO customer (they need a verify-only API key)
b) A distribution channel (their users' agents need OKORO tokens to call their server)
...
**This score is not portable.** A competitor could build a scoring system, but an agent's behavioral history on OKORO cannot be migrated. This creates a switching cost that compounds over time.

**The signal flywheel:**
```
More agents using OKORO
       ↓
...
More accurate trust scores
       ↓
Relying parties trust OKORO scores more
       ↓
More relying parties require OKORO
       ↓
More agents use OKORO (requirement, not optional)
```
...
**What this means for OKORO:**
OKORO is already NIST-aligned on every technical dimension that will matter:
...
Companies that implement OKORO before NIST finalizes standards can claim "NIST-aligned agent identity infrastructure." This is worth multiples in enterprise sales. Companies that wait until mandate will be scrambling to find a compliant solution — OKORO will already have 18 months of production track record.

**Legal precedent tailwind:** Amazon sued Perplexity (Nov 2025) over agent identity violations. When companies are suing each other over this, enterprise legal teams are already drafting compliance requirements. OKORO's audit chain is exhibit-quality evidence in such disputes.

### 3.4 The Switzerland Positioning Moat

OKORO's architecture is deliberately vendor-neutral:
...
OKORO has no such objection. A company using Claude today and migrating to GPT-4o next year has the same OKORO integration. This is why infrastructure companies with neutral positioning (Cloudflare, Stripe, Twilio) command premium multiples — they're not betting on which LLM wins. They're the plumbing beneath all of them.
...
- Is it actually authorized by the human? (OKORO policy)
- Has its behavior been trustworthy? (BATE)
- Can the merchant independently verify the claim? (OKORO audit chain)

OKORO is the identity complement to ACP. The pitch to a Stripe-integrated merchant:
> "You're already using Stripe ACP to accept agent payments. OKORO is the trust layer that tells you whether to accept the payment request. Add `okoro.verify(token)` before your ACP handler."
...
| 3-line MCP integration | `@okoro/mcp-bridge` `wrap()` | `packages/mcp-bridge` | 🔶 Interface done, transport glue pending |
...
| GTM Requirement | Implementation | File/Endpoint | Status |
|----------------|---------------|---------------|--------|
| MCP bridge (`wrap()` 3 lines) | `@okoro/mcp-bridge` | `packages/mcp-bridge/` | 🔶 Interface done |
| TypeScript SDK | `@okoro/sdk` | `packages/sdk-ts/` | ✅ |
| Python SDK | `okoro` Python package | `packages/sdk-py/` | 🔶 Scaffold |
...
| Fintech integration guide | Stripe+OKORO pattern | `docs/INTEGRATION_GUIDE_FINTECH.md` | ✅ |
| OKORO as MCP server | Tools: register, verify, trust | `modules/mcp/` | ✅ |
...
2. DID method publication (`did:okoro:`) 
...
**Files to create:** `packages/sdk-py/okoro/` — `agent.py`, `client.py`, `crypto.py`, `types.py`
...
- `OkoroAgent.sign(payload)` → signs with private key, returns JWT
- `OkoroClient.verify(token)` → calls `/v1/verify`, returns VerifyResult
...
// Extract OKORO_HEADER_TOKEN from transport headers
// Call okoro.verify(token, { action: actionPrefix + method })
...
**What's needed:**
1. `okoro_plan_quota_pct` gauge metric — `(monthVerifyCount / monthlyQuota) × 100`
2. Alert: `okoro_plan_quota_pct{plan="FREE"} > 90` → page (user needs upgrade prompt)
...
**What's needed:**
```bash
pnpm add @aws-sdk/client-kms @google-cloud/kms -F @okoro/api
```
...
3. `DenialReason` in `verify.dto.ts` matches the list in `OKORO_API_SPEC.yaml`
...
| `PLAN_LIMIT_EXCEEDED` not in OpenAPI spec YAML | Update `docs/spec/OKORO_API_SPEC.yaml` |
...
1. **Private keys never enter OKORO.** Agent private keys are generated client-side. The SDK is the only surface that touches a private key.
...
### What "OKORO Won" Looks Like

A developer building a LangChain agent in 2028 does not ask "should I use OKORO?" They ask "what's my OKORO policy?" the same way they ask "what's my Stripe plan?" or "what's my Auth0 tenant?" OKORO is the default agent identity infrastructure — not a product you evaluate but a layer you configure.
