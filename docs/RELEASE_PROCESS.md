...
|---|---|---|---|
| `@aegis/sdk` | `packages/sdk-ts` | npm | MIT |
| `@aegis/types` | `packages/types` | npm | MIT |
| `@aegis/verifier-rp` | `packages/verifier-rp` | npm | MIT |
| `aegis` (Python) | `packages/sdk-py` | PyPI | MIT |

Internal packages (`@aegis/api`, `@aegis/dashboard`, `@aegis/scripts`,
`@aegis/cli`, `@aegis/eslint-config`, `@aegis/tsconfig`,
`@aegis/audit-verifier`, `@aegis/mcp-bridge`, `@aegis/mcp-server`) are
...
- [ ] You have an `npm` login with publish rights on the `@aegis` scope
...
```bash
pnpm --filter @aegis/scripts run gen:changelog --since <YYYY-MM-DD>
```
...
```bash
pnpm --filter @aegis/scripts run gen:changelog --since <date> --dry-run
```
...
```bash
pnpm --filter @aegis/scripts run gen:changelog --package sdk-ts --since <date>
...
- **Major** — breaking changes. **Lockstep across the SDK packages**:
  if `@aegis/sdk` breaks, `@aegis/verifier-rp` and the Python SDK
  bump majors at the same time so cross-version compatibility doesn't
  drift. `@aegis/types` may bump independently when only schema field
  names change, but a breaking `@aegis/types` bump always implies a
...
git commit -m "chore(release): @aegis/sdk@x.y.z, @aegis/verifier-rp@x.y.z, ..."
```
...
```bash
pnpm --filter @aegis/scripts run publish:dry-run:all
```

This iterates every public `@aegis/*` package and asserts:
...
| `package.json` requires | `name`, `version` (valid semver), `description`, `license`, `repository.url` referencing the aegis repo, `engines.node`, `keywords` (≥3) |
...
```bash
pnpm --filter @aegis/scripts run publish:dry-run -- --all --json > pre-publish.json
```

Strict mode for "everything must be perfect" releases:

```bash
pnpm --filter @aegis/scripts run publish:dry-run -- --all --strict
```
...
```bash
# Tag the release.
git tag -a v$(node -p "require('./packages/sdk-ts/package.json').version") -m "AEGIS SDK release"

# Publish each public package. pnpm rewrites workspace:* automatically.
pnpm --filter @aegis/sdk publish --access public
pnpm --filter @aegis/types publish --access public
pnpm --filter @aegis/verifier-rp publish --access public
...
```bash
npm view @aegis/sdk version       # should match
npm view @aegis/verifier-rp version
pip index versions aegis
```
...
npm deprecate @aegis/sdk@<bad-version> "Broken — use @aegis/sdk@<good-version> or later."

# For a range:
npm deprecate "@aegis/sdk@>=1.2.0 <1.2.4" "Broken — see GH issue #N. Upgrade to 1.2.4."
```
...
python -m twine yank aegis==<bad-version> --reason "Broken — use <good-version>."
```
...
`aegis:release-<date>` so other sessions don't tag/publish in parallel:

```bash
~/.claude/peers/bin/claude-peers claim aegis release-2026-05 --note "publishing v0.2.0 across SDKs" --ttl 7200
# ... do the release ...
~/.claude/peers/bin/claude-peers release aegis:release-2026-05
```
