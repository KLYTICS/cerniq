...
**Spec reference**: MCP Authorization, revision **2025-03-26** (current at time of writing). Operator must reconfirm against the live spec at <https://spec.modelcontextprotocol.io/specification/2025-03-26/basic/authorization/> before public launch — see _Open questions_ §4.
...
That paragraph is the lens for everything below. CERNIQ slots into this picture in three different positions, and they have different operational and trust trade-offs.

---

## 2. Three integration patterns

| Pattern                                  | CERNIQ role                                                 | MCP server change                                                                              | Auth0 needed?                      | Token shape                                                  | Best for                                          |
| ---------------------------------------- | ----------------------------------------------------------- | ---------------------------------------------------------------------------------------------- | ---------------------------------- | ------------------------------------------------------------ | ------------------------------------------------- |
| **(a) CERNIQ as Authorization Server**   | OAuth 2.1 AS for the MCP server                             | Verify our JWT, trust our metadata                                                             | No (or yes for human consent step) | EdDSA JWT signed by CERNIQ audit key                         | New MCP servers, CERNIQ-native shops              |
| **(b) CERNIQ as token verifier**         | Sidecar verifier the MCP server calls before tool execution | Add `cerniq.verify()` call in the tool handler (or `wrapMcpHandler` from `@cerniq/mcp-bridge`) | No, MCP server keeps its own AS    | Whatever the MCP server's AS issues, plus CERNIQ agent token | Existing MCP servers that already have an AS      |
| **(c) CERNIQ-signed JWT, JWKS-verified** | Pure issuer of EdDSA JWTs, no runtime call                  | Verify JWT via `/.well-known/jwks.json` cache                                                  | No                                 | EdDSA JWT signed by CERNIQ audit key (same as (a))           | High-throughput / edge MCP servers, p99-sensitive |

### Trade-offs

#### (a) CERNIQ as the OAuth 2.1 Authorization Server

**Pros**

- Single source of truth: agent identity, policy, and OAuth all live in CERNIQ. The MCP server only knows how to verify a token.
- Implements the full spec — `/authorize`, `/token`, `/.well-known/oauth-authorization-server`, `/register`, `/jwks.json` — meaning _any_ spec-conformant MCP client (Claude Desktop, Cursor, Cline, future hosts) works without per-server SDK changes.
...
- Adds a stateful AS to CERNIQ: authorization-code storage, refresh-token rotation, client-credentials store. ADR-0009 says human identity is Auth0's job — for the consent screen we **delegate the user-auth step to Auth0** and then mint our own CERNIQ access token. See §3 pattern (a) step 4.
...
#### (b) CERNIQ as token verifier (sidecar)

**Pros**

- **Zero changes to the MCP server's auth.** Operator keeps Okta / Auth0 / their own AS for the human-or-machine login part; CERNIQ verifies the _agent identity_ on top.
- Two independent signals — "this client passed our OAuth" + "this agent is in good standing per CERNIQ" — defense in depth.
...
- Compatible with stdio transport (where the OAuth flow is undefined) because the CERNIQ token rides in `params._cerniq_token` per `extractToken()` in the bridge.

**Cons**

- Two tokens on every call. Marginally heavier wire and more failure modes ("OAuth said yes but CERNIQ said no" UX).
- The MCP server still has to operate its own AS — none of that complexity goes away for the operator.
- Latency: every tool call triggers a network round-trip to CERNIQ `/v1/verify`. Mitigate with the Phase-3 Cloudflare Workers edge per ADR-0003.
...
#### (c) CERNIQ-signed JWT, JWKS-verified locally

**Pros**

- **Zero round-trip.** The MCP server downloads `https://cerniq.dev/.well-known/jwks.json` once, caches it (TTL from `Cache-Control: max-age=86400` per `wellknown.controller.ts`), and verifies every request locally. p99 verification is sub-millisecond.
- Works at the edge (Cloudflare Workers, Vercel Edge, Deno Deploy) without an CERNIQ dependency in the hot path.
...
- This is the _same JWT shape_ as (a) — pattern (c) is just (a) without the round-trip-on-every-call. An MCP server can start at (a) and migrate to (c) by flipping a config flag.
...
**Recommended when:** the MCP server is latency-critical and the operator accepts ≤60 s revocation lag, or as the _outer_ layer in a (b)+(c) stack ("verify JWT signature locally, then call `/v1/verify` only for high-risk actions").

### Default recommendation

**Pattern (b) is the default for v1.** It maps 1:1 to the existing `@cerniq/mcp-bridge` skeleton (one `wrap()` call, no spec ceremony), keeps revocation immediate, and does not require us to ship an OAuth 2.1 AS implementation in the same release as the verify-path hardening. Pattern (a) ships in a follow-on (M-021) once the AS surface is cut and audited. Pattern (c) is a deployment switch on top of (a), flipped per-relying-party from the dashboard.

---

## 3. Concrete integration steps (10 steps each)

### Pattern (a) — CERNIQ as OAuth 2.1 AS for an MCP server

1. **MCP server operator registers their server with CERNIQ.** `POST /v1/mcp-servers` (existing in `mcp.controller.ts`) with `endpoint`, `transport: 'streamable-http'`, `actionPrefix`, `minTrustBand`. Returns an `mcp_<ulid>` id.
2. **CERNIQ provisions an OAuth client record** (NEW — M-021). Per the AS-metadata standard (RFC 8414), CERNIQ exposes the metadata document at `https://cerniq.dev/.well-known/oauth-authorization-server` containing `issuer`, `authorization_endpoint`, `token_endpoint`, `registration_endpoint`, `jwks_uri`, `code_challenge_methods_supported: ["S256"]`, `grant_types_supported: ["authorization_code", "refresh_token", "client_credentials", "urn:ietf:params:oauth:grant-type:token-exchange"]`.
3. **MCP server points its `WWW-Authenticate` challenge at the CERNIQ metadata URL.** When an unauthenticated request arrives the server replies `401 WWW-Authenticate: Bearer realm="cerniq", as_uri="https://cerniq.dev/.well-known/oauth-authorization-server"`. Spec-conformant clients (Claude Desktop ≥ 2025-04, Cursor ≥ 0.45, Cline ≥ 3.0) parse this and start the auth flow.
4. **MCP client performs Dynamic Client Registration** (RFC 7591) against `POST /v1/oauth/register` (NEW). CERNIQ returns `client_id`, `client_secret` (only for confidential clients), `registration_access_token`. Public-client / native MCP clients (Claude Desktop) get no secret.
...
6. **CERNIQ delegates the human consent step to Auth0** (per ADR-0009). The `/v1/oauth/authorize` handler checks for an active Auth0 session cookie; if absent, redirects to Auth0 Universal Login. After Auth0 callback, CERNIQ shows the consent screen ("MCP server X wants to invoke tools on behalf of agent Y owned by you"), records consent in `OAuthConsent` table, and 302s back to the MCP client's `redirect_uri` with `code=<authorization_code>`.
7. **MCP client exchanges the code for tokens.** `POST /v1/oauth/token` with `grant_type=authorization_code&code=...&code_verifier=...&client_id=...`. CERNIQ validates PKCE, issues an EdDSA JWT (`access_token`, 60 s TTL) signed by the audit key, plus a refresh token. JWT carries `iss`, `sub: <agentId>`, `aud: <mcpServerId>`, `scope`, `cnf: { jkt: <DPoP thumbprint> }` (if DPoP), `principalId`, `trustBand` claims.
...
9. **MCP server verifies the JWT.** Either via `@cerniq/mcp-bridge.wrapMcpHandler({ cerniq, mode: 'jwks' })` (local verify, pattern (c)) or by calling `POST /v1/verify` (round-trip, pattern (b) hybrid). Both paths share the same algorithm via `verify.algorithm.ts` per ADR-0003.
10. **CERNIQ audits every issuance.** `OAuthIssuance` audit row per token, signed via the audit chain (ADR-0005). Refresh-token rotation logged as `oauth.refresh`. Revocation via `POST /v1/oauth/revoke` (RFC 7009) — flips `revoked=true` and posts an audit event; pattern-(c) consumers see staleness only until JWT TTL elapses.

### Pattern (b) — CERNIQ as a sidecar verifier

1. **MCP server operator keeps their existing AS** (Auth0, Okta, Keycloak, custom). No change to the human-or-machine login flow.
2. **MCP server operator registers the server with CERNIQ:** `POST /v1/mcp-servers` (same as pattern (a) step 1). The registration is what gives the server an `mcp_<ulid>` audit slice.
3. **MCP server adds the CERNIQ verifier package** to dependencies: `pnpm add @cerniq/mcp-bridge @cerniq/sdk`.
4. **MCP server wraps each tool handler:**
   ...
   server.setRequestHandler(
     ToolSchema,
     wrapMcpHandler(
       { cerniq, actionPrefix: 'mcp.fs.', minTrustBand: 'VERIFIED' },
       async (req, ctx) => readFile(req.params.path),
     ),
   );
   ```
5. **Agent (the MCP client side) acquires an CERNIQ token** via the SDK: `await cerniq.agent.sign({ action: 'mcp.fs.read_file', amount: null })`. This is the existing flow; no MCP-specific changes.
6. **Agent passes the token alongside the MCP server's own auth.** For `streamable-http` transport: `Authorization: Bearer <mcp-server-token>` AND `X-CERNIQ-Token: <cerniq-token>` (header constant `CERNIQ_HEADER_TOKEN` from `@cerniq/types`). For `stdio`: CERNIQ token rides in `params._cerniq_token` per the bridge's `extractToken()`.
7. **MCP server's transport authenticates the request** via its existing AS (no CERNIQ involvement).
8. **The bridge intercepts before the tool handler runs**, calls `cerniq.verify(token, { action: 'mcp.fs.<method>' })` against `POST /v1/verify`, and either invokes the tool or throws `BridgeDenialError` with the CERNIQ denial reason.
9. **CERNIQ records the verify** with `relyingPartyId = mcp_<ulid>` (M-022 wiring) so the dashboard can show "MCP server X invoked Y tools today, Z denied."
...
### Pattern (c) — CERNIQ-signed JWT, JWKS-verified

1. **CERNIQ publishes its JWKS** (already live: `wellknown.controller.ts` `GET /.well-known/jwks.json`). EdDSA / Ed25519 public key, `kid` is the audit-key thumbprint, `Cache-Control: public, max-age=86400, stale-while-revalidate=604800`.
...
3. **Agent acquires an CERNIQ access token** from `POST /v1/oauth/token` with `grant_type=client_credentials&audience=<mcpServerId>` (NEW endpoint — pattern (a) infrastructure). Returns an EdDSA JWT.
4. **Agent invokes the MCP tool** with `Authorization: Bearer <jwt>` and a DPoP proof.
5. **MCP server fetches and caches the JWKS** at startup. In Node: `import { createRemoteJWKSet, jwtVerify } from 'jose'; const jwks = createRemoteJWKSet(new URL('https://cerniq.dev/.well-known/jwks.json'));`. The `jose` library honors `Cache-Control` automatically.
6. **MCP server verifies the JWT locally** on every call: `const { payload } = await jwtVerify(token, jwks, { issuer: 'https://cerniq.dev', audience: mcpServerId, algorithms: ['EdDSA'] });`. Reject if `payload.cnf.jkt` doesn't match the DPoP proof's JWK thumbprint.
7. **MCP server enforces scope and trust band locally** from the JWT claims (`scope`, `trustBand`, `principalId`).
8. **MCP server schedules a periodic `GET /v1/agents/:agentId/status`** (5-minute cadence) for any long-lived session — this is the staleness mitigation for revocation. Optional: subscribe to webhook `cerniq.agent.trust_score_changed` for push.
...
3. **JWT audience policy.** Should an CERNIQ-issued JWT be valid for a _single_ MCP server (`aud: mcp_01HZ...`) or for any MCP server the principal owns (`aud: <principalId>`)? Single-server is safer, multi-server is friendlier. Recommendation: single-server, with a `aud_list` extension claim if multi-server is needed.
...
6. **Refresh-token lifetime.** Default 30 days is OAuth norm; CERNIQ short-lived ethos suggests 24 hours. Open. Recommendation: 7 days, rotated on every use, revocable per agent.
...
| `packages/mcp-server/` (NEW package)                             | The `@cerniq/mcp-server` distribution — exposes CERNIQ API as MCP tools per ADR-0008 §1.2                                                             | inverse direction |
...
- `config/config.schema.ts` — add `OAUTH_ISSUER` (default `https://cerniq.dev`), `OAUTH_AUTHORIZATION_CODE_TTL_S=60`, `OAUTH_REFRESH_TTL_S=604800`, `OAUTH_REQUIRE_DPOP=true`.
