...
This document maps CERNIQ's product surfaces to the controls and
...
8. [What CERNIQ is NOT in scope for](#8-what-cerniq-is-not-in-scope-for)

---

## 1. SOC 2 Trust Services Criteria

CERNIQ targets **SOC 2 Type II** for the Security, Availability, and
...
| Control | Title                     | CERNIQ evidence                                                                                                            |
| ------- | ------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| C1.1    | Confidentially identified | Public keys only; private keys never enter CERNIQ (CLAUDE.md invariant 1)                                                  |
...
| Control | Title                                                       | CERNIQ evidence                                                                                              |
| ------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| A.8.1   | User endpoint devices                                       | N/A — CERNIQ is a server product                                                                             |
| A.8.2   | Privileged access rights                                    | Admin endpoints gated by `X-CERNIQ-Admin` token (separate from FULL keys)                                    |
...
| A.8.11  | Data masking                                                | Pino redact list (`req.headers.x-cerniq-api-key` etc.) in `app.module.ts`                                    |
...
| A.8.18  | Use of privileged utility programs                          | `cerniq` CLI scoped to ops actions; admin endpoints behind separate token                                    |
...
The GDPR requirements that touch CERNIQ directly. Customers acting
as Data Controllers retain primary GDPR responsibility; CERNIQ as
Data Processor (per the standard DPA shipped with enterprise
contracts) implements the controls below.

| Article | Title                                      | CERNIQ evidence                                                                                                                                               |
| ------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Art. 6  | Lawfulness of processing                   | Customer-supplied consent; CERNIQ processes per contract                                                                                                      |
...
raw, the chain stays valid**. `@cerniq/audit-verifier` honors this:
...
**CERNIQ is NOT in PCI scope by default.** CERNIQ does not see, store,
...
customer's PSP side (Stripe / Adyen / etc.), not in CERNIQ.

The controls below apply when an CERNIQ deployment is bundled with
the customer's PCI environment (e.g. they ship our verifier inline
in their checkout). In that case CERNIQ is in **PCI Zone B** at most.

| Requirement | Title                                                                | CERNIQ evidence                                                                                |
| ----------- | -------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
...
| Req. 3      | Protect stored account data                                          | **N/A — CERNIQ does not store cardholder data.** SPTs / PSP tokens pass through; never persist |
...
CERNIQ is **infrastructure** under the EU AI Act, not an AI system per
se (no model deployed by CERNIQ makes safety-relevant decisions on
its own). Customers using CERNIQ to gate their own AI agents inherit
specific obligations; CERNIQ provides the evidence layer.
...
For high-risk AI applications (Annex III), CERNIQ supplies the
evidence layer that the customer's quality management system needs.
The customer remains the Provider / Deployer; CERNIQ is the
...
| Function | Category                       | CERNIQ coverage                                            |
| -------- | ------------------------------ | ---------------------------------------------------------- |
...
3. Send them the row's CERNIQ-evidence link as the answer.
4. For the 5–10% of questions the table doesn't cover, escalate to
   CERNIQ engineering and add a row to this doc.
...
3. Show the auditor `npx @cerniq/audit-verifier verify` running
   green against their export.

---

## 8. What CERNIQ is NOT in scope for

Setting expectations explicitly so customers don't ask CERNIQ to do
things CERNIQ deliberately does not:

- **Card processing.** CERNIQ gates agent identity, not payment auth.
  PCI Zone A stays with the PSP.
- **AML / sanctions screening.** Customer's compliance stack screens
  the transaction; CERNIQ authorizes the agent making it.
...
- **Anti-money laundering rules.** CERNIQ is not a transaction-monitor.
- **The customer's own SOC 2.** CERNIQ provides evidence; the
  customer's controls are the customer's own.
- **End-user authentication.** That's the IdP's job (Auth0 / Clerk /
  WorkOS); CERNIQ handles the _agent_ layer above the user layer.

---

_This document is generated and maintained alongside the codebase.
File a PR if any row's evidence link is wrong or stale._
