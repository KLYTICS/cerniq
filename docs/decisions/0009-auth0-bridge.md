# ADR-0009 — Auth0 bridges human identity; CERNIQ owns agent identity
...
CERNIQ authenticates agents. It does not authenticate humans. But every
production deployment needs _someone_ — a human operator, an SRE, a
...
We pick **Auth0** as the default human-identity provider for CERNIQ for
five reasons:
...
3. Tenant model maps cleanly: Auth0 Organizations ↔ CERNIQ Principals.
4. Auth0 Actions allow runtime hooks — perfect for "every human-issued
   API key gets logged into the CERNIQ audit chain."
5. Customers already deploy it; we don't ask them to add a vendor.

What this ADR is _not_: a commitment that Auth0 is the only IdP we'll
...
1. **Auth0 is the default IdP for CERNIQ dashboard + admin API access.**
   Bootstrap defaults document Auth0; an `cerniq bootstrap --idp clerk`
   path remains open.
2. **Auth0 Organization ↔ CERNIQ Principal binding.** Every CERNIQ
...
3. **JWKS-based token verification.** CERNIQ API verifies Auth0 access
...
4. **Auth0 Actions for CERNIQ-side enforcement.** Two Actions ship in
   `infra/auth0/actions/`:
   - `cerniq-audit-login.js` — logs every human login as an CERNIQ
     audit event with decision=APPROVED, principal=Auth0 user.
   - `cerniq-block-non-admin-mfa-skip.js` — denies MFA-skipped logins for
     users with `cerniq:admin` role.
...
- Org → Principal binding gives multi-tenancy "for free": every Auth0
  customer org maps to one CERNIQ principal, isolation guaranteed by
...
- Hard dependency on a third-party SaaS for dashboard logins. Mitigation:
  ...
  is down still call the CERNIQ API directly via curl/SDK.
...
- Cost: Auth0 enterprise pricing is non-trivial. We pay for CERNIQ-internal
...
- `packages/sdk-ts` gains a thin `Cerniq.fromAuth0(accessToken)` helper.
...
- ADR-0011 (key rotation) — Auth0 client secret rotation is documented
  in the runbook, separate from CERNIQ Ed25519 keys.
