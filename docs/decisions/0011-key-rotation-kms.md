...
CERNIQ Phase 0 ships with two long-lived signing keys held in env vars:
...
1. **No `signingKeyId` on signed records.** When we rotate, every audit
   ...
   The well-known endpoint must serve a _list_ keyed by `kid`, and every
...
3. **`KmsAdapter` interface.** Defined in
   ...
   - `listKeys(purpose): Promise<KeyMetadata[]>` (powers the JWKS endpoint)
     The adapter never exposes raw private key material to CERNIQ code; all
...
5. **Rotation cadence (operator decision OD-007):**
   ...
   - JWT signing key: every 6 months. Tokens issued under old key remain
     ...
     of accepting set.
   - Both rotations: `cerniq-cli kms rotate <purpose>` (M-027).
...
- Two-key validation period during rotation (typically 24 hours): both
  ...
  be observable — adds metric `cerniq.kms.rotation.dual_active_seconds`.
...
would need to discover N JWKS endpoints. Single CERNIQ-wide signing key
...
keys for self-hosted distros. Cloud-hosted CERNIQ keeps KMS. Concretely:
