# ADR-0008 — MCP is the control plane for AEGIS-verified tool calls
...
AEGIS already issues short-lived Ed25519-signed agent tokens with policy
...
What's missing is the inverse direction: AEGIS itself exposed *as* an
MCP server, so any agent host (Claude Desktop, Cursor, etc.) can call
`aegis.verify`, `aegis.agents.create`, `aegis.policies.list` directly
through the same MCP transport its other tools use.

This ADR commits to MCP as the canonical control-plane shape — every
AEGIS API surface gets an MCP method binding alongside the REST/HTTP
binding, and every MCP server in the wild is a candidate AEGIS relying
party.

## Decision

1. **Bidirectional MCP integration.** AEGIS ships two MCP packages:
   - `@aegis/mcp-bridge` (existing, this ADR confirms scope) — wraps an
     upstream MCP server's handler so every tool call is gated by an
     AEGIS verify decision. Verification surface: `verify(token, action)`.
   - `@aegis/mcp-server` (NEW, this ADR introduces) — exposes the AEGIS
     management API (identity, policy, verify, audit-read) as MCP tools.
     Distribution: `npx @aegis/mcp-server`. Configures from
     `~/.aegis/credentials.json` or env. Speaks `stdio` and `streamable-http`.
2. **MCP method namespace.** All AEGIS tools live under the `aegis.*`
   namespace. Stable names below — versioned with the API minor, never
   renamed without an ADR:
   - `aegis.verify` — 1:1 with `POST /v1/verify`
   - `aegis.agents.create | get | list | revoke | report`
   - `aegis.policies.create | get | list | revoke`
   - `aegis.audit.search` (read-only, principal-scoped)
   - `aegis.well_known.audit_signing_key` (no auth)
3. **Trust direction inversion.** When AEGIS is *the MCP server*, the
   caller is a host (e.g. Claude Desktop). Authentication is by API key
   in the MCP `initialize` exchange (`clientInfo.metadata.aegisApiKey`),
   per-request audited. When AEGIS is *behind an MCP bridge* (other
   server), the caller is an agent. Authentication is by AEGIS token.
...
### Positive
- One `wrap()` line in any MCP server inherits AEGIS identity, policy,
  audit. Distribution wedge; every popular MCP becomes a customer.
- Claude Desktop / Cursor users provision an `aegis-mcp` server in their
...
- MCP transport reuse: TLS, content-types, error shapes, batching are
  the host's problem. AEGIS only owns the verification logic.
- Audit chain enriched with `mcpServerId` + `mcpMethod` per event —
  lets relying parties slice their own AEGIS audit log per-tool.
...
- Two AEGIS surfaces (REST + MCP) means double the spec maintenance.
  Mitigation: MCP server is generated from the OpenAPI spec at
  `docs/spec/AEGIS_API_SPEC.yaml`; one source of truth.
- DPoP replay prevention (ADR-0010) requires the MCP transport to
  ...
  proofs ride in `params._aegis_dpop` instead. Code path duplication.

### Neutral
- `@aegis/mcp-bridge` README updated to point at this ADR.
- Verifier-rp package gets an MCP-aware example.
- Dashboard adds an MCP-server-discovery view (deferred to M-028).

## Alternatives considered

### Alt A: Custom AEGIS protocol over WebSocket
...
This was the Phase-0 default. Rejected because it forces every AEGIS
...
schema-stitching surface AEGIS doesn't need. The verify call is one
...
`@aegis/mcp-server` with a 12-month sunset. `@aegis/mcp-bridge` stays
