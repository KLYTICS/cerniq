---
title: CERNIQ — Revenue Next Steps
date: 2026-05-07
author: claude-session (status synthesis + 14-day revenue path)
audience: operator (Erwin) + parallel build sessions
classification: INTERNAL
supersedes: none — additive to docs/CERNIQ_MASTER_STATE_2026_05.md and docs/BETA_ONBOARDING_RUNBOOK.md
---

# CERNIQ — Status & path to first revenue
...
`docs/CERNIQ_MASTER_STATE_2026_05.md` (engineering ground truth) +
`docs/decisions/0014-pricing-and-free-trial.md` (pricing locked) +
`docs/finance/CERNIQ_Financial_Model_v1.xlsx` (unit economics) +
...
| ------------------------------------------- | -------------------------------------------------------------- | ----------------------------------------------------------- |
| Phase 1 GA gates G-1…G-4                    | **All closed**                                                 | `docs/CERNIQ_MASTER_STATE_2026_05.md` PART I table          |
...
3. **Auth0 v4 SDK install (M-020-pkg-install)**
   ...
   - `pnpm --filter @cerniq/dashboard add @auth0/nextjs-auth0@^4.0.0`. 15 min + handler wiring.
...
`docs/CERNIQ_MASTER_STATE_2026_05.md` PART V:

> The $500 MRR gate can be hit with **11 DEVELOPER conversions** ($49 ea = $539)
> or **2 TEAM conversions** ($299 ea = $598).

`docs/finance/CERNIQ_Financial_Model_v1.xlsx` Trial_Economics:
...
CERNIQ_API_BASE=$CERNIQ_API_BASE \
CERNIQ_VERIFY_KEY=<verify_only_api_key> \
MIN_TRUST_SCORE=600 \
...
export CERNIQ_E2E_TRIAL_CAP_OVERRIDE=10
pnpm --filter @cerniq/e2e test --testPathPattern='19_customer_journey'
# expect:
...
Sequenced. Day numbers assume operator can spend ~3 hours/day on CERNIQ in
...
- [ ] Land MCP bridge transport glue (`packages/mcp-bridge` — Terminal B in `docs/CERNIQ_MASTER_STATE_2026_05.md` PART VII). This is the only artifact that lets the `wrap(server, { cerniq })` claim in `WEDGE_PROOF.md` actually run. Without it, the wedge is a slide.
...
**The list.** From `docs/CERNIQ_MASTER_STATE_2026_05.md` PART III.5 (the ACP
compatibility vector) plus the wedge logic in `docs/WEDGE_PROOF.md`:

| Persona                                                           | Fit                                                                   | Where to find them                                                                                              | Pitch                                                                                                                                                          |
| ----------------------------------------------------------------- | --------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **MCP server authors handling sensitive APIs**                    | Highest fit. Three lines of code, instant trust score.                | `awesome-mcp-servers` GitHub list (top 30); MCP Discord; Anthropic dev forum                                    | "You wrap a sensitive API in MCP. Your tool callers carry no identity. CERNIQ gives you Ed25519-signed agent identity in 3 lines: `wrap(server, { cerniq })`." |
| **AI fintech / agent-payments startups**                          | Stripe ACP merchants. Identity is the gap ACP explicitly leaves open. | Stripe Sessions 2025 ACP launch attendees; Y Combinator Spring/Summer 2026 batches; OpenAI fintech partner page | "ACP solves the payment leg. CERNIQ is the trust layer that says whether to accept the request before you charge. One verify call."                            |
| **LangChain / CrewAI / AutoGen heavy users**                      | Python SDK shipping; LangChain integration guide already written      | LangChain Discord, CrewAI Slack, AutoGen forum                                                                  | "Your agents don't have identity that survives a session restart. CERNIQ gives them a portable cryptographic identity + scoped policies you can revoke."       |
| **Compliance-driven AI teams (FSI, healthcare, legaltech)**       | Audit chain + JWKS verifier + GDPR-survivable design = SOC2 evidence  | LinkedIn search "AI compliance", "agentic AI" + (CISO OR "head of security")                                    | "Your auditor will eventually ask 'how do you know this LLM made that decision under what authority?' CERNIQ is the answer with cryptographic proof."          |
| **MCP server hosting platforms (Cloudflare AI Gateway, similar)** | Integration partner > customer; one win = many downstream customers   | Direct outbound                                                                                                 | "Bundle CERNIQ as an opt-in trust layer for your hosted MCP servers. Your customers want it; you don't have to build it."                                      |
...
- [ ] Generate them an invitation token (`cerniq admin invite-batch` — see runbook).
...
- [ ] Verify the Stripe webhook actually flipped `Principal.planTier = 'DEVELOPER'` for the paying account (forensic confirmation; operator runs `cerniq principals get <id>`).
...
2. **MCP bridge transport glue not yet shipped.** The `wrap(server, { cerniq })` pitch in `docs/WEDGE_PROOF.md` is the wedge. Without the package working, the highest-conversion ICP (MCP server authors) cannot integrate in 3 lines.
