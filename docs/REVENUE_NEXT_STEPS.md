---
title: OKORO — Revenue Next Steps
date: 2026-05-07
author: claude-session (status synthesis + 14-day revenue path)
audience: operator (Erwin) + parallel build sessions
classification: INTERNAL
supersedes: none — additive to docs/OKORO_MASTER_STATE_2026_05.md and docs/BETA_ONBOARDING_RUNBOOK.md
---

# OKORO — Status & path to first revenue
...
`docs/OKORO_MASTER_STATE_2026_05.md` (engineering ground truth) +
`docs/decisions/0014-pricing-and-free-trial.md` (pricing locked) +
`docs/finance/OKORO_Financial_Model_v1.xlsx` (unit economics) +
...
3. **Auth0 v4 SDK install (M-020-pkg-install)**
   ...
   - `pnpm --filter @okoro/dashboard add @auth0/nextjs-auth0@^4.0.0`. 15 min + handler wiring.
...
`docs/OKORO_MASTER_STATE_2026_05.md` PART V:
...
OKORO_API_BASE=$OKORO_API_BASE \
OKORO_VERIFY_KEY=<verify_only_api_key> \
MIN_TRUST_SCORE=600 \
...
export OKORO_E2E_TRIAL_CAP_OVERRIDE=10
pnpm --filter @okoro/e2e test --testPathPattern='19_customer_journey'
# expect:
...
Sequenced. Day numbers assume operator can spend ~3 hours/day on OKORO in
...
- [ ] Land MCP bridge transport glue (`packages/mcp-bridge` — Terminal B in `docs/OKORO_MASTER_STATE_2026_05.md` PART VII). This is the only artifact that lets the `wrap(server, { okoro })` claim in `WEDGE_PROOF.md` actually run. Without it, the wedge is a slide.
...
- [ ] Generate them an invitation token (`okoro admin invite-batch` — see runbook).
...
- [ ] Verify the Stripe webhook actually flipped `Principal.planTier = 'DEVELOPER'` for the paying account (forensic confirmation; operator runs `okoro principals get <id>`).
...
2. **MCP bridge transport glue not yet shipped.** The `wrap(server, { okoro })` pitch in `docs/WEDGE_PROOF.md` is the wedge. Without the package working, the highest-conversion ICP (MCP server authors) cannot integrate in 3 lines.
