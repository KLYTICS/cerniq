...
# CERNIQ — Capacity plan

> **Purpose.** Quantitative model for sizing every load-bearing
> component of CERNIQ so the SLOs in `docs/SLO.md` are demonstrably
...
CERNIQ has three workload classes with fundamentally different cost
...
### 2.4 Per-RP workload mix (per `CERNIQ_AS_BACKBONE.md`)
...
nightly into `s3://cerniq-perf/pg-stat/<date>.csv` for trend tracking.
...
- CERNIQ workload at +12mo (1.5 K rps reads + 50 rps writes) fits
...
| `audit:dlq`       | < 1 000 (transient by design) | 10 000        | 50 000     | n/a — this _is_ the DLQ |
...
- Failover is _not_ automatic across data-residency boundaries — EU
...
A dedicated Railway environment (`cerniq-capacity`) sized **at the
...
Per `CERNIQ_AS_BACKBONE.md` §3 and §7 roll-out order, each sister
...
adoption owner confirms via `claude-peers msg` to the CERNIQ owner.
...
| Multi-project capacity bumps   | `docs/CERNIQ_AS_BACKBONE.md` §3 + §7                   |
...
2. <!-- assumption: §4.1 NestJS per-pod concurrency 25 is empirically defensible from prior projects but not yet measured on CERNIQ itself. Replace after k6 in apps/api/test/load/. -->
...
5. <!-- assumption: §9.2 GCP KMS EdDSA sign rate 6 000 rps per key version is the documented figure; AWS RSA fallback rate 5 500 rps is per-region shared which makes hot-key fanout the real bottleneck — sized assuming CERNIQ is one of several KMS consumers in the AWS account. -->
