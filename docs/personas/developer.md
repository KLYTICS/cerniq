---
title: CERNIQ for developers
audience: software engineers integrating CERNIQ as a relying party or as an agent operator
last-reviewed: 2026-05-02
---

# CERNIQ for developers — 30-second onboarding
...
- **An agent operator** — you are running an AI agent that calls other
  services. You need an CERNIQ _agent identity_ and a _policy_ binding
  it to scopes. Start with `examples/node-quickstart/`.
- **A relying party (RP)** — you operate a service that _receives_
  agent requests. You need a _verify-only key_ and the
  `@cerniq/verifier-rp` adapter for your stack. Start with
...
2. **Authenticate.** `cerniq login --api-key cerniq_sk_...` (or via
   device-code OAuth if your tenant has Auth0 wired — see
   `docs/decisions/0009-auth0-bridge.md`).
3. **Diagnose.** `cerniq doctor` confirms reachability, credential, JWKS,
   clock skew, plugin discovery. Run it whenever something feels off.
4. **Scaffold.** `cerniq init --industry fintech-payments` (or
