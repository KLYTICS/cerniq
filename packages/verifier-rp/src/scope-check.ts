...
export function checkScopeAndSpend(
  ...
  if (ctx.merchantDomain && claims.allowedDomains.length > 0) {
    const domain = ctx.merchantDomain;
    const ok = claims.allowedDomains.some((d) => domainMatches(d, domain));
    if (!ok) {
      return {
        reason: 'SCOPE_NOT_GRANTED',
  ...
  // refused locally; we don't need to round-trip CERNIQ. (Per-day/per-month
  // ledgering still requires CERNIQ-side enforcement; we don't fabricate it.)
