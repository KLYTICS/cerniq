...
const GENESIS_BYTES = utf8('CERNIQ-AUDIT-GENESIS-v1');
 ...
 *  - genesis row (both prev fields null) → sha256("CERNIQ-AUDIT-GENESIS-v1")
...
export async function verifyChain(
  ...
  const iter: AsyncIterable<AuditEventRow> =
    Symbol.asyncIterator in events ? events : asyncFromIterable(events);
