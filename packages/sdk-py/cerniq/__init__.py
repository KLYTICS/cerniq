"""CERNIQ Python SDK — neutral identity & verification layer for AI agents.

Public surface:

- :class:`AsyncCerniq` — primary async client
- :class:`Cerniq` — sync wrapper (uses ``asyncio.run`` per call)
- :func:`generate_keypair` / :func:`sign_agent_token` / :func:`decode_unsafe`
  / :func:`verify_agent_token` — local crypto helpers
- :class:`CerniqError` and the typed subtree (:class:`ValidationError`,
...
from .client import Cerniq, AsyncCerniq
from .crypto import (
    Keypair,
    SignContext,
    b64u_decode,
