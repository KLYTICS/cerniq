"""Top-level CERNIQ clients — :class:`AsyncCerniq` (primary) + :class:`Cerniq` sync wrapper."""
...
__all__ = ["Cerniq", "AsyncCerniq"]


class AsyncCerniq:
    """Async CERNIQ client. Composition root for ``agents``, ``policies``, ``verify``.
    ...
    Example:
        ...
        >>> async def main() -> None:
        ...     kp = generate_keypair()
        ...     async with AsyncCerniq(api_key="cerniq_sk_example") as cerniq:
...
class Cerniq:
    """Sync facade over :class:`AsyncCerniq`. Each method runs ``asyncio.run``.

    Use this only when you don't already have an event loop. For server-side
    code (FastAPI, ASGI, etc.) prefer :class:`AsyncCerniq` directly.
    ...
    def close(self) -> None:
        """No-op — ``Cerniq`` opens / closes the async client per call."""
        return None
    ...
    def verify(
        ...
        *,
        ...
        async def _go() -> VerifyResult:
            async with AsyncCerniq(**self._kwargs) as a:
...
class _SyncAgents:
    ...
    def __init__(self, parent: Cerniq) -> None:
        self._parent = parent
    ...
    def register(
        ...
        *,
        ...
        async def _go() -> AgentRecord:
            async with AsyncCerniq(**self._parent._kwargs) as a:
