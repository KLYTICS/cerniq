...
CERNIQ is non-custodial: the code in this module runs in the developer's
...
assert _HEADER_B64 == "eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9", (
    "CERNIQ JWT header drifted — must match TS SDK byte-for-byte"
)
...
def generate_keypair() -> Keypair:
    ...
    The private half MUST stay on this host. CERNIQ only ever sees the public
    half, registered via ``cerniq.agents.register``.
...
def decode_unsafe(token: str) -> dict[str, Any] | None:
    ...
    this — call ``cerniq.verify(token, ...)`` (server-side) for that.
...
def verify_agent_token(token: str, public_key_b64u: str) -> dict[str, Any] | None:
    ...
    Production verification should call ``cerniq.verify(...)`` on the server —
