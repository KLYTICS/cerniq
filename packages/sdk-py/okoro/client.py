"""Top-level OKORO clients — :class:`AsyncOkoro` (primary) + :class:`Okoro` sync wrapper."""
...
__all__ = ["Okoro", "AsyncOkoro"]


class AsyncOkoro:
    """Async OKORO client. Composition root for ``agents``, ``policies``, ``verify``.
    ...
    Example:
        ...
        >>> async def main() -> None:
        ...     kp = generate_keypair()
        ...     async with AsyncOkoro(api_key="okoro_sk_example") as okoro:
...
class Okoro:
    """Sync facade over :class:`AsyncOkoro`. Each method runs ``asyncio.run``.

    Use this only when you don't already have an event loop. For server-side
    code (FastAPI, ASGI, etc.) prefer :class:`AsyncOkoro` directly.
    ...
    def close(self) -> None:
        """No-op — ``Okoro`` opens / closes the async client per call."""
        return None
    ...
    def verify(
        ...
        *,
        ...
        async def _go() -> VerifyResult:
            async with AsyncOkoro(**self._kwargs) as a:
...
class _SyncAgents:
    ...
    def __init__(self, parent: Okoro) -> None:
        self._parent = parent
    ...
    def register(
        ...
        *,
        ...
        async def _go() -> AgentRecord:
            async with AsyncOkoro(**self._parent._kwargs) as a:
