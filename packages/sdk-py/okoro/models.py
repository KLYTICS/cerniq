...
class AgentRegistrationRequest(_OkoroModel):
    public_key: str
...
class AgentRegistrationResponse(_OkoroModel):
    agent_id: str
...
class AgentStatusResponse(_OkoroModel):
    agent_id: str
    status: AgentStatus
    trust_score: int
    trust_band: TrustBand
...
class PolicyCreateRequest(_OkoroModel):
    scopes: list[PolicyScope] = Field(min_length=1, max_length=10)
    expires_at: datetime
    label: str | None = None


class PolicyRecord(_OkoroModel):
...
class VerifyRequest(_OkoroModel):
    token: str
...
class VerifySpendRemaining(_OkoroModel):
    today: float | None = None
    this_month: float | None = None


class VerifyResult(_OkoroModel):
    """Result of ``okoro.verify(...)``. ``valid=False`` paired with ``denial_reason``."""
...
class AuditLogResponse(_OkoroModel):
...
class ReportRequest(_OkoroModel):
...
class ReportAccepted(_OkoroModel):
    accepted: bool = True
