"""Async HTTP layer for the OKORO SDK.
...
- per-request timeout
- ``X-OKORO-API-Key`` vs. ``X-OKORO-Verify-Key`` selection per call
"""
...
from .errors import OkoroError, NetworkError, from_status

HttpMethod = Literal["GET", "POST", "PUT", "PATCH", "DELETE"]

_DEFAULT_USER_AGENT: Final[str] = f"okoro-python/{__version__}"


class HttpClient:
    """Async HTTP transport for the OKORO API.
    ...
    def __init__(
        ...
        *,
        ...
        if api_key is None and verify_key is None:
            raise OkoroError(
                "OKORO SDK requires at least one of api_key= or verify_key=.",
                code="AUTH_REQUIRED",
            )
    ...
    async def request(
        ...
        *,
        ...
        Raises:
            OkoroError: A typed subclass on HTTP failure.
        ...
        while True:
            try:
                ...
                if attempt >= self._max_retries:
                    raise NetworkError(
                        f"OKORO request failed: {exc.__class__.__name__}: {exc}",
                        code="NETWORK_ERROR",
                    ) from exc
        ...
        raise NetworkError(
            f"OKORO request retries exhausted ({last_exc!r})",
            code="NETWORK_ERROR",
        )
    ...
    def _build_headers(
        ...
        *,
        ...
        headers: dict[str, str] = {
            ...
            OKORO_HEADER_REQUEST_ID: uuid.uuid4().hex,
        }
        if verify_only:
            ...
            headers[OKORO_HEADER_VERIFY_KEY] = self._verify_key
        else:
            ...
            headers[OKORO_HEADER_API_KEY] = self._api_key
        if extra:
    ...
    @staticmethod
    def _error_from_response(response: httpx.Response, *, payload: Any) -> OkoroError:
        request_id = response.headers.get(OKORO_HEADER_REQUEST_ID)
        message = f"OKORO request failed ({response.status_code})"
        code: str | None = None
        details: Any = None
    ...
    def _catalog_says_retry(catalog_code: str | None, status_code: int) -> bool:
        ...
        # No catalog code present (older server, non-OKORO error). Preserve
