...
from cerniq import AsyncCerniq, PolicyRecord, PolicyScope
...
async def test_list_policies(cerniq: AsyncCerniq, respx_mock: Any) -> None:
    server = [
        {
            "policyId": "pol_a",
            "agentId": "agt_x",
    ...
    policies = await cerniq.policies.list("agt_x")
    assert len(policies) == 2
    assert policies[0].policy_id == "pol_a"
    assert policies[1].status == "expired"


async def test_revoke_policy(cerniq: AsyncCerniq, respx_mock: Any) -> None:
    route = respx_mock.delete("/agents/agt_x/policies/pol_a").respond(204)
    res = await cerniq.policies.revoke("agt_x", "pol_a")
    assert res is None
    assert route.called
