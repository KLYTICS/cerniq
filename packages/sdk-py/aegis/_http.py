"""Async HTTP layer for the AEGIS SDK.
...
- per-request timeout
- ``X-AEGIS-API-Key`` vs. ``X-AEGIS-Verify-Key`` selection per call
"""
...
class HttpClient:
    ...
    def __init__(
        ...
        *,
        ...
        if api_key is None and verify_key is None:
            raise AegisError(
                "AEGIS SDK requires at least one of api_key= or verify_key=.",
                code="AUTH_REQUIRED",
            )
    ...
    async def request(
        ...
        *,
        ...
        Raises:
            AegisError: A typed subclass on HTTP failure.
        ...
        while True:
            try:
                ...
                if attempt >= self._max_retries:
                    raise NetworkError(
                        f"AEGIS request failed: {exc.__class__.__name__}: {exc}",
                        code="NETWORK_ERROR",
                    ) from exc
        ...
        raise NetworkError(
            f"AEGIS request retries exhausted ({last_exc!r})",
            code="NETWORK_ERROR",
        )
    ...
    def _build_headers(
        ...
        *,
        ...
        headers: dict[str, str] = {
            ...
            AEGIS_HEADER_REQUEST_ID: uuid.uuid4().hex,
        }
        if verify_only:
            ...
            headers[AEGIS_HEADER_VERIFY_KEY] = self._verify_key
        else:
    ...
    @staticmethod
    def _error_from_response(response: httpx.Response, *, payload: Any) -> AegisError:
        request_id = response.headers.get(AEGIS_HEADER_REQUEST_ID)
        message = f"AEGIS request failed ({response.status_code})"
        code: str | None = None
        details: Any = None
