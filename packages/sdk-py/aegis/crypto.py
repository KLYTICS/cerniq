...
AEGIS is non-custodial: the code in this module runs in the developer's
...
assert _HEADER_B64 == "eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9", (
    "AEGIS JWT header drifted — must match TS SDK byte-for-byte"
)
...
def generate_keypair() -> Keypair:
    ...
    The private half MUST stay on this host. AEGIS only ever sees the public
    half, registered via ``aegis.agents.register``.
...
def decode_unsafe(token: str) -> dict[str, Any] | None:
    ...
    this — call ``aegis.verify(token, ...)`` (server-side) for that.
...
def verify_agent_token(token: str, public_key_b64u: str) -> dict[str, Any] | None:
    ...
    Production verification should call ``aegis.verify(...)`` on the server —
