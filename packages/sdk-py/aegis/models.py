...
# DenialReason is re-exported from the generated module to keep TS↔PY
# drift structurally impossible. See ``_denial_reason_generated.py``
# header and ``scripts/generate-denial-reason.ts`` for the contract.
# CI gate ``pnpm check:denial-reason-gen`` reds on drift.
#
# Public consumers should keep importing from this module
# (``from aegis.models import DenialReason``) — the generated module
# is private (leading underscore) and may move between minor versions.
from ._denial_reason_generated import DenialReason  # noqa: F401
...
class AgentRegistrationRequest(_AegisModel):
    public_key: str
...
class AgentRegistrationResponse(_AegisModel):
    agent_id: str
...
class AgentStatusResponse(_AegisModel):
    agent_id: str
    status: AgentStatus
    trust_score: int
    trust_band: TrustBand
...
class PolicyCreateRequest(_AegisModel):
    scopes: list[PolicyScope] = Field(min_length=1, max_length=10)
    expires_at: datetime
    label: str | None = None


class PolicyRecord(_AegisModel):
...
class VerifyRequest(_AegisModel):
    token: str
...
class VerifySpendRemaining(_AegisModel):
    today: float | None = None
    this_month: float | None = None


class VerifyResult(_AegisModel):
    """Result of ``aegis.verify(...)``. ``valid=False`` paired with ``denial_reason``."""
...
class AuditLogResponse(_AegisModel):
...
class ReportRequest(_AegisModel):
...
class ReportAccepted(_AegisModel):
    accepted: bool = True
