import { OkoroInternalError, OkoroServiceUnavailableError } from './errors.js';
...
function makeFake(impl: () => Promise<VerifyResult>): FakeOkoro {
...
describe('VerifyGateway: caching', () => {
  it('caches valid results and serves them within TTL', async () => {
    ...
    const gw = new VerifyGateway(fake as unknown as Okoro);
...
describe('VerifyGateway: single-flight', () => {
  ...
  it('fires onCoalesce hook with waiter count', async () => {
    ...
    const gw = new VerifyGateway(fake as unknown as Okoro, { hooks: { onCoalesce } });
    const p1 = gw.verify('tok');
    const p2 = gw.verify('tok');
    resolveUpstream(makeResult());
...
describe('VerifyGateway: circuit breaker', () => {
  it('opens after consecutive failures and fails fast', async () => {
    const fake = makeFake(async () => {
      throw new OkoroInternalError('boom', 500, undefined);
    });
    ...
    await expect(gw.verify('a')).rejects.toBeInstanceOf(OkoroInternalError);
    await expect(gw.verify('b')).rejects.toBeInstanceOf(OkoroInternalError);
    // Breaker is now open — third call must fast-fail with 503.
    await expect(gw.verify('c')).rejects.toBeInstanceOf(OkoroServiceUnavailableError);
  ...
  it('transitions open → half-open → closed on successful probe', async () => {
    ...
    const fake = makeFake(async () => {
      if (mode === 'fail') throw new OkoroInternalError('boom', 500, undefined);
      return makeResult();
    });
    const stateChanges: [string, string][] = [];
    const gw = new VerifyGateway(fake as unknown as Okoro, {
    ...
    now = 500;
    await expect(gw.verify('b')).rejects.toBeInstanceOf(OkoroServiceUnavailableError);
  ...
  it('serve-stale fallback returns expired cache when breaker is open', async () => {
    ...
    now = 5_000;
    mode = 'fail';
    await expect(gw.verify('other')).rejects.toBeInstanceOf(OkoroInternalError);
    expect(gw.state).toBe('open');
...
describe('VerifyGateway: half-open serialization', () => {
  it('only one probe through during half-open; concurrent callers fast-fail', async () => {
    ...
    const fake = makeFake(() => {
      probeCount += 1;
      if (probeCount === 1) throw new OkoroInternalError('boom', 500, undefined);
    ...
    await expect(gw.verify('c')).rejects.toBeInstanceOf(OkoroServiceUnavailableError);
    await expect(gw.verify('d')).rejects.toBeInstanceOf(OkoroServiceUnavailableError);
...
describe('VerifyGateway: TTL jitter', () => {
  it('cached expiresAt is at most server-clamped TTL (jitter only shortens)', async () => {
    const now = 0;
    const fake = makeFake(async () => makeResult({ ttl: 30 }));
    const gw = new VerifyGateway(fake as unknown as Okoro, { now: () => now });
    await gw.verify('tok');
