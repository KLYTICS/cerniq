...
describe('@okoro/sdk crypto', () => {
  ...
  it('signHandshake round-trips through Ed25519 verify (M-003)', async () => {
    ...
    const message = 'okoro-handshake-v1::agt_demo::WoQRdRAVOFhKtnxyNVz6jjW9PQ5gN_DKCsADKHsozqo';
  ...
  it('signHandshake produces different signatures for different challenges (no determinism leak)', async () => {
    const { privateKey } = await generateKeypair();
    const a = await signHandshake(privateKey, 'okoro-handshake-v1::a::AAAA');
    const b = await signHandshake(privateKey, 'okoro-handshake-v1::a::BBBB');
    expect(a).not.toBe(b);
  });
});
