// Public types for @okoro/sdk. Mirror the API contract; intentionally
...
export interface OkoroConfig {
  /** Management API key (`okoro_sk_…`). Required for agent/policy operations. */
  apiKey?: string;
  /** Verify-only key (`okoro_vk_…`). Required for `verify()` calls — relying parties should never see the management key. */
  verifyKey?: string;
  baseUrl?: string;
  timeoutMs?: number;
