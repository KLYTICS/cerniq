...
/**
 * `cerniq kms list <purpose>` — shows the JWKS for a signing purpose.
 * `cerniq kms rotate <purpose>` — provider-side; the CLI initiates a
...
export async function kmsList(opts: { purpose?: string }): Promise<void> {
  ...
  const jwks = (await cerniq.http.get(`/v1/.well-known/audit-signing-key${purposeQs}`)) as {
    keys: {
      kid: string;
      alg: string;
      use: string;
...
export async function kmsRotate(purpose: string): Promise<void> {
  ...
  info(`  2. Update CERNIQ_${purpose.toUpperCase()}_ACTIVE_KID env to the new kid.`);
  info(`  3. Restart the API. Old kid stays in the verify set until natural expiry.`);
  info(`  4. Run \`cerniq audit verify\` once new signed events appear to confirm.`);
