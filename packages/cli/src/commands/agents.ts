...
export async function agentsCreate(opts: { name: string; runtime?: string; printPrivateKey?: boolean }): Promise<void> {
  // Generate a fresh keypair locally — OKORO never sees the private key.
  const priv = ed.utils.randomPrivateKey();
  const pub = await ed.getPublicKeyAsync(priv);
  const okoro = await client();
  const agent = await okoro.agents.create({ name: opts.name, publicKey: b64u(pub) });
  ok(`agent created: ${(agent as { id: string }).id}`);
  emitJson(agent);
  if (opts.printPrivateKey) {
    info('PRIVATE KEY (store securely — OKORO never sees this):');
...
export async function agentsList(opts: { limit?: number; cursor?: string; json?: boolean }): Promise<void> {
  const okoro = await client();
  const result = (await okoro.agents.list({ limit: opts.limit, cursor: opts.cursor })) as { agents: { id: string; name: string; status: string; trustScore: number; trustBand: string }[] };
  if (opts.json) {
    emitJson(result);
    return;
  }
...
export async function agentsGet(id: string, opts: { json?: boolean }): Promise<void> {
  const okoro = await client();
  const agent = await okoro.agents.get(id);
...
export async function agentsRevoke(id: string, opts: { reason?: string }): Promise<void> {
  const okoro = await client();
  await okoro.agents.revoke(id, { reason: opts.reason });
  ok(`agent revoked: ${id}`);
}
