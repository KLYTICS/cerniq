...
// `okoro foo` with no `okoro-foo` on PATH must report not-found, not
...
// TestFind_RejectsTraversal protects against `okoro ../bin/sh` style
...
func TestList_FindsExecutableWithPrefix(t *testing.T) {
	...
	dir := t.TempDir()
	binPath := filepath.Join(dir, "okoro-zoltar")
	...
	if !found {
		t.Fatalf("expected to find okoro-zoltar; got %v", plugins)
	}
}
