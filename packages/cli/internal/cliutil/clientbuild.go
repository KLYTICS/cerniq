...
// ErrNotAuthenticated to drive the "run aegis login" hint.
func ResolveAPIKey(flag string) string {
	if flag != "" {
		return flag
	}
	if env := os.Getenv("AEGIS_API_KEY"); env != "" {
		return env
	}
...
// Used by `aegis verify` when the caller has minted a verify key.
func ResolveVerifyKey(flag string) string {
	if flag != "" {
		return flag
	}
	if env := os.Getenv("AEGIS_VERIFY_KEY"); env != "" {
		return env
	}
...
type BuildOpts struct {
	...
	// is configured. Set false for `aegis doctor` (which can run
...
// Used by `aegis events tail` and any other long-running poll loop —
...
// `aegis agents show` and friends so a missing resource can be
