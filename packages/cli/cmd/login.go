...
func init() {
	loginCmd.Flags().StringP("api-key", "k", "",
		"OKORO API key (use this in CI / non-interactive environments)")
...
var loginCmd = &cobra.Command{
	Use:   "login",
	Short: "Authenticate the CLI against an OKORO API",
	Long: `Authenticate the CLI against an OKORO API.
...
func runLogin(cmd *cobra.Command, args []string) error {
	...
	if apiKey == "" {
		...
		fmt.Fprintf(cmd.ErrOrStderr(),
			"  okoro login --api-key okoro_sk_...\n")
		fmt.Fprintln(cmd.ErrOrStderr(),
			"Or set OKORO_API_KEY in your shell.")
