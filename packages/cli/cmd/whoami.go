	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err != nil {
			...
			if errors.As(err, &apiErr) && apiErr.IsUnauthorized() {
				return errors.New("credential rejected — run `aegis login` to refresh")
			}
			return err
		}
...
// resolveAPIKey applies precedence: --api-key flag > AEGIS_API_KEY env
...
func resolveAPIKey() string {
	...
	if env := getEnv("AEGIS_API_KEY"); env != "" {
		return env
	}
