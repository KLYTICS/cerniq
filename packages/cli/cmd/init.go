...
import (
	...
	"github.com/klytics/aegis/packages/cli/internal/templates"
...
Templates are embedded in the binary so 'aegis init' works offline.
...
Examples:
  aegis init --industry fintech-payments --dir ./checkout-svc
  aegis init -i ai-platform-tool-call -d ./agent-runner
...
func runInit(cmd *cobra.Command, args []string) error {
	...
	if !ok {
		return fmt.Errorf("unknown industry %q — run `aegis init` with no flags to list available templates", industry)
	}
