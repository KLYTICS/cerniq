	...
	checks := []check{
		{
			...
			run: func(_ context.Context) checkResult {
				return ok("aegis " + version.String() + " (" + runtime.GOOS + "/" + runtime.GOARCH + ")")
			},
		},
		{
			name: "config file",
			run: func(_ context.Context) checkResult {
				...
				if err != nil {
					return errCheck("could not resolve config path: "+err.Error(), "set --config or AEGIS_CONFIG")
				}
				return ok("path = " + p)
			},
		},
		{
			...
			run: func(_ context.Context) checkResult {
				if _, err := url.ParseRequestURI(baseURL); err != nil {
					return errCheck("invalid base URL "+baseURL, "set --base-url or run `aegis login --base-url ...`")
				}
				return ok(baseURL)
			},
		},
		{
			...
			run: func(_ context.Context) checkResult {
				if resolveAPIKey() == "" {
					return warn("no credential configured", "run `aegis login --api-key aegis_sk_...`")
				}
			...
			run: func(ctx context.Context) checkResult {
				...
				if err != nil {
					...
					if errors.As(err, &apiErr) && apiErr.IsUnauthorized() {
						return errCheck("credential rejected (HTTP "+fmt.Sprint(apiErr.Status)+")", "run `aegis login` to refresh")
					}
					return errCheck(err.Error(), "")
				}
		...
		},
		{
			...
			run: func(_ context.Context) checkResult {
				...
				if err != nil {
					return warn("git not on PATH", "git is needed for `aegis init` template fetching")
				}
