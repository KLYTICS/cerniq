...
func init() {
	verifyCmd.Flags().String("token", "",
		"AEGIS-issued JWT to verify (required if not passed positionally)")
	...
	verifyCmd.Flags().String("verify-key", "",
		"verify-only key (X-AEGIS-Verify-Key); falls back to env AEGIS_VERIFY_KEY then keychain")
...
var verifyCmd = &cobra.Command{
	...
	Long: `Verify an AEGIS-issued token by round-tripping POST /v1/verify.
...
Examples:
  aegis verify eyJhbGciOi...                              # smoke check
  aegis verify --token "$T" --action commerce.purchase \
               --amount 450 --currency USD --merchant-domain delta.com
  aegis verify "$T" --json | jq '.denialReason'`,
	RunE: runVerify,
}
...
func runVerify(cmd *cobra.Command, args []string) error {
	...
	// Non-zero exit so shell pipelines can branch on `aegis verify ...`.
...
func denialHint(reason *client.DenialReason) string {
	...
	case client.DenialAgentNotFound:
		return "agent ID does not exist; check `aegis agents register` output"
	...
	case client.DenialInvalidSignature:
		return "token signature does not match agent's public key; re-mint via `aegis policy create`"
	...
	case client.DenialPolicyExpired:
		return "policy TTL elapsed; `aegis policy create` again with a longer --ttl"
	...
	case client.DenialAnomalyFlagged:
		return "BATE anomaly engine flagged this transaction; investigate via `aegis events <agentId>`"
	}
	return ""
}
