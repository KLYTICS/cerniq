...
describe('shadowMode', () => {
  it('live wins over shadow when both flags are set', () => {
    expect(
      shadowMode({ CERNIQ_EDGE_VERIFY_ENABLED: 'true', CERNIQ_EDGE_VERIFY_SHADOW_MODE: 'true' }),
    ).toBe('live');
  });
  it('returns shadow when only shadow flag is set', () => {
    expect(shadowMode({ CERNIQ_EDGE_VERIFY_SHADOW_MODE: 'true' })).toBe('shadow');
  });
...
describe('compareVerifyResponses', () => {
  it('reports agree on identical decision tuples (ignoring timestamps)', () => {
    const r = compareVerifyResponses(baseResponse, {
      ...baseResponse,
...
describe('divergenceHeader', () => {
  ...
  it('emits "diverge:<fields>" on divergence', () => {
    expect(divergenceHeader({ divergent: true, fields: ['valid', 'denialReason'] })).toBe(
