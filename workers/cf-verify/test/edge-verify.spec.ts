...
import type { VerifyRequest } from '@cerniq/types';
...
describe('edgeVerify — ADR-0004 denial precedence sweep', () => {
  ...
  it('decides INVALID_SIGNATURE on malformed token', async () => {
    const r = await edgeVerify(
      { ...baseRequest, token: 'not.a.jwt' } as VerifyRequest,
      makeCache(),
    );
    expect(r.response?.denialReason).toBe('INVALID_SIGNATURE');
  });
  ...
  it('forwards on agent cache miss', async () => {
    ...
    const r = await edgeVerify(
      { ...baseRequest, token: t } as VerifyRequest,
      makeCache({ agent: null, policy: activePolicy() }),
    );
  ...
  it('forwards on policy cache miss', async () => {
    ...
    const r = await edgeVerify(
      { ...baseRequest, token: t } as VerifyRequest,
      makeCache({ agent: activeAgent(k), policy: null }),
    );
    expect(r.outcome).toBe('forward');
  });
  ...
  it('decides AGENT_REVOKED on revoked cached agent', async () => {
    ...
    const r = await edgeVerify(
      { ...baseRequest, token: t } as VerifyRequest,
      makeCache({ agent: activeAgent(k, { status: 'REVOKED' }), policy: activePolicy() }),
    );
    expect(r.outcome).toBe('decided');
  ...
  it('forwards on suspended agent (origin handles nuance)', async () => {
    ...
    const r = await edgeVerify(
      { ...baseRequest, token: t } as VerifyRequest,
      makeCache({ agent: activeAgent(k, { status: 'SUSPENDED' }), policy: activePolicy() }),
    );
    expect(r.outcome).toBe('forward');
  });
  ...
  it('decides POLICY_REVOKED on revoked cached policy', async () => {
    ...
    const r = await edgeVerify(
      { ...baseRequest, token: t } as VerifyRequest,
      makeCache({ agent: activeAgent(k), policy: activePolicy({ status: 'REVOKED' }) }),
    );
  ...
  it('decides POLICY_EXPIRED on expired-by-timestamp policy', async () => {
    ...
    const r = await edgeVerify(
      { ...baseRequest, token: t } as VerifyRequest,
      makeCache({ agent: activeAgent(k), policy: expiredPol }),
    );
  ...
  it('decides INVALID_SIGNATURE on bad signature (wrong key)', async () => {
    ...
    const r = await edgeVerify(
      { ...baseRequest, token: t } as VerifyRequest,
      makeCache({ agent: activeAgent(k2), policy: activePolicy() }),
    );
    expect(r.response?.denialReason).toBe('INVALID_SIGNATURE');
  });
  ...
  it('decides SCOPE_NOT_GRANTED on action outside scope', async () => {
    ...
    const r = await edgeVerify(
      { ...baseRequest, token: t, action: 'data.export' } as VerifyRequest,
      makeCache({ agent: activeAgent(k), policy: activePolicy() }),
    );
      ...
      scopes: [
        {
          category: 'commerce',
          actions: ['commerce.purchase'],
          spendLimit: { amount: '500.00', currency: 'USD', window: 'per_request' },
        },
      ],
    });
    const r = await edgeVerify(
  ...
  it('decides TRUST_SCORE_TOO_LOW on FLAGGED-band agent', async () => {
    ...
    const r = await edgeVerify(
      { ...baseRequest, token: t } as VerifyRequest,
      makeCache({
        agent: activeAgent(k, { trustBand: 'FLAGGED', trustScore: 100 }),
        policy: activePolicy(),
      }),
    );
  ...
  it('respects merchant-domain allow-list when present', async () => {
    ...
    const policy = activePolicy({
      scopes: [
        { category: 'commerce', actions: ['commerce.purchase'], merchantDomains: ['delta.com'] },
      ],
    });
