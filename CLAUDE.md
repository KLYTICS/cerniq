# OKORO - Claude operating contract
...
attestation layer between AI agents and the services they act on. OKORO holds
...
1. Private keys never enter OKORO. Client SDKs may generate and hold private
...
- `/.well-known/pricing.json` is the canonical public pricing mirror; dashboard
  pricing SSR-fetches it through `OKORO_API_BASE_URL` with an explicit build-time
...
3. Run
   `~/.claude/peers/bin/claude-peers claim okoro <module-id> --note "<what you will do>" --ttl 7200`.
...
7. Release with `~/.claude/peers/bin/claude-peers release okoro:<module-id>`.
...
4. Dashboard production and preview environments need `OKORO_API_BASE_URL` so
...
| API unit tests            | `pnpm --filter @okoro/api test -- --passWithNoTests` |
