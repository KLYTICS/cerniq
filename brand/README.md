# OKORO — Brand & Design System v1.0

> **Direction:** Cinematic immersive
> **Ecosystem strategy:** Standalone-but-compatible
> **Owns:** the hexagonal Okoro Shield mark, the aurora gradient, the Verified Light palette
...
| `logos/01-shield.svg` | **Recommended primary mark** — the Okoro Shield | Default everywhere |
...
| `logos/05-verified-mark.svg` | Verification badge motif | Inline "Verified by OKORO" badges |
| `logos/wordmark-primary.svg` | Horizontal lockup with shield + OKORO wordmark | Marketing default |
...
<!-- 2. On <body>, add the okoro class -->
<body class="okoro">

<!-- 3. Use the tokens -->
<button style="background: var(--okoro-gradient-aurora); color: var(--okoro-obsidian);
               padding: 12px 20px; border-radius: var(--okoro-radius-sm);
               font-family: var(--okoro-font-body); font-weight: 600;">
  Verify identity
</button>
```
...
- **Technically literal, occasionally human.** 90% surgical sans. 10% editorial serif italic. The serif is when OKORO exhales.
...
**Phase 1 (this week):**
1. Drop `03_design-tokens.css` into `apps/dashboard`. Apply `class="okoro"` to root.
...
2. **Domain** — `okoro.dev` / `okoro.id` / something else. Affects wordmark length + lockup proportions.
...
*OKORO · Brand v1.0 · 2026-05-05 · Authored for Erwin Kiess-Alfonso*
