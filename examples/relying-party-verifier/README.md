...
**consumes** AEGIS verify results to decide whether to honor an inbound
...
```
   ...
   1. signs a per-request token   →    2. POST /api/checkout
                                              X-AEGIS-Token: <jwt>
                                              { amount, currency, ... }
                                       3. calls aegis.verify(token, ctx)
                                       4. AEGIS API returns valid + scope
...
`AEGIS_VERIFY_KEY` should be a verify-only key (`aegis_vk_…`). Production RPs
must NOT use a full-management key (`aegis_sk_…`); the SDK's verify path will
...
pnpm --filter @aegis/scripts run aegis -- agent register --runtime CUSTOM
pnpm --filter @aegis/scripts run aegis -- policy create --agent <agentId> \
  --scope commerce --max-per-tx 100 --expires-in 30d

# 2. sign a token and call the RP
TOKEN=$(pnpm --filter @aegis/scripts run aegis -- verify \
