# `reconciliation` — AEGIS audit ↔ underlying-system reconciler

Joins two NDJSON streams on `endToEndId` (the AEGIS jti = Stripe
...
|---------------------|------------------------------------------------------------|---------------------------------|
| `matched_settled`   | AEGIS approved + system has a settled record              | Happy path; informational       |
| `approved_missing`  | AEGIS approved, system has NO record                       | **Investigate** — network drop or system never executed |
| `denied_present`    | AEGIS denied, system has a record anyway                   | **Investigate** — gate bypass or attacker |
| `reversed`          | AEGIS approved, system settled, system later reversed      | BATE feedback signal (`fraud_confirmed` or `false_positive`) |
...
```sh
# 1. Pull AEGIS audit log for the window.
curl -fsSL "https://api.aegislabs.io/v1/audit-events/export?since=$START&until=$END" \
...
pnpm tsx src/cli.ts --aegis aegis-export.ndjson --psp psp-charges.ndjson --json \
  > reconciliation-$(date +%Y-%m-%d).json
```

## BATE feedback loop

The `reversed` rows feed back into AEGIS so the trust score learns
...
const report = reconcile(aegisRows, systemRows);

for (const entry of report.entries) {
  if (entry.class !== 'reversed' || !entry.aegis) continue;
  await aegis.report({
    agentId: entry.aegis.agentId,
...
- **No PII in the report.** AEGIS rows reference agent ids and
...
- `packages/audit-verifier/` (verifies the AEGIS-side input is
