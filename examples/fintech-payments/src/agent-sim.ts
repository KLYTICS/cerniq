...
//
// Mints a per-request CERNIQ token client-side and prints it to stdout
...
//   curl -H "X-CERNIQ-Token: $TOKEN" -d '{"amount":49,...}' \
//        http://localhost:3001/api/charge
//
// Why this lives in the example (and not just a shell snippet): it
// demonstrates CERNIQ invariant #1 in practice — the agent's private
// key never leaves the agent. The merchant service receives only the
// signed JWT; CERNIQ only ever held the public key. This file is the
...
//                          falls back to CERNIQ_AGENT_PRIVATE_KEY env, else
...
import { signAgentToken, generateKeypair } from '@cerniq/sdk';
...
function readArgs(argv: string[]): CliArgs {
  ...
  return {
    ...
    ttlSeconds,
    privateKeyB64u: get('--private-key') ?? process.env.CERNIQ_AGENT_PRIVATE_KEY,
...
async function main(): Promise<number> {
  ...
  if (!privateKeyB64u) {
    ...
    // AGENT_NOT_FOUND demo branch (the public key never reached CERNIQ,
    ...
    process.stderr.write(
      'agent-sim: no --private-key or CERNIQ_AGENT_PRIVATE_KEY — generated an ephemeral keypair (verify will deny AGENT_NOT_FOUND)\n',
    );
  }
