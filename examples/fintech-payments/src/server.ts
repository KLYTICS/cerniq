// Minimal payment-authorization service with an OKORO verify gate.
//
// Route: POST /api/charge
//   Header X-OKORO-Token: a JWT signed by an agent (jti, exp, scope, etc.).
//   Body { amount, currency, mcc, merchantDomain, idempotencyKey? }.
//
// On every request:
//   1. Read the token + the per-merchant policy bindings.
//   2. Call okoro.verify({ policyJwt, agentSignature, action, ... }).
//   3. If valid: charge the card (stub) and respond 200.
//   4. If denied: respond 402 + denialReason (RFC 9110 §15.5.2).
//
// OKORO does NOT see card data. Card processing is your PSP's job.
// OKORO gates *agent authorization* — who, scoped to what, with what
...
app.post('/api/charge', async (req: Request, res: Response) => {
  ...
  if (!token) {
    return res.status(400).json({ error: 'missing X-OKORO-Token header' });
  }
  ...
  // OKORO verifies the agent's signed policy permits that exact shape.
  ...
  // The merchant's own business logic. OKORO does not call the PSP.
...
// Worldpay, Lithic, etc.) — OKORO has no opinion on which one. The
// OKORO verify gate runs upstream of this call.
