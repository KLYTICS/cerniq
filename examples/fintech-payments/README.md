# `fintech-payments` — OKORO verify gate for a payment authorization flow

A minimal but realistic payment-authorization service that places an
OKORO verify call between an inbound agent request and the actual
charge. The shape mirrors how Stripe-style merchants integrate today,
just with OKORO as the cryptographic gate underneath the merchant's
own business logic.
...
OKORO answers all three in a single `verify` call — and writes a
tamper-evident audit row regardless of the outcome. The payment
service stays focused on the payment domain; OKORO handles identity,
...
```
   ...
   1. signs a per-tx token   →      2. POST /api/charge
                                          X-OKORO-Token: <jwt>
                                          { amount, currency, mcc, ... }
                                    3. okoro.verify(token, ctx)
                                    4. OKORO returns valid + scope
                                       + trust + denialReason
...
okoro init --industry fintech-payments  # if you haven't yet
okoro agents register --runtime CUSTOM --name "checkout-bot-v1"
okoro policy create --agent <agentId> --scope commerce --max-per-tx 500 \
  --domain acme-checkout.com --expires-in 30d

# sign a request token (the SDK does this client-side; the agent
# never shares its private key with OKORO or with your service)
...
the single best way to understand what OKORO will refuse and how to
surface each refusal to a human operator. See
`docs/okoro-denial-mapping.md` (template in
`docs/OKORO_AS_BACKBONE.md` § 5) for the user-facing translation
table.

## Production checklist

- [ ] `OKORO_VERIFY_KEY` is a verify-only key (`okoro_vk_…`), not a
...
- [ ] Idempotency: the `jti` in every token must be unique per request.
      OKORO's replay cache enforces this server-side, but a duplicate
...
- No mock card processor. The `chargeCard()` helper is a pure stub
  ...
  PSP. OKORO is the agent-authorization gate, not the payment gate.
- No retry / circuit-breaker around the OKORO verify call. Production
  RPs should add one (matches `@okoro/sdk` retry semantics). Kept out
  of the example to keep the gate logic readable.
- No Spanish-localized denial messages. Adopt the table in
  `docs/OKORO_AS_BACKBONE.md` § 5 verbatim if your verticals span
  PR / LATAM.
