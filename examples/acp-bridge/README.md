# `acp-bridge` — CERNIQ + Stripe Agentic Commerce Protocol (ACP)
...
```
  ...
  CERNIQ-signed agent token            →  is the agent the one the cardholder
                                          authorized, is the policy live, has
                                          its behavior been trustworthy?
```

ACP solved the payment leg. CERNIQ solves the agent leg. Both questions
...
A merchant that adopts ACP without CERNIQ has no way to distinguish:
...
prove which agent presented the token. That's the slot CERNIQ fills.
...
```
  ...
  │  signs CERNIQ token client-   │───►│   { paymentToken, cerniqToken,   │
  │   side; pkey never leaves    │    │     amount, currency, … }       │
  │                              │    │                                 │
  └──────────────────────────────┘    │  1. cerniq.verify(cerniqToken)    │
                                      │      → identity + policy + trust│
                                      │  2. spt.verify(paymentToken)    │
                                      │      → amount / currency / exp  │
                                      │  3. stripe.charges.create(...)  │
                                      │      idempotency-key = cerniqJti │
                                      └─────────────────────────────────┘
```

Both gates are **fail-closed**. CERNIQ is checked first because it's
cheaper and identity errors dominate denials in agent traffic. If CERNIQ
...
cerniq agents register --runtime CUSTOM --label "checkout-bot-v1" --generate-keypair > agent.json
AGENT_ID=$(jq -r .agentId agent.json)
POLICY_ID=$(cerniq policy create --agent "$AGENT_ID" \
              --scope commerce \
              --max-per-tx 100.00 \
...
CERNIQ_AGENT_ID=$AGENT_ID \
CERNIQ_POLICY_ID=$POLICY_ID \
CERNIQ_AGENT_PRIVATE_KEY=$PKEY \
...
The example uses the **CERNIQ jti** as the Stripe `idempotency-key`.
This single unique key per request closes the "CERNIQ approved, Stripe
charged twice on retry" race. The agent never sees the idempotency-
key — it's derived server-side from the token jti.

If your retry layer mints a fresh jti per attempt (which CERNIQ's replay
cache requires), you also get a fresh Stripe idempotency-key. If you
retry with the same jti, CERNIQ denies as `INVALID_SIGNATURE` (replay)
and Stripe returns the cached prior result.

## Cross-check — payerUserId ↔ principalId

The mock SPT carries `payerUserId`. CERNIQ returns the agent's
...
```ts
if (sptVerdict.payerUserId !== mapPrincipalToUser(cerniqVerdict.principalId)) {
  ...
  // session-leak or a malicious agent. Report back to CERNIQ so the
  // BATE engine learns from it.
  await cerniq.report({
    agentId: cerniqVerdict.agentId,
    eventType: 'suspicious_behavior',
    severity: 'critical',
    description: 'SPT payer mismatch with CERNIQ principal',
  });
...
- [ ] Wire `chargeCard` to `stripe.charges.create()` (or
      `paymentIntents.create`) with the CERNIQ jti as
      `idempotency-key`.
- [ ] Subscribe to `cerniq.agent.revoked` and `cerniq.agent.policy_expired`
      webhooks so the merchant stops accepting an agent within seconds
      of revocation, not at SPT TTL expiry.
- [ ] Cross-link audit trails: every successful charge stores CERNIQ
...
- [ ] Replay defence: if the agent's `cerniqToken` jti is reused, CERNIQ
...
- [ ] Trust score floor for **new** agents: cold-start policy
      ...
      the first 7 days even if their CERNIQ policy permits the action.
...
- **3DS / SCA challenge handoff.** A real production gate uses CERNIQ's
...
- **Settlement reconciliation.** Periodic job that joins CERNIQ audit
...
- `packages/verifier-rp/` (drop-in offline verifier the merchant API
  could swap in instead of round-tripping `cerniq.verify()`)
