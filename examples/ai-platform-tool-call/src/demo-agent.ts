// Demo "agent" — registers a keypair with OKORO, issues a scoped policy,
// signs a token, calls the tool server. End-to-end.
//
// In a real deployment, the AGENT side runs in your customer's environment.
// They generate the keypair LOCALLY (OKORO never sees the private key per
// ADR-0002), register the public key with OKORO, issue policy tokens for
...
async function main(): Promise<void> {
  ...
  const { signAgentToken } = await import('@okoro/sdk/dist/crypto.js');
  ...
  //    extracts it and calls okoro.verify. We can simulate that here:
