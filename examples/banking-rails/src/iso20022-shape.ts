...
// authorize and OKORO would gate. The bank-side adapter (out of scope
...
/** A programmable-banking action. OKORO gates "is the agent authorized
...
export interface PaymentInstruction {
   ...
   *  on the camt.054 settlement notification. We use OKORO jti for
   ...
   *  by the bank-adapter based on cutoffs / cost. OKORO's policy may
...
export interface InstructResponse {
  allowed: boolean;
  endToEndId: string;
  /** OKORO audit event id — lets a regulator independently verify
   *  the agent-leg of the audit trail. */
  auditEventId?: string;
  /** Bank id when accepted. */
  bankId?: string;
  /** When denied at OKORO: which reason. */
