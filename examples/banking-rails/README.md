# `banking-rails` — CERNIQ verify gate for programmable banking

Layers CERNIQ over a bank adapter (Modern Treasury, Increase, Mercury,
...
Agent-driven money movement is the highest-stakes CERNIQ surface.
...
1. **Identity** — Ed25519 keypair generated client-side, only public
   key in CERNIQ. (CLAUDE.md invariant 1.)
...
4. **Audit** — every attempt (allowed, CERNIQ-denied, bank-rejected)
...
```
  ...
  │                              │    │                                  │
  │  CERNIQ-signed token bound to │    │  POST /api/instruct              │
  │  creditor BIC + amount       │───►│   { cerniqToken, instruction }    │
  │                              │    │                                  │
  └──────────────────────────────┘    │  1. cerniq.verify(token)          │
...
cerniq agents register --runtime CUSTOM --label "treasury-bot-v1" --generate-keypair > agent.json
...
The adapter's job is shape-mapping; CERNIQ's job is identity / policy /
...
- CERNIQ audit events (`POST /v1/audit-events`) by `endToEndId`
...
- [ ] Subscribe to `cerniq.agent.revoked` and `cerniq.agent.policy_expired`
      ...
      it's revoked.
- [ ] Run a per-rail latency budget alarm. If `cerniq.verify` p99 ever
      exceeds the rail's submission window (RTP / FedNow are 5s end-to-
      end), CERNIQ becomes the bottleneck. p99 SLA is 200ms (see
      `docs/CAPACITY_PLAN.md`).
- [ ] Pair CERNIQ audit events to bank confirmations in your data
      warehouse. SOC2 auditors will ask for both halves of the trail.
- [ ] BATE signal feedback: when the bank rejects, report the
      rejection back to CERNIQ via `/v1/agents/:id/report` so the
      trust score reflects bank-side issues that CERNIQ can't directly
...
- **AML / sanctions screening.** That's a layer ABOVE CERNIQ — your
  compliance stack screens the counterparty before the agent even
  attempts the action. CERNIQ gates _agent authorization_, not the
