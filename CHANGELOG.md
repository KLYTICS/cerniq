...
All notable changes to the AEGIS CLI are documented here. The format
...
- `aegis agents register` — full wire to `POST /v1/agents/register` with
  optional local `--generate-keypair` (Ed25519, base64url; private key
  printed once and never sent to AEGIS, honoring CLAUDE.md invariant 1)
  or `--public-key <path|->` for an existing key.
- `aegis agents show / status / revoke` — wired to the corresponding
  spec endpoints. `status` is the public read (no API key required).
- `aegis policy create / list / revoke / inspect` — full policy surface
...
- `aegis verify` — POST /v1/verify with `--token`, `--action`, `--amount`,
...
- `aegis events list / tail / export` — cursor-paginated read view of
...
- `aegis report` — submit a behavioral signal to BATE with `--type`,
...
- `--verify-key` flag on `aegis verify` plus `KeyVerifyKey` keychain
...
- `aegis login` device-code OAuth flow (OD-009) remains stubbed pending
  the peer's auth0 module exposing `/v1/idp/auth0/device/{authorize,token}`
  endpoints. The `--api-key` fallback is fully wired.
- `aegis listen` (webhook subscription tail) is deferred — the server-side
  webhook subscription endpoint is not in `AEGIS_API_SPEC.yaml` today.
