...
| OD-017 | Audit-chain integrity workflow secrets | Source: `.github/workflows/audit-chain-integrity.yml` env block. Daily cron run on `main` has been failing 2026-05-13 onward because the GitHub Environment `staging` has no values for `AUDIT_DB_READONLY_URL`, `AEGIS_API_BASE`, and `SLACK_INCIDENT_WEBHOOK`. The workflow now fails fast with an operator-actionable error (see fix/audit-chain-defensive-secrets PR), but the cron will stay red until the secrets are populated. | Populate the three secrets under **Settings → Environments → staging → Secrets** (and again under `production` when ready). `AUDIT_DB_READONLY_URL` must point to a read-only Postgres replica role (the verifier doesn't write; defense-in-depth means no write grant). `AEGIS_API_BASE` is the public https URL of the staging API. `SLACK_INCIDENT_WEBHOOK` is the incoming-webhook URL for incident channel — optional; workflow runs without it but loses the notification ping. | operator | before next monthly SOC2 evidence pull | OPEN   |
