...
| OD-017 | Legacy `kid-genesis-v1` JWKS handling | Source: M-038 (PR #9 / `docs/SESSION_HANDOFF.md` 2026-05-12). Pre-M-038 AuditEvent rows (~600 in staging, unknown in prod) were stamped with the placeholder kid `kid-genesis-v1` because of the env-fallback bug. M-038 fixes new rows. The historical rows are immutable per CLAUDE.md inv. #3. Until JWKS exposes a public key under `kid-genesis-v1`, `@aegis/audit-verifier` reports "kid not in JWKS — cannot verify" for every pre-M-038 row, breaking SOC2 evidence pulls for that window. | **(a) Publish a second JWK aliased to `kid-genesis-v1`** with the same `x` bytes as the current `computeKid(rawPublicKey)` JWK. Adds one entry to `getJwks()` in `apps/api/src/modules/wellknown/wellknown.service.ts`, gated on `WELLKNOWN_PUBLISH_LEGACY_KID=1` so the alias auto-removes once retention ages out the rows. Off-the-shelf verifier resolves the legacy kid → same public key → signatures verify → chain still walks. **Fallback (b)**: accept "individually-signed only, not chain-walkable" for the pre-M-038 window. Per-event signatures still verify, but `verifyChain()` reports a chain-link break at the cut-over row. SOC2 evidence for pre-cut dates falls back to per-event signature + database backup attestation. | operator | before first SOC2 audit covering pre-M-038 dates | OPEN   |
