...
/**
 * CERNIQ — `pnpm seed:demo` — believable, multi-principal demo dataset.
 *
 * A fresh CERNIQ install boots to an empty database. The dashboard is blank,
 ...
 *
 *   - multi-principal isolation (`maria@cerniq-demo.test`, `roberto@cerniq-demo.test`)
 ...
 * `@cerniq-demo.test` and recreate from scratch. Production rows are untouched.
 *
 * Flags:
...
export const DEMO_EMAIL_SUFFIX = '@cerniq-demo.test' as const;
const API_KEY_PREFIX = 'cerniq_sk_' as const;
...
export const PERSONAS: ReadonlyArray<PersonaSpec> = [
  {
    ...
    webhookUrl: 'https://hooks.cerniq-demo.test/maria',
    auditEventCount: 10,
  },
  {
    ...
    webhookUrl: 'https://hooks.cerniq-demo.test/roberto',
    auditEventCount: 50,
  },
] as const;
...
/**
 * Mint an `cerniq_sk_<random>` API key. Same shape as
 ...
 */
export function planAgentEvents(
  args: PlanAuditEventsArgs,
): Omit<PlannedAuditEvent, 'cerniqSignature' | 'prevEventId'>[] {
  const span = 14 * 24 * 60 * 60 * 1000; // 14 days
  const out: Omit<PlannedAuditEvent, 'cerniqSignature' | 'prevEventId'>[] = [];
  for (let i = 0; i < args.count; i++) {
    ...
    out.push({
      id: `evt_${toB64Url(randomBytes(15))
        .replace(/[^a-zA-Z0-9]/g, '')
        .slice(0, 22)}`,
...
interface SignAuditChainArgs {
  events: Omit<PlannedAuditEvent, 'cerniqSignature' | 'prevEventId'>[];
  /**
   * CERNIQ audit-signing private key (32-byte Ed25519 seed). One key per
...
function rebuildPayload(
  row: Omit<PlannedAuditEvent, 'cerniqSignature' | 'prevEventId'>,
...
export async function signAuditChain(args: SignAuditChainArgs): Promise<PlannedAuditEvent[]> {
  ...
  for (const ev of args.events) {
    ...
    out.push({ ...ev, cerniqSignature: sigB64Url, prevEventId: part.prevId });
...
export async function verifySignedChain(
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    const canonical = enc.encode(canonicalize(rebuildPayload(ev)));
    const message = Buffer.concat([prev, canonical]);
    const sig = Buffer.from(ev.cerniqSignature, 'base64url');
    let ok = false;
    try {
    ...
    partitions.set(partKey, { prevId: ev.id, prevSig: ev.cerniqSignature });
  }
  return { ok: true };
}
...
export function planBateForAgent(args: PlanBateForAgentArgs): PlannedBateSignal[] {
  ...
  for (let i = 0; i < args.cleanCount; i++) {
    out.push({
      ...
      severity: 'LOW',
      source: 'cerniq-demo-seed',
      occurredAt: new Date(
        args.endTime.getTime() - span + (i / Math.max(1, args.cleanCount)) * span,
      ),
      scoreDelta: +4,
...
export async function runSeedDemo(
  ...
  const allPlannedEvents: Omit<PlannedAuditEvent, 'cerniqSignature' | 'prevEventId'>[] = [];
    ...
    for (const ev of signed) {
      await prisma.auditEvent.create({
        data: {
          ...
          cerniqSignature: ev.cerniqSignature,
...
function emitHumanSummary(records: SeededPrincipalRecord[], outcome: SeedDemoOutcome): void {
  ...
  lines.push('');
  lines.push('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  lines.push(' CERNIQ demo seed — STORE NOW — never shown again');
  lines.push('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  for (const rec of records) {
    lines.push('');
    lines.push(`Principal: ${rec.email}  (id=${rec.principalId})`);
    lines.push(`  API key (use as x-cerniq-api-key):`);
  ...
  if (maria && mariaAgent) {
    ...
    lines.push(`    -H "x-cerniq-api-key: ${maria.apiKey}" \\`);
...
async function main(): Promise<void> {
  ...
  program
    .name('seed-demo')
    .description(
      'Idempotent demo seed for CERNIQ — multi-principal, multi-agent, valid audit chain.',
    )
  ...
  try {
    ...
    cipher = new Ctor({
      webhookSecretDekB64:
        env.CERNIQ_WEBHOOK_SECRET_DEK_B64 ?? Buffer.from(randomBytes(32)).toString('base64'),
...
const isMain =
  argv[1] !== undefined && (argv[1].endsWith('seed-demo.ts') || argv[1].endsWith('seed-demo.js'));
if (isMain) {
  main().catch((err) => {
