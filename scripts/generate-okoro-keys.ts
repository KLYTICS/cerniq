...
/**
 * OKORO — generate the internal Ed25519 signing keypair used to sign audit
 * records and the audit hash chain.
 *
 * Outputs (any combination, controlled by --format):
 *   - env  : .env-style file with OKORO_SIGNING_PRIVATE_KEY / _PUBLIC_KEY (base64url)
 ...
 *
 *   pnpm --filter @okoro/scripts keys -- --format both --out ./.local/keys
 */
...
export function toEnvFile(material: KeypairMaterial, generatedAt: string): string {
  return [
    `# OKORO internal Ed25519 audit-signing keypair`,
    `# generated: ${generatedAt}`,
    `# kid:       ${material.kid}`,
    `# DO NOT commit. Source via direnv or paste into your secret manager.`,
    ``,
    `OKORO_SIGNING_PRIVATE_KEY=${material.privateKeyB64Url}`,
    `OKORO_SIGNING_PUBLIC_KEY=${material.publicKeyB64Url}`,
    `OKORO_SIGNING_KID=${material.kid}`,
    ``,
  ].join('\n');
}
...
export async function writeArtifacts(
  ...
  const envPath = join(opts.outDir, 'okoro-signing.env');
  const jwkPath = join(opts.outDir, 'okoro-signing.jwk.json');
...
function parseCli(args: string[]): CliOpts {
  const program = new Command()
    .name('generate-okoro-keys')
    .description('Mint OKORO internal Ed25519 audit-signing keypair.')
