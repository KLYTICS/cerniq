...
# aegis-named DB objects to okoro. Idempotent: skips if migration already exists.
set -euo pipefail
...
--    NOTE: every policy that previously called aegis_current_principal() /
--    aegis_rls_bypass_active() needs to be recreated against the new names.
