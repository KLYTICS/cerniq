...
- Replaces `OKORO` → `OKORO`, `Okoro` → `Okoro`, `okoro` → `okoro` in every
  tracked text file except `pnpm-lock.yaml` and `apps/api/prisma/migrations/`.
- Renames every file and directory whose path contains `okoro` (any case).
- Emits a new Prisma migration that ALTERs the `okoro_app` / `okoro_owner`
  roles, `okoro_current_principal` / `okoro_rls_bypass_active` functions, and
  the `okoro.*` GUC namespace to their `okoro` equivalents.
...
- Optionally renames the root folder `OKORO` → `OKORO` at the end.

## What it does NOT do

- Touch existing migration files (CLAUDE.md immutability contract). Adds a new
  migration instead.
- Touch other worktrees (`/private/tmp/okoro-*`, `/Users/money/Desktop/OKORO-*`,
...
```bash
./scripts/rename-okoro-to-okoro/run.sh
```
...
| 2    | `10-rename-checkout.sh`            | Renames text + paths in the current checkout (`okoro`→`okoro`, `okorolabs.io`→`okorolabs.io`) |
...
| 6    | `30-rename-folder.sh`              | Renames `OKORO` → `OKORO` (must be last)                                                      |

**Why two passes for the apex domain?** `10-rename-checkout.sh` is a
case-preserving `okoro`→`okoro` substitution; it mechanically converts
`okorolabs.io` to `okorolabs.io`. The operator's chosen apex is
...
2. Close `docs/finance/OKORO_Financial_Model_v1.xlsx` in Excel (it currently
...
4. Make a backup. The script commits per branch and there's no undo button:
   ```bash
   cd ..
   cp -R OKORO OKORO.backup-2026-05-21
   ```
...
- **CI may go red.** Any pinned GitHub Actions referencing `okoro-` artifacts
  or images will need their pins updated. Search for `okoro` after running:
  ```bash
  git grep -i okoro
  ```
...
- **Migrations applied to existing environments.** The new migration ALTERs
  ...
  okoro-named objects, this migration will rename them in place. If your
...
```bash
# list rename commits
git log --all --grep='chore: rename okoro to okoro' --oneline

# undo on current branch
git reset --hard HEAD~1

# undo across all branches that were renamed
./scripts/rename-okoro-to-okoro/rollback.sh    # (not generated; see 20-rename-all-branches.sh log)
```

If you took the `OKORO.backup-2026-05-21` snapshot, you can also just
`rm -rf OKORO && mv OKORO.backup-2026-05-21 OKORO`.
