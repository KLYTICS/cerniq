 ...
 *
 * Flags (global):
 *   --api-base   (default: $OKORO_API_BASE or http://localhost:4000)
 *   --api-key    (default: $OKORO_API_KEY or read from ./.okororc.json)
 *   --json       (raw JSON, default human-readable)
 *
 * Local state: ./.okororc.json (cwd-relative — gitignored project state)
...
const RC_PATH = resolve('./.okororc.json');
const KEYS_DIR = resolve('./.local/keys');
const DEFAULT_BASE = env.OKORO_API_BASE ?? 'http://localhost:4000';
...
async function http<T = unknown>(
  ...
  const headers: Record<string, string> = {
    ...
    'user-agent': 'okoro-cli/0.1',
  };
  if (opts.apiKey) headers['x-okoro-api-key'] = opts.apiKey;
...
async function cmdAgentList(g: GlobalOpts): Promise<void> {
  ...
  if (ids.length === 0) {
    emit(g, () => 'no agents in ./.okororc.json (run `okoro agent register`)', { agents: [] });
    return;
  }
...
async function loadAgentPrivateKey(agentId: string): Promise<Uint8Array> {
  ...
  try {
    ...
    throw new CliError(
      `private key not found at ${path} — register the agent first with \`okoro agent register\``,
      2,
    );
  }
...
export function buildCli(): Command {
  ...
  program
    .name('okoro')
    .description('OKORO operator CLI')
    .version('0.1.0')
    .addOption(new Option('--api-base <url>', 'OKORO API base URL').default(DEFAULT_BASE))
    .addOption(new Option('--api-key <key>', 'OKORO API key (mgmt)').env('OKORO_API_KEY'))
