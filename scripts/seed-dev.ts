...
/**
 * OKORO — idempotent dev seed.
 *
 * Creates (or no-ops on) the minimum row set a developer needs to hit the
 * API end-to-end:
 *
 *   1. Principal             — email "dev@okoro.local", planTier=DEVELOPER
 *   2. ApiKey                — full-scope key prefixed "okoro_sk_"  (per ref impl)
 ...
 *                                ./.okoro-dev-key.txt             (operator-facing)
 ...
 *
 *   pnpm --filter @okoro/scripts seed -- --fast
 *   NODE_ENV=development pnpm --filter @okoro/scripts seed -- --reset
 */
...
const AGENT_PRIVATE_KEY_OP_PATH = resolve('./.okoro-dev-key.txt');
...
/** `okoro_sk_<22-char b64url of 16 random bytes>` — matches ref impl shape. */
 ...
 * dev-only convenience; real policies are signed by the OKORO audit key.
 */
...
async function writeAgentPrivateKey(privateKeyB64Url: string): Promise<void> {
  ...
  // NOTE: .gitignore must exclude .okoro-dev-key.txt (verify post-seed).
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (!policy) {
      ...
      // policies are signed by the OKORO audit key; this is a dev convenience.
      ...
      // signed under the OKORO audit key, not the agent key. Documented above.
...
${issuedApiKey ? `  API key (use as x-okoro-api-key): ${issuedApiKey}\n` : '  API key: already issued (use --reset to rotate)\n'}`,
    );
  } finally {
