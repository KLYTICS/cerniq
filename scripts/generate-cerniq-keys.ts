...
/**
 * CERNIQ — generate the internal Ed25519 signing keypair used to sign audit
 * records and the audit hash chain.
 *
 * Outputs (any combination, controlled by --format):
 *   - env  : .env-style file with CERNIQ_SIGNING_PRIVATE_KEY / _PUBLIC_KEY (base64url)
 ...
 *
 *   pnpm --filter @cerniq/scripts keys -- --format both --out ./.local/keys
 */
...
export function toEnvFile(material: KeypairMaterial, generatedAt: string): string {
  return [
    `# CERNIQ internal Ed25519 audit-signing keypair`,
    `# generated: ${generatedAt}`,
    `# kid:       ${material.kid}`,
    `# DO NOT commit. Source via direnv or paste into your secret manager.`,
    ``,
    `CERNIQ_SIGNING_PRIVATE_KEY=${material.privateKeyB64Url}`,
    `CERNIQ_SIGNING_PUBLIC_KEY=${material.publicKeyB64Url}`,
    `CERNIQ_SIGNING_KID=${material.kid}`,
    ``,
  ].join('\n');
}
...
export async function writeArtifacts(
  ...
  const envPath = join(opts.outDir, 'cerniq-signing.env');
  const jwkPath = join(opts.outDir, 'cerniq-signing.jwk.json');
  ...
  if (!opts.force) {
    for (const t of targets) {
      ...
      if (await fileExists(t.path)) {
        throw new Error(`refusing to overwrite ${t.path} — re-run with --force to replace.`);
      }
    }
  }
...
function parseCli(args: string[]): CliOpts {
  const program = new Command()
    .name('generate-cerniq-keys')
    .description('Mint CERNIQ internal Ed25519 audit-signing keypair.')
