...
/**
 * CERNIQ — one-shot bulk-encryptor for legacy plaintext webhook secrets.
 *
 ...
 *
 *   pnpm --filter @cerniq/scripts encrypt-webhook-secrets -- --dry-run
 *   pnpm --filter @cerniq/scripts encrypt-webhook-secrets
 *
 * Exit codes:
 ...
 *   2  CLI usage error
 *   3  config error (missing CERNIQ_WEBHOOK_SECRET_DEK_B64)
 *
...
// the cipher itself but on every transitive file (cerniq-error,
...
async function main(): Promise<void> {
  ...
  program
    ...
    .addOption(new Option('--principal-id <id>', 'restrict to one tenant (incident-response use)'));
  ...
  const dekB64 = env.CERNIQ_WEBHOOK_SECRET_DEK_B64;
  if (!dekB64 || dekB64.length === 0) {
    stderr.write(
      'CERNIQ_WEBHOOK_SECRET_DEK_B64 is required. Generate with:\n' +
