...
import {
  ...
  toJwkPublic,
  writeArtifacts,
} from './generate-aegis-keys.js';

describe('generate-aegis-keys', () => {
  const dirs: string[] = [];
  afterEach(async () => {
    while (dirs.length) {
      const d = dirs.pop();
  ...
  async function tmpDir(): Promise<string> {
    const d = await mkdtemp(join(tmpdir(), 'aegis-keys-'));
    dirs.push(d);
    return d;
  }
  ...
  it('toEnvFile emits the three documented env vars', async () => {
    ...
    expect(env).toContain(`AEGIS_SIGNING_PRIVATE_KEY=${m.privateKeyB64Url}`);
    expect(env).toContain(`AEGIS_SIGNING_PUBLIC_KEY=${m.publicKeyB64Url}`);
    expect(env).toContain(`AEGIS_SIGNING_KID=${m.kid}`);
  });
  ...
  it('writeArtifacts overwrites when --force is set', async () => {
    ...
    const jwk = JSON.parse(
      await readFile(join(out, 'aegis-signing.jwk.json'), 'utf8'),
    ) as { kid: string };
