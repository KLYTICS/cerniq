 ...
 *
 * Flags (global):
 *   --api-base   (default: $CERNIQ_API_BASE or http://localhost:4000)
 *   --api-key    (default: $CERNIQ_API_KEY or read from ./.cerniqrc.json)
 *   --json       (raw JSON, default human-readable)
 *
 * Local state: ./.cerniqrc.json (cwd-relative — gitignored project state)
...
const RC_PATH = resolve('./.cerniqrc.json');
const KEYS_DIR = resolve('./.local/keys');
const DEFAULT_BASE = env.CERNIQ_API_BASE ?? 'http://localhost:4000';
...
async function http<T = unknown>(
  ...
  const headers: Record<string, string> = {
    ...
    'user-agent': 'cerniq-cli/0.1',
  };
  if (opts.apiKey) headers['x-cerniq-api-key'] = opts.apiKey;
  ...
  try {
    ...
    throw new CliError(`network error reaching ${url.host}: ${(err as Error).message}`, 4);
  }
...
async function cmdAgentList(g: GlobalOpts): Promise<void> {
  ...
  if (ids.length === 0) {
    emit(g, () => 'no agents in ./.cerniqrc.json (run `cerniq agent register`)', { agents: [] });
    return;
  }
  ...
  emit(g, () => agents.map((a) => JSON.stringify(a)).join('\n'), { agents });
}
...
async function cmdPolicyRevoke(g: GlobalOpts, agentId: string, policyId: string): Promise<void> {
  await http(
    { baseUrl: g.apiBase, apiKey: g.apiKey },
    'DELETE',
...
async function loadAgentPrivateKey(agentId: string): Promise<Uint8Array> {
  ...
  try {
    ...
    throw new CliError(
      `private key not found at ${path} — register the agent first with \`cerniq agent register\``,
      2,
    );
  }
...
export function buildCli(): Command {
  ...
  program
    .name('cerniq')
    .description('CERNIQ operator CLI')
    .version('0.1.0')
    .addOption(new Option('--api-base <url>', 'CERNIQ API base URL').default(DEFAULT_BASE))
    .addOption(new Option('--api-key <key>', 'CERNIQ API key (mgmt)').env('CERNIQ_API_KEY'))
  ...
  agent
    ...
    .requiredOption(
      '--runtime <runtime>',
      'agent runtime: OPENAI|ANTHROPIC|GOOGLE|HUGGINGFACE|CUSTOM',
    )
