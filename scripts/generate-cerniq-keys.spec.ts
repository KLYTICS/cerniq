...
import {
  ...
  toJwkPublic,
  writeArtifacts,
} from './generate-cerniq-keys.js';

describe('generate-cerniq-keys', () => {
  const dirs: string[] = [];
  afterEach(async () => {
    while (dirs.length) {
      const d = dirs.pop();
  ...
  async function tmpDir(): Promise<string> {
    const d = await mkdtemp(join(tmpdir(), 'cerniq-keys-'));
    dirs.push(d);
    return d;
  }
  ...
  it('toEnvFile emits the three documented env vars', async () => {
    ...
    expect(env).toContain(`CERNIQ_SIGNING_PRIVATE_KEY=${m.privateKeyB64Url}`);
    expect(env).toContain(`CERNIQ_SIGNING_PUBLIC_KEY=${m.publicKeyB64Url}`);
    expect(env).toContain(`CERNIQ_SIGNING_KID=${m.kid}`);
  });
  ...
  it('writeArtifacts refuses to overwrite without --force', async () => {
    ...
    await expect(writeArtifacts(m, { outDir: out, format: 'both', force: false })).rejects.toThrow(
  ...
  it('writeArtifacts overwrites when --force is set', async () => {
    ...
    const jwk = JSON.parse(await readFile(join(out, 'cerniq-signing.jwk.json'), 'utf8')) as {
      kid: string;
    };
