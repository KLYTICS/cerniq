...
> `.local/`. The scripts here (and `cerniq-cli`) write private keys and the
> `.cerniqrc.json` operator state to `./.local/keys/` and `./.cerniqrc.json` —
> add `.local/` and `.cerniqrc.json` to `.gitignore` before committing.

| Script                        | Run as                                                 | Purpose                                                                    |
| ----------------------------- | ------------------------------------------------------ | -------------------------------------------------------------------------- |
| `generate-cerniq-keys.ts`     | `pnpm --filter @cerniq/scripts keys`                   | Mint the CERNIQ internal Ed25519 audit-signing keypair (env + JWK forms).  |
| `seed-dev.ts`                 | `pnpm --filter @cerniq/scripts seed`                   | Idempotently seed a Principal, ApiKey, AgentIdentity, and ACTIVE policy.   |
| `verify-spec.ts`              | `pnpm --filter @cerniq/scripts verify-spec`            | OpenAPI ↔ Zod ↔ Prisma parity check. CI gate on contract drift.            |
| `cerniq-cli.ts`               | `pnpm --filter @cerniq/scripts run cerniq -- <subcmd>` | Operator CLI: register / agent / policy / verify / audit / trust / health. |
| `backtest-verify.ts`          | `pnpm --filter @cerniq/scripts run backtest-verify`    | Replay AuditEvent rows through the current verify algorithm; drift gate.   |
...
## `generate-cerniq-keys.ts`

Mints a fresh Ed25519 keypair and writes:

- **`./.local/keys/cerniq-signing.env`** (mode `0600`) — `CERNIQ_SIGNING_PRIVATE_KEY`, `CERNIQ_SIGNING_PUBLIC_KEY`, `CERNIQ_SIGNING_KID` (base64url encoded).
- **`./.local/keys/cerniq-signing.jwk.json`** — the public JWK consumed by the `/.well-known/audit-signing-key` endpoint. Shape: `{ kty: "OKP", crv: "Ed25519", kid, use: "sig", alg: "EdDSA", x }`.
...
**Production:** do not use this output directly. Generate keys inside your KMS (Railway secrets, AWS KMS, GCP KMS) and inject `CERNIQ_SIGNING_*` at deploy time.
...
1. `Principal` — `email = dev@cerniq.local`.
2. `ApiKey` — full-scope, plaintext shape `cerniq_sk_<22ch>`. **Only minted on first run.** The plaintext can never be recovered after that — bcrypt-hashed at rest. Use `--reset` to rotate.
...
> Requires `@cerniq/types` to have been built (`pnpm --filter @cerniq/types build`) so the script can import its compiled `./schemas` entry.
...
```sh
pnpm --filter @cerniq/scripts test
```

Coverage:

- `generate-cerniq-keys.spec.ts` — base64url roundtrip, kid stability, JWK shape, file mode `0600`, idempotency refusal, `--force` overwrite.
