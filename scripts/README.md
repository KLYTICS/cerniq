...
> `.local/`. The scripts here (and `okoro-cli`) write private keys and the
> `.okororc.json` operator state to `./.local/keys/` and `./.okororc.json` —
> add `.local/` and `.okororc.json` to `.gitignore` before committing.

| Script                        | Run as                                       | Purpose                                                                    |
| ----------------------------- | -------------------------------------------- | -------------------------------------------------------------------------- |
| `generate-okoro-keys.ts`      | `pnpm --filter @okoro/scripts keys`          | Mint the OKORO internal Ed25519 audit-signing keypair (env + JWK forms).   |
| `seed-dev.ts`                 | `pnpm --filter @okoro/scripts seed`          | Idempotently seed a Principal, ApiKey, AgentIdentity, and ACTIVE policy.   |
| `verify-spec.ts`              | `pnpm --filter @okoro/scripts verify-spec`   | OpenAPI ↔ Zod ↔ Prisma parity check. CI gate on contract drift.            |
| `okoro-cli.ts`                | `pnpm --filter @okoro/scripts run okoro -- <subcmd>` | Operator CLI: register / agent / policy / verify / audit / trust / health. |
| `backtest-verify.ts`          | `pnpm --filter @okoro/scripts run backtest-verify`   | Replay AuditEvent rows through the current verify algorithm; drift gate.   |
...
## `generate-okoro-keys.ts`

Mints a fresh Ed25519 keypair and writes:

- **`./.local/keys/okoro-signing.env`** (mode `0600`) — `OKORO_SIGNING_PRIVATE_KEY`, `OKORO_SIGNING_PUBLIC_KEY`, `OKORO_SIGNING_KID` (base64url encoded).
- **`./.local/keys/okoro-signing.jwk.json`** — the public JWK consumed by the `/.well-known/audit-signing-key` endpoint. Shape: `{ kty: "OKP", crv: "Ed25519", kid, use: "sig", alg: "EdDSA", x }`.
...
**Production:** do not use this output directly. Generate keys inside your KMS (Railway secrets, AWS KMS, GCP KMS) and inject `OKORO_SIGNING_*` at deploy time.
...
1. `Principal` — `email = dev@okoro.local`.
2. `ApiKey` — full-scope, plaintext shape `okoro_sk_<22ch>`. **Only minted on first run.** The plaintext can never be recovered after that — bcrypt-hashed at rest. Use `--reset` to rotate.
...
> Requires `@okoro/types` to have been built (`pnpm --filter @okoro/types build`) so the script can import its compiled `./schemas` entry.
...
```sh
pnpm --filter @okoro/scripts test
```

Coverage:

- `generate-okoro-keys.spec.ts` — base64url roundtrip, kid stability, JWK shape, file mode `0600`, idempotency refusal, `--force` overwrite.
