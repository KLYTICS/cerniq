 ...
 * pass strictly.
 *
 * Doubles as a demo runner: `pnpm --filter @okoro/e2e test 16_quickstart`
 * with `OKORO_E2E_VERBOSE=1` prints each step's outcome to stdout in the
 * narrative order an operator would experience.
 */

import { afterAll, beforeAll, describe, expect, it } from 'vitest';
import { Okoro, generateKeypair, signHandshake } from '@okoro/sdk';
import type { AgentRecord } from '@okoro/sdk';
...
const VERBOSE = process.env['OKORO_E2E_VERBOSE'] === '1';
...
describe('16 · quickstart workflow (the documented promise as a test)', () => {
  ...
  it('step 4 · Okoro.handshake() proves possession and lifts trust to ≥600', async () => {
  ...
  it('step 7 · signHandshake byte-format matches the API verify-handshake expectation', async () => {
    ...
    const message = `okoro-handshake-v1::${agentId || 'agt_demo'}::AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA`;
