 ...
 *   OKORO_STRIPE_WEBHOOK_SECRET            (== API's STRIPE_WEBHOOK_SECRET)
 *   OKORO_E2E_STRIPE_TEST_PRINCIPAL_ID     target principal id
 *   OKORO_E2E_STRIPE_DEVELOPER_PRICE_ID    price_id mapped to DEVELOPER
 *
...
describe('18 · Stripe subscription state machine', () => {
  ...
  beforeAll(() => {
    if (!fullCoverage) {
      const missing = [
        !SECRET && 'OKORO_STRIPE_WEBHOOK_SECRET',
        !TEST_PRINCIPAL_ID && 'OKORO_E2E_STRIPE_TEST_PRINCIPAL_ID',
        !DEVELOPER_PRICE_ID && 'OKORO_E2E_STRIPE_DEVELOPER_PRICE_ID',
      ].filter(Boolean);
      // eslint-disable-next-line no-console
      console.warn(
  ...
  async function countPlanChanged(stripeEventId: string): Promise<number> {
    ...
    const r = await fetch(`${cfg.baseUrl}/v1/audit-events/export`, {
      method: 'GET',
      headers: { 'x-okoro-key': cfg.apiKey },
    });
