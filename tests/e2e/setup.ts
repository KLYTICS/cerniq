/**
 * Vitest globalSetup — pings the CERNIQ API once before any test fork.
 *
 * Behavior:
 *   - If CERNIQ_E2E_URL/health/live returns 2xx within the timeout, we proceed.
 ...
 *   2. terminal B: `CERNIQ_E2E_URL=http://localhost:3000 \
 *                   CERNIQ_E2E_API_KEY=cerniq_sk_... pnpm --filter @cerniq/e2e test`
 */
...
export async function setup(): Promise<void> {
  const url = (process.env['CERNIQ_E2E_URL'] ?? DEFAULT_URL).replace(/\/+$/, '');
  ...
  if (!reachable) {
    banner([
      'CERNIQ API not reachable — skipping e2e suite.',
      '',
  ...
  if (!apiKey) {
    banner([
      'CERNIQ_E2E_API_KEY is not set — skipping e2e suite.',
      '',
      'The harness needs a valid management key (cerniq_sk_...) to register',
  ...
  banner([
    'CERNIQ e2e harness — preflight OK',
    `  url     ${url}`,
