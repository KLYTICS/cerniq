/**
 * Vitest globalSetup — pings the OKORO API once before any test fork.
 *
 * Behavior:
 *   - If OKORO_E2E_URL/health/live returns 2xx within the timeout, we proceed.
 ...
 *   2. terminal B: `OKORO_E2E_URL=http://localhost:3000 \
 *                   OKORO_E2E_API_KEY=okoro_sk_... pnpm --filter @okoro/e2e test`
 */
...
export async function setup(): Promise<void> {
  const url = (process.env['OKORO_E2E_URL'] ?? DEFAULT_URL).replace(/\/+$/, '');
  ...
  if (!reachable) {
    banner([
      'OKORO API not reachable — skipping e2e suite.',
      '',
  ...
  if (!apiKey) {
    banner([
      'OKORO_E2E_API_KEY is not set — skipping e2e suite.',
      '',
      'The harness needs a valid management key (okoro_sk_...) to register',
  ...
  banner([
    'OKORO e2e harness — preflight OK',
    `  url     ${url}`,
