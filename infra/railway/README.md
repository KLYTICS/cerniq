...
Phase 1 production target. AEGIS runs as four Railway services in a single
project, all in `us-east`:

| Service       | Descriptor                | Public ingress | Notes                              |
|---------------|---------------------------|----------------|------------------------------------|
| `aegis-api`   | `api.service.json`        | yes (HTTPS)    | NestJS, attaches a custom domain   |
| `aegis-worker`| `worker.service.json`     | no             | BATE / webhook / audit queues      |
| `aegis-pg`    | `postgres.service.json`   | no             | Or external Neon                   |
| `aegis-redis` | `redis.service.json`      | no             | Or external Upstash                |

The legacy `aegis-api.json` in this directory predates the Phase-1 split and
...
3. From the repo root: `railway link` — pick the AEGIS project; if it
...
4. Confirm the four services exist:
   ...
   ```
   Expected: `aegis-api`, `aegis-worker`, `aegis-pg`, `aegis-redis`.
...
```sh
pnpm tsx scripts/generate-aegis-keys.ts --env > /tmp/aegis-prod-keys
```

The script prints two Ed25519 keypairs as base64 env-var lines:
`JWT_ED25519_*` and `AUDIT_ED25519_*`. **Treat `/tmp/aegis-prod-keys` like
the master key it is** — pipe it directly into Railway:

```sh
while IFS='=' read -r key value; do
  railway variables set --service aegis-api "$key=$value"
done < /tmp/aegis-prod-keys
shred -u /tmp/aegis-prod-keys   # or `rm -P` on macOS
```

Mirror the **AUDIT_** keys to `aegis-worker` (the worker writes audit
...
railway variables set --service aegis-api \
  "DATABASE_URL=$(railway variables get --service aegis-pg DATABASE_URL)"
railway variables set --service aegis-worker \
  "DATABASE_URL=$(railway variables get --service aegis-pg DATABASE_URL)"
railway variables set --service aegis-api \
  "REDIS_URL=$(railway variables get --service aegis-redis REDIS_URL)"
railway variables set --service aegis-worker \
  "REDIS_URL=$(railway variables get --service aegis-redis REDIS_URL)"
```

(Railway also supports reference variables — `${{ aegis-pg.DATABASE_URL }}`
...
# Failure: process is crashlooping. Check `railway logs --service aegis-api`.
...
# Diagnose with `railway run --service aegis-api -- env | grep -E 'DATABASE|REDIS'`.
...
railway logs --service aegis-worker --lines 50 | grep -i "queue.*ready\|worker.*started"
...
- **Forward**: `railway up --service aegis-api` from a clean tree.
- **Rollback**: `railway redeploy <previous-deployment-id> --service aegis-api`.
  Get IDs with `railway deployments list --service aegis-api`.
