/**
 * Auth0 Action — log every successful login as an OKORO audit event.
 ...
 *   OKORO_ACTION_SECRET — shared secret matching `AUTH0_ACTION_SECRET` on the OKORO API.
 *
 * The Action runs server-side in Auth0; it has no OKORO API key — only
...
exports.onExecutePostLogin = async (event, api) => {
  const base = event.secrets.OKORO_API_BASE;
  const secret = event.secrets.OKORO_ACTION_SECRET;
  if (!base || !secret) {
    console.warn('okoro-audit-login: OKORO_API_BASE or OKORO_ACTION_SECRET not configured; skipping audit emit.');
    return;
  }
  ...
  const roles = (event.authorization?.roles ?? []).filter(
    (r) => typeof r === 'string' && r.startsWith('okoro:'),
  );
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`okoro-audit-login: OKORO returned ${res.status}`);
    }
  } catch (err) {
    console.warn(`okoro-audit-login: post failed: ${err && err.message ? err.message : err}`);
  }
};
