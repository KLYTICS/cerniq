#!/usr/bin/env bash
# =============================================================================
# AEGIS — Audit-signing key rotation driver
# =============================================================================
...
usage() {
  cat <<'EOF' >&2
rotate-aegis-keys.sh — drive an AEGIS audit-signing key rotation.

Usage:
  rotate-aegis-keys.sh [--execute] [--json] [--service <name>] [--out <dir>]
...
Cross-references:
  ...
  - scripts/generate-aegis-keys.ts      keypair generator (reused, never re-implemented here)
...
if [[ ! -f "${SCRIPT_DIR}/../../scripts/generate-aegis-keys.ts" ]]; then
  log_step "prereq" "cannot find scripts/generate-aegis-keys.ts from ${SCRIPT_DIR}"
  exit 3
fi
...
if [[ "${EXECUTE}" -eq 1 ]]; then
  log_step "generate" "running pnpm --filter @aegis/scripts run keys -- --out ${OUT_DIR} --format both"
  confirm "About to mint a NEW Ed25519 keypair into ${OUT_DIR}. Proceed?"
  if ! ( cd "${REPO_ROOT}" && pnpm --filter @aegis/scripts run keys -- \
  ...
  log_step "generate" "DRY RUN — would invoke: pnpm --filter @aegis/scripts run keys -- --out ${OUT_DIR} --format both"
fi
...
railway variables set AEGIS_SIGNING_PUBLIC_KEY_NEXT="<new-pub-b64url>" --service ${SERVICE}
railway variables set AEGIS_SIGNING_KID_NEXT="<new-kid>"               --service ${SERVICE}

# Restart the API service:
#   railway redeploy --service ${SERVICE}
# Then verify:
#   curl -fsS https://<your-aegis>/.well-known/jwks.json | jq '.keys | length'
...
railway variables set AEGIS_SIGNING_PRIVATE_KEY="<new-priv-b64url>" --service ${SERVICE}
railway variables set AEGIS_SIGNING_PUBLIC_KEY="<new-pub-b64url>"   --service ${SERVICE}
railway variables set AEGIS_SIGNING_KID="<new-kid>"                 --service ${SERVICE}

railway variables set AEGIS_SIGNING_KEY_PREVIOUS_PUBLIC_KEY="<old-pub-b64url>" --service ${SERVICE}
railway variables set AEGIS_SIGNING_KEY_PREVIOUS_KID="<old-kid>"               --service ${SERVICE}

railway variables delete AEGIS_SIGNING_PUBLIC_KEY_NEXT --service ${SERVICE}
railway variables delete AEGIS_SIGNING_KID_NEXT       --service ${SERVICE}

railway variables set AEGIS_SIGNING_KEY_ROTATED_AT="$(date -u +%FT%TZ)" --service ${SERVICE}
...
if [[ "${JSON}" -eq 1 ]]; then
  ...
  printf 'rotate-aegis-keys: mode=%s log=%s\n' \
    "$([[ ${EXECUTE} -eq 1 ]] && echo execute || echo dry-run)" \
    "${LOG_FILE}"
fi
