# CERNIQ — Audit-signing key rotation runbook

> Cadence: **quarterly** (calendar quarters; Q1=Mar, Q2=Jun, Q3=Sep, Q4=Dec, week 1).
> Scope: the CERNIQ Ed25519 audit-signing key — the keypair whose public
...
> Driver script: [`rotate-cerniq-keys.sh`](./rotate-cerniq-keys.sh).
> Generator: [`../../scripts/generate-cerniq-keys.ts`](../../scripts/generate-cerniq-keys.ts).
...
Post to status page: "CERNIQ audit-signing key will rotate on YYYY-MM-DD.
...
```bash
DATE="$(date -u +%Y%m%d)"
pnpm --filter @cerniq/scripts run keys -- \
...
- `./.local/keys/rotation-<DATE>/cerniq-signing.env` (mode 0600 — contains the **next** private key).
- `./.local/keys/rotation-<DATE>/cerniq-signing.jwk.json` (mode 0644 — public key + `kid`).
...
> [`scripts/generate-cerniq-keys.ts`](../../scripts/generate-cerniq-keys.ts)).
...
> `CERNIQ_SIGNING_PUBLIC_KEY_NEXT` (optional) and emit two JWK entries on
...
```
railway variables set CERNIQ_SIGNING_PUBLIC_KEY_NEXT="<new-pub-b64url>" --service api
railway variables set CERNIQ_SIGNING_KID_NEXT="<new-kid>"               --service api
```
...
```
# new becomes current
railway variables set CERNIQ_SIGNING_PRIVATE_KEY="<new-priv-b64url>"     --service api
railway variables set CERNIQ_SIGNING_PUBLIC_KEY="<new-pub-b64url>"       --service api
railway variables set CERNIQ_SIGNING_KID="<new-kid>"                     --service api

# old preserved for verification
railway variables set CERNIQ_SIGNING_KEY_PREVIOUS_PUBLIC_KEY="<old-pub-b64url>" --service api
railway variables set CERNIQ_SIGNING_KEY_PREVIOUS_KID="<old-kid>"               --service api

# clean up the staging variables
railway variables delete CERNIQ_SIGNING_PUBLIC_KEY_NEXT --service api
railway variables delete CERNIQ_SIGNING_KID_NEXT       --service api

# record the rotation moment for /.well-known/jwks.json metadata
railway variables set CERNIQ_SIGNING_KEY_ROTATED_AT="$(date -u +%FT%TZ)" --service api
```
...
- Audit chain verification passes for at least 100 events on either side
  of the cutover (`pnpm --filter @cerniq/api audit:verify-chain --since <T-1h>`
  once the CLI ships per `M-006-ext`).

## Step 5 — Backfill window (90 days)

Keep `CERNIQ_SIGNING_KEY_PREVIOUS_PUBLIC_KEY` set for **90 days**. SOC2
...
```
railway variables delete CERNIQ_SIGNING_KEY_PREVIOUS_PUBLIC_KEY --service api
railway variables delete CERNIQ_SIGNING_KEY_PREVIOUS_KID        --service api
```

Restart the API. Confirm the JWKS returns only the current key. Shred
`./.local/keys/rotation-<DATE>/cerniq-signing.env` from any operator
...
railway variables set CERNIQ_SIGNING_PRIVATE_KEY="<old-priv-b64url>" --service api
railway variables set CERNIQ_SIGNING_PUBLIC_KEY="<old-pub-b64url>"   --service api
railway variables set CERNIQ_SIGNING_KID="<old-kid>"                 --service api

# the new key, briefly active, becomes the previous (so historical
# events signed by it during the window remain verifiable)
railway variables set CERNIQ_SIGNING_KEY_PREVIOUS_PUBLIC_KEY="<new-pub-b64url>" --service api
railway variables set CERNIQ_SIGNING_KEY_PREVIOUS_KID="<new-kid>"               --service api
```
...
1. `wellknown.service.ts` does not yet emit a multi-key JWKS array.
   Spec: when `CERNIQ_SIGNING_PUBLIC_KEY_NEXT` (or
   `CERNIQ_SIGNING_KEY_PREVIOUS_PUBLIC_KEY`) is set, the JWKS response
...
3. Two-person concurrence on `--execute` is not yet enforced by
   `rotate-cerniq-keys.sh` — currently a single operator confirmation.
