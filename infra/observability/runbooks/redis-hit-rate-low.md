...
> `cerniq_cache_hits_total` / `cerniq_cache_misses_total`, which the
...
- **Cost**: Postgres read load scales linearly with the miss rate.
  At CERNIQ's traffic shape, a sustained drop from 95% → 70% hit
...
1. **Confirm the hit rate** (when exporter ships):
   ...
   ```bash
   railway run -s cerniq-redis -- redis-cli INFO stats | rg -F 'keyspace_hits|keyspace_misses'
   railway run -s cerniq-redis -- redis-cli INFO memory | rg -F 'used_memory_human|maxmemory_human|evicted_keys'
   ```
...
2. **Check the per-key-prefix hit rate.**
   ...
   ```bash
   railway run -s cerniq-redis -- redis-cli --scan --pattern 'agent:*' | wc -l
   railway run -s cerniq-redis -- redis-cli --scan --pattern 'policy:*' | wc -l
   railway run -s cerniq-redis -- redis-cli --scan --pattern 'trust:*' | wc -l
   ```
...
3. **Eviction check.** If `evicted_keys` is increasing, the cache is
   ...
   ```bash
   railway run -s cerniq-redis -- redis-cli CONFIG GET maxmemory
   railway run -s cerniq-redis -- redis-cli CONFIG GET maxmemory-policy
   ```
...
4. **Recent invalidation churn.** A deploy that introduced over-eager
   ...
   railway logs -s cerniq-api | rg -F 'cache.invalidate' | tail -50
   ```

5. **TTL inspection** for a sample key:

   ```bash
   railway run -s cerniq-redis -- redis-cli RANDOMKEY
   # take the returned key, then:
   railway run -s cerniq-redis -- redis-cli TTL '<key>'
   ```
...
- **Memory-bound (eviction-driven)**: scale Redis vertically —
  `railway service scale cerniq-redis --memory 2GB` (or appropriate
  tier). LRU eviction means warming back to a healthy hit rate
  takes ~minutes.
- **Over-eager invalidation from a recent deploy**: rollback the
  API: `railway rollback -s cerniq-api <prev-deploy-id>`. The cache
...
- For memory-bound scaling: update the default Railway memory setting
  in `infra/railway/cerniq-redis.json` and document in
...
```bash
railway run -s cerniq-redis -- redis-cli INFO stats | rg -F 'evicted_keys'
# Run twice 15 min apart; values should match.
```

## Escalate

- **Not resolved by next business day** → notify `#cerniq-oncall`
...
`#cerniq-ops`. **Yes** if the cache health degradation cascaded into a
