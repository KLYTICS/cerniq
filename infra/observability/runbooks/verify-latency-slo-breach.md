...
- **Group**: `aegis.verify.slo` (latency alerts), `aegis.platform`
  (event-loop alert)
...
1. **Confirm the breach is real and current** (Prometheus expression
   ...
   ```promql
   histogram_quantile(0.99, sum by (le) (rate(aegis_verify_latency_seconds_bucket[5m])))
   ```
...
2. **Decide: is this global, per-decision, or per-route?**
   ...
   histogram_quantile(0.99, sum by (le, decision) (rate(aegis_verify_latency_seconds_bucket[5m])))

   # By route (drift to non-verify routes implies platform-level issue)
   histogram_quantile(0.99, sum by (le, route) (rate(aegis_http_requests_total[5m])))
   ```

3. **Open the verify SLO Grafana dashboard.**
   `https://grafana.internal/d/aegis-verify-slo` — panel 1 (latency)
...
4. **Event loop lag — the leading indicator.**
   ...
   ```promql
   aegis_nodejs_eventloop_lag_seconds
   ```
...
5. **Postgres + Redis health.**
   ...
   railway logs -s aegis-api | rg "PrismaClientKnownRequestError|Pool" | tail -20

   # Postgres slow queries (init.sql sets log_min_duration_statement=500ms)
   railway run -s aegis-postgres -- psql -c "SELECT pid, now()-query_start AS dur, state, substring(query, 1, 80) FROM pg_stat_activity WHERE state='active' AND now()-query_start > interval '500ms' ORDER BY dur DESC LIMIT 10;"

   # Redis latency (verify path reads agent: / policy: / trust: keys)
   railway run -s aegis-redis -- redis-cli --latency-history -i 1
   ```

6. **Recent deploys.**

   ```bash
   railway deployments -s aegis-api --json | jq '.[0:3] | .[] | {createdAt, status, meta}'
   ```

   If a deploy landed within the breach window, suspect it first.

7. **OTel traces.** In your trace backend (Tempo/Jaeger), filter:
   `service.name="aegis-api" span.name="POST /v1/verify" duration > 200ms`
...
- **Recent deploy regression** → `railway rollback -s aegis-api <prev-deploy-id>`.
  Confirm latency drops within 2 min.
- **Postgres slow query** (one repeat offender in step 5):
  `railway run -s aegis-postgres -- psql -c "SELECT pg_terminate_backend(<pid>);"`
...
- **Redis latency spike** (>5 ms p99 sustained in step 5): scale Redis
  vertically — `railway service scale aegis-redis --memory 2GB` — and
  watch latency drop. If it doesn't, the issue is network/AZ; failover
  via Railway dashboard.
- **Pool exhaustion**: scale the API horizontally —
  `railway service scale aegis-api --replicas <n+1>`. Each replica
...
- **Event loop lag from sync work**: take a heap snapshot and a
  CPU profile via `railway run -s aegis-api -- node --prof` on a
...
- File a follow-up ticket in the issue tracker (project: AEGIS, label:
...
- If the cause was a regression, add a verify-path benchmark to
  ...
  The load gate (`pnpm --filter @aegis/api test:load`) runs before
  each release.
...
```promql
aegis_nodejs_eventloop_lag_seconds{quantile="0.99"} < 0.05
```
...
- **Customer reports impact** → page status-page owner; post
  acknowledgment at https://status.aegislabs.io within 5 min of report.
...
`#aegis-ops` so the pattern is visible.
