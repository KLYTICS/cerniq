...
One or more of:
1. `aegis_audit_retention_events_redacted_total` flat for > 25 hours when traffic is normal (the cron skipped — round 15 default is 24h interval).
...
1. **Confirm the symptom — when did retention last fire?**
   ...
   ```promql
   max(aegis_audit_retention_events_redacted_total) by (instance)
   # If flat for > 25h, the tick missed. Pair with:
   sum(rate(aegis_audit_retention_events_redacted_total[1d]))
   ```
   ...
   railway logs -s aegis-api | rg -F 'audit-retention' | tail -30
   ```
...
4. **Inspect the service interval handle.**
   ...
   ```bash
   railway run -s aegis-api -- node -e "
   ...
   "
   railway logs -s aegis-api | rg -F 'audit-retention' | tail -10
   ```
...
- **Cron skipped (no errors)**: trigger a manual run via the operator CLI, dry-run first to confirm scope:
  ...
  ```bash
  pnpm --filter @aegis/scripts run audit-retention -- --dry-run
  pnpm --filter @aegis/scripts run audit-retention   # commit
  pnpm --filter @aegis/scripts run audit-retention -- --principal-id=<id>  # narrow
  ```
...
- After the gap is closed, audit how it happened. Common causes:
  ...
  - The `AEGIS_AUDIT_RETENTION_INTERVAL_MS` env var was changed to something unreasonable (e.g., 31 days because someone confused ms and days).
...
- Add or verify the alert: a flat `aegis_audit_retention_events_redacted_total` for > 25h with traffic flowing.
...
increase(aegis_audit_retention_events_redacted_total[26h]) > 0
```
