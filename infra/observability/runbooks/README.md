# AEGIS — On-call runbooks

Every alert in `infra/observability/alerts/aegis.rules.yml` has exactly
...
- **Critical alert** — phone page via PagerDuty. Acknowledge within
  ...
  page update (https://status.aegislabs.io) within 15 min if the
  alert is still firing.
- **Warning alert** — PagerDuty notify (no page) + Slack
  `#aegis-oncall`. Triage within 30 min during business hours, by
  next business day if after-hours. No status page entry unless it
  escalates.
- **Info alert** — Slack `#aegis-ops` only. Triage during business
  hours.
...
3. **Cross-link the alert.** The `runbook` annotation on the alert in
   `aegis.rules.yml` must use the runbook's repo path; the
   `runbook_url` must use `https://docs.aegislabs.io/runbooks/<file
   without .md>`.
