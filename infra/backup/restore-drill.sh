#!/usr/bin/env bash
# =============================================================================
# OKORO — Production restore drill
# =============================================================================
# Verifies that the latest pgBackRest backup of the `okoro` stanza is restorable
...
TEMP_PG_NAME="okoro-restore-drill-$$"
TEMP_PG_PORT="55432"
TEMP_PG_PASSWORD=""
RESTORE_DIR=""
...
usage() {
  cat <<'EOF' >&2
restore-drill.sh — exercise the OKORO pgBackRest restore path.
...
RESTORE_DIR="$(mktemp -d -t okoro-restore-XXXXXX)"
...
for _ in $(seq 1 60); do
  if docker exec "${TEMP_PG_NAME}" pg_isready -U okoro >/dev/null 2>&1; then
    ready=1
    break
  fi
  sleep 1
...
for table in Principal AgentIdentity AuditEvent; do
  out="$(PGPASSWORD="${TEMP_PG_PASSWORD}" \
    psql -h 127.0.0.1 -p "${TEMP_PG_PORT}" -U okoro -d okoro -At \
...
# chain-walker CLI (pnpm --filter @okoro/api audit:verify-chain) is not yet
...
chain_detail=""
if ( cd / && pnpm --filter @okoro/api audit:verify-chain --since "${TARGET_TIME}" ) \
    >/tmp/okoro-chain-check.log 2>&1; then
  chain_status="OK"
elif grep -q 'No script matched' /tmp/okoro-chain-check.log 2>/dev/null \
  || grep -q 'No projects matched' /tmp/okoro-chain-check.log 2>/dev/null \
