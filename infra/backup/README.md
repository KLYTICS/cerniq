# CERNIQ — Backups (operator-facing)

> Stanza: `cerniq`. Engine: pgBackRest. Repo: S3-compatible object store
...
- `AgentIdentity` — the public-key registry. (Private keys are NOT held by CERNIQ — see `CLAUDE.md` invariant #1. We do not back up anything we never had.)
...
nothing usable. Rotate this passphrase on the same cadence as the CERNIQ
...
| RPO    | 5 min  | CERNIQ audit log is SOC2 evidence; >5 min of unrecoverable audit events would void the "complete records" claim under CC7.2. Drives `archive-async=y` + a 1 GiB push queue. |
...
5. **Audit chain verification**: the drill calls
   `pnpm --filter @cerniq/api audit:verify-chain --since <target>`. The
...
`pgbackrest verify --stanza=cerniq` and exits non-zero on any error. It
...
0 2 * * *    /opt/cerniq/infra/backup/verify-backup.sh --json >> /var/log/cerniq/verify-backup.log 2>&1

# Heavyweight weekly drill — exercises restore end-to-end.
0 3 * * 0    /opt/cerniq/infra/backup/restore-drill.sh --execute --json >> /var/log/cerniq/restore-drill.log 2>&1
```
