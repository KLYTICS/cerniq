...
# (optional) generate the CERNIQ audit-signing keypair and wire its public half
pnpm --filter @cerniq/scripts run keys
# then copy the CERNIQ_SIGNING_PUBLIC_KEY value into infra/dev/.env
...
# 2. Metrics flowing?
curl -s http://localhost:4000/metrics | grep ^cerniq_
...
# CERNIQ folder → "Verify path" dashboard provisioned
```
...
The Grafana dashboard mounted at `infra/dev/grafana/dashboards/cerniq-verify.json`
is a copy of the production dashboard `infra/observability/grafana-dashboards/cerniq-verify-latency.json`.
...
- `cerniq_verify_denials_total`
- `cerniq_bate_recompute_lag_seconds_bucket`
- `cerniq_bullmq_waiting_jobs`
- `cerniq_cache_hits_total`
- `cerniq_cache_misses_total`
...
`cerniq_verify_total{decision,denial_reason}`, `cerniq_bate_score_delta`,
`cerniq_audit_append_total`, `cerniq_webhook_delivery_total`,
`cerniq_http_requests_total`, default Node metrics — render correctly.
...
| ----------------------------------------------- | ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| `api` container crash-loops with Prisma errors  | Migrations not applied                            | `pnpm --filter @cerniq/api exec prisma migrate deploy` against this DB.                                                        |
| `api` boot fails on `CERNIQ_SIGNING_PUBLIC_KEY` | Public key env var unset                          | `pnpm --filter @cerniq/scripts run keys`, copy `CERNIQ_SIGNING_PUBLIC_KEY` into `infra/dev/.env`, restart.                     |
...
- **Alertmanager.** The `cerniq.rules.yml` rules file is mounted into Prometheus
...
- **Real KMS.** `CERNIQ_SIGNING_PUBLIC_KEY` is read from the `.env` file in dev.
